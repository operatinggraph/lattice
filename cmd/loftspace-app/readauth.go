...
// the shared verify-only seam (internal/gateway/auth): the app verifies an IdP
// JWT and uses the verified subject as the RLS principal. The app never holds
// authorization logic — Postgres RLS is the single authorization source.
//
...
func setupReadAuth(logger *slog.Logger, loopback bool, revocationChecker auth.RevocationChecker) (*auth.Authenticator, *devSigner, error) {
	if isTruthy(os.Getenv("LOFTSPACE_APP_DEV_AUTH")) {
		...
		if !loopback {
			return nil, nil, fmt.Errorf("LOFTSPACE_APP_DEV_AUTH is only permitted on a loopback bind (the in-process minter trusts any subject); use LOFTSPACE_APP_JWT_PUBLIC_KEY for a non-local deployment")
		}
...
func (s *server) authenticateRead(r *http.Request) (auth.VerifiedActor, error) {
	if s.authn == nil {
		return auth.VerifiedActor{}, fmt.Errorf("read boundary not configured (set LOFTSPACE_APP_DEV_AUTH or LOFTSPACE_APP_JWT_PUBLIC_KEY)")
	}
	tok := bearerToken(r)
	if tok == "" {
