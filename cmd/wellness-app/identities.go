...
// wellnessIdentitiesRead as the SIGNED-IN actor — mirroring
// cmd/loftspace-app's handleStaffIdentities / applicantRosterRead. The lens
// is a Secure Lens (Contract #3 §3.10): the identity `name` is a sensitive
// aspect the Refractor decrypts into this RLS-protected table alone, so an
// unauthenticated full-name roster cannot exist.
//
// Unlike loftspace's applicantRosterRead, each row is SELF-anchored on the
// identity's own bare NanoID (packages/wellness-domain/lenses.go), so an
// ordinary signed-in member reads their OWN row via the platform's base
// cap-read self-grant — not only an actor holding the reserved
// WildcardAnchor grant. The FE reads this to resolve "Signed in as <name>"
// for every hat, not just staff.

// protectedIdentityRow is one row of the wellnessIdentitiesRead protected
...
func (s *server) handleIdentities(w http.ResponseWriter, r *http.Request) {
	...
	if s.pgPool == nil {
		s.logger.Error("identities protected read requested but pgPool is nil (set WELLNESS_APP_PG_DSN + ensure Postgres and the wellness-domain protected lens are up)")
