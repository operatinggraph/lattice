...
// suite, since the flattening logic is shared verbatim; this proves
// wellness-app wires its own toDescriptor the same way rather than assuming it
// by mirror. In particular, deleting `Enumerations: p.DispatchEnumerations`
// from this app's toDescriptor would leave the whole tree green without this
// test.
func TestComputeOpCatalog_ReNestsTheDescriptorVocabulary(t *testing.T) {
	const key = "vtx.meta.wo"
	...
	got := computeOpCatalog([]string{key}, get)
...
func TestOpCatalogKeysFromTypesParam(t *testing.T) {
	...
	got := opCatalogKeysFromTypesParam("CreateInstructor,SetInstructorProfile")
	want := []string{"CreateInstructor", "SetInstructorProfile"}
...
// the cache identifier (and of submitBillingEntry, whose opType argument
// indexes the cache) must be classified as a known non-read or as a read this
// test extracts a name from. An occurrence the classifier does not recognise
// — optional chaining, destructuring, a variable index — fails loudly rather
// than passing as "not a read"; the author extends the classifier and the
// literal together.
func TestKnownCatalogOpsCoversEveryCacheRead(t *testing.T) {
	...
	// loadOpCatalog, the truthiness guards, and the one variable index
	// (`[opType]` inside submitBillingEntry, whose names come from the call
	// sites classified next) are the non-reads; a dotted property or a
	...
	classify("opCatalogCache",
		...
		[]*regexp.Regexp{
			...
			regexp.MustCompile(`^opCatalogCache\[opType\]`),
		},
		[]*regexp.Regexp{
			regexp.MustCompile(`^opCatalogCache\.([A-Z][A-Za-z0-9_]*)`),
			regexp.MustCompile(`^opCatalogCache\["([A-Z][A-Za-z0-9_]*)"\]`),
		})
	if !regexp.MustCompile(`opCatalogCache\[opType\]`).MatchString(app) {
		t.Fatal("app.js: no `opCatalogCache[opType]` read found — submitBillingEntry no longer indexes the cache by its opType argument, so the call-site scan guards nothing")
	}
	// submitBillingEntry: its definition, and calls whose first argument is a
	// string literal (the op the `[opType]` read above resolves to).
	classify("submitBillingEntry",
		nil,
		[]*regexp.Regexp{regexp.MustCompile(`^submitBillingEntry\(opType, `)},
		[]*regexp.Regexp{regexp.MustCompile(`^submitBillingEntry\("([A-Z][A-Za-z0-9_]*)"`)})
