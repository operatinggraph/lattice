...
// ---- Tasks ----
// The task inbox. Each row is link-sourced server-side (GET /api/tasks):
// assignee / operation / target from the task's links, and the operation's
// human label (name + description) resolved from its forOperation meta-vertex.
...
// startTaskOp jumps to Submit Op with the task's operation pre-selected (or the
// operationType override filled when it is not a catalog command), so the
