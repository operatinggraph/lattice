...
func TestComputeSystemMapPendingReadpathExcludedFromRollup(t *testing.T) {
	...
	m := computeSystemMap(keys, read, nil, spec, time.Minute)
	if m.Overall != "green" {
...
func TestComputeSystemMapAlertsAndBootstrapFoldIntoOverall(t *testing.T) {
	...
	if m := computeSystemMap(keysOf(withAlert), read, nil, nil, time.Minute); m.Overall != "yellow" {
	...
	if m := computeSystemMap(keysOf(noBoot), read, nil, nil, time.Minute); m.Overall != "red" {
