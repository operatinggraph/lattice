...
// instead of rendering both as "no results". Silent when the health lookup
// itself could not resolve a status (nil conn, unreachable health-kv, a rule
// that has never reported) — never a false projectionHealthy either way.
