...
// AckerImpl is Story 1.8's step-10 implementation. It wraps the original
// jetstream.Msg and calls Ack(ctx) on it. The explicit Acker boundary
// (separate from step 1's natural ack) makes step 10 testable for fault
// injection (NFR-R1): an AckerImpl wrapped by FailAfterN can simulate
// a crash exactly at step 10.
//
...
// Ack implements Acker (step 10). It calls jetstream.Msg.Ack and
// surfaces ack errors. If ack fails, the durable consumer will
// redeliver — Contract #4 §4.5 + Story 1.5 tracker dedup makes this
// safe (step 2 short-circuits).
