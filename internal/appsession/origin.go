...
// Sec-Fetch-Site, which the caller has already found non-empty.
//
// The one cross-origin shape it admits is a top-level GET navigation
// (Sec-Fetch-Mode: navigate): a link, bookmark or redirect that lands the user
// on a page. Refusing those would break ordinary navigation between the
// co-hosted apps, and a navigation is the user's own act — unlike a
// subresource or a scripted fetch, which the initiating page chose alone.
func metadataAdmits(site string, r *http.Request) bool {
	switch strings.ToLower(site) {
	case "same-origin", "none":
		return true
	}
	return !stateChanging(r.Method) && strings.EqualFold(r.Header.Get("Sec-Fetch-Mode"), "navigate")
}

// requestScheme is the scheme the request actually arrived over, which is what
// the app's own pages carry in their Origin. It is read from the connection,
// never from a forwarding header: X-Forwarded-Proto is set by whoever spoke to
// us, so trusting it would let a caller name its own scheme and satisfy the
// Origin↔Host agreement below. A proxied deployment declares its origin instead
// (PublicOrigin), which is compared against a boot-time constant.
...
// CrossOriginBlocked reports whether r came from somewhere other than this
// app's own origin, writing the 403 when it did. RequireSession calls it ahead
// of everything else, including the auth-exempt session endpoints: a forced
...
// the explicitly-configured bind host (the warned-about non-loopback opt-in), or
// the declared public origin. Origin "null" (a sandboxed iframe, some
// redirects) has no host and fails closed.
func (m *Manager) CrossOriginBlocked(w http.ResponseWriter, r *http.Request) bool {
	if site := r.Header.Get("Sec-Fetch-Site"); site != "" && !metadataAdmits(site, r) {
		m.writeError(w, http.StatusForbidden, "cross-origin request blocked (Sec-Fetch-Site "+site+")")
		return true
	}
	...
	if m.cfg.PublicOrigin.matches(origin) {
		return false
	}
	if origin == requestScheme(r)+"://"+r.Host {
		if u, err := url.Parse(origin); err == nil {
			if h := normalizeOriginHost(u.Hostname()); h != "" {
				if IsLoopbackHost(h) || (m.cfg.BindHost != "" && h == normalizeOriginHost(m.cfg.BindHost)) {
					return false
				}
			}
		}
	}
	m.writeError(w, http.StatusForbidden, "cross-origin request blocked (Origin "+origin+")")
	return true
}
