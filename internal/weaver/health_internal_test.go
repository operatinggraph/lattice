...
// TestShouldLogPaced_RationsLoudRecordsAgainstAClock pins the pacing decision at
...
func TestShouldLogPaced_RationsLoudRecordsAgainstAClock(t *testing.T) {
	t.Parallel()
	...
	t.Run("one loud record per interval at the sweep cadence", func(t *testing.T) {
		...
		for minute := 0; minute < 120; minute++ {
			if c.shouldLogPaced(key, sev, code, start.Add(time.Duration(minute)*time.Minute)) {
				loud++
	...
	t.Run("an issues.clear between passes does not restore loudness", func(t *testing.T) {
		c := newIssueCache()
		start := time.Date(2026, 8, 25, 9, 0, 0, 0, time.UTC)
		if !c.shouldLogPaced(key, sev, code, start) {
		...
		c.clear(key)
		if c.shouldLogPaced(key, sev, code, start.Add(time.Minute)) {
	...
	t.Run("a different severity or code is loud at once", func(t *testing.T) {
		c := newIssueCache()
		start := time.Date(2026, 8, 25, 9, 0, 0, 0, time.UTC)
		if !c.shouldLogPaced(key, "warning", "UnresolvedReference", start) {
			t.Fatal("setup: the arrival must be loud")
		}
		if !c.shouldLogPaced(key, "error", "UnresolvedReference", start.Add(time.Minute)) {
			t.Fatal("a severity change at one key is a different fact and must not wait out the window")
		}
		if !c.shouldLogPaced(key, "error", "PlaybookConfigError", start.Add(2*time.Minute)) {
		...
		if c.shouldLogPaced(key, "error", "PlaybookConfigError", start.Add(3*time.Minute)) {
	...
	t.Run("clearPrefix takes the pace entry with the issue", func(t *testing.T) {
		...
		for _, k := range []string{revoked, survivor} {
			if !c.shouldLogPaced(k, sev, code, start) {
		...
		if !c.shouldLogPaced(revoked, sev, code, start.Add(time.Minute)) {
			t.Fatal("a revoked target's pace entry must go with its issue; a re-registration's first raise is an arrival")
		}
		if c.shouldLogPaced(survivor, sev, code, start.Add(time.Minute)) {
	...
	t.Run("prunePaced bounds the map and a pruned key re-arrives loud", func(t *testing.T) {
		...
		c.shouldLogPaced(stale, sev, code, start)
		c.shouldLogPaced(recent, sev, code, start.Add(90*time.Minute))
		...
		if !c.shouldLogPaced(stale, sev, code, start.Add(2*logPaceInterval)) {
			t.Fatal("a pruned key's next raise must be loud")
		}
		if c.shouldLogPaced(recent, sev, code, start.Add(2*logPaceInterval)) {
...
func TestEmit_PrunesThePaceMemory(t *testing.T) {
	...
	now := time.Now()
	cache.shouldLogPaced(aged, "warning", "UnresolvedReference", now.Add(-3*logPaceInterval))
	cache.shouldLogPaced(live, "warning", "UnresolvedReference", now)
