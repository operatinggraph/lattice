...
func TestMarkClaimID_MintedThenPreserved(t *testing.T) {
	...
	rev, claim1, exists, err := m.create(ctx, tID, eID, gap, eKey, "triggerLoom", "", "", 0)
	if err != nil || exists {
	...
	if _, conflict, err := m.replace(ctx, tID, eID, gap, eKey, "triggerLoom", "", "", claim1, 0, rev, markTTLBackstopFactor*m.lease); err != nil || conflict {
	...
	_, claim2, _, err := m.create(ctx, tID, eID, gap, eKey, "triggerLoom", "", "", 0)
	if err != nil {
...
func TestDeleteByTargetPrefix_OnlyMatchesOwnTarget(t *testing.T) {
	...
	// t1's marks.
	if _, _, _, err := m.create(ctx, "t1", "entityAAAAAAAAAAAAAA", "missing_a", "vtx.entity.entityAAAAAAAAAAAAAA", "MarkExpired", "", "", 0); err != nil {
	...
	if _, _, _, err := m.create(ctx, "t10", "entityBBBBBBBBBBBBBB", "missing_b", "vtx.entity.entityBBBBBBBBBBBBBB", "MarkExpired", "", "", 0); err != nil {
