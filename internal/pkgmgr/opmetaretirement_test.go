...
// ops.default (CancelTask — a normal business op, never routed on the meta
// lane). A single CommitPath is load-bearing here, not cosmetic: the DDL
// cache a CommitPath resolves operationTypes against is populated by its own
// CDC watch over vtx.meta.>, so two independent CommitPath instances each
// carry an independently-lagging cache — the DDL this harness installs on
// one instance is not guaranteed visible yet on the other's. One instance
...
func newDualLaneInstallerHarness(t *testing.T) (context.Context, *substrate.Conn, *Installer) {
	...
	// Direct bootstrap.LoadOrGenerate, not testutil.EnsurePrimordials: this
	// file is package pkgmgr, and testutil imports pkgmgr — the same
	// import-cycle exemption installer_test.go's newInstallerHarness already
	// documents (also named in bootstrap-primordial-globals-race-design.md
	// §3.2's census as pre-existing, tracked debt for that design's own
	// Fire 2, not this fire's scope).
...
// against it, ops.meta and ops.default, so package-lifecycle ops and
// CancelTask share one DDL cache (see newDualLaneInstallerHarness's doc
// comment for why that has to be one instance, not two).
