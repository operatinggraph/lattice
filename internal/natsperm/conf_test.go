...
// env.Actor over the transport (#75 Fire 2b). All three vertical apps are closed:
...
func TestVerticalAppOpsPublishDenied(t *testing.T) {
	url := startServerFromConf(t)
	assertDeniedPublish(t, url, "ops.default", []string{"clinic-app", "cafe-app", "loftspace-app"})
}
