...
var corpusGroupingVerdicts = map[string]groupingVerdict{
	...
	"edgeEntitySessions#0":           {"key(instr sess studio) p!instr", 0},
	"edgeEntitySessions#1":           {"key(instr sess studio) p!instr", 0},
	...
	"edgeManifestReadGrants":         {"key(identity) key(identity)+1 key(identity)+2 key(identity)+3 key(identity)+4 key(identity)+5 key(identity)+6 key(identity)+7 key(identity)+8 p!identity", 8},
	"edgeManifestStaffReadGrants":    {"key(identity) key(identity)+1 key(identity)+2 key(identity)+3 key(identity)+4 p!identity", 4},
	"edgeManifestTaskReadGrants":     {"key(identity) p!identity", 0},
	...
	"tenancyEnd":                        {"key(endedAt entityKey landlordDecision lapsedAt leaseEnd marketFrom moveOutAt residenceLinkKey signedAt termEnd unitAvailableFrom unitKey unitStatus) p!landlordDecision", 0},
	"leaseRentSettlement":               {"key(accountKey decision depositAmount endedAt entityKey lateFeeCents leaseEnd leaseStart moveOutAt requestedRent termRent termStart) p!decision", 0},
	"ledgerHistory":                     {"p", 0},
	...
	"opCatalog":                         {"key(ceremonyMintedSecretHashField ceremonyRevealHelp ceremonyRevealTitle description dispatchAuthContext dispatchClass dispatchClassChoices dispatchContextParams dispatchEnumerations dispatchOptionalReads dispatchReads dispatchTargetField dispatchTargetType dispatchVisibleWhen fieldDescriptions group icon inputSchema opMetaKey operationType sensitive shortLabel submitLabel title tone)", 0},
	...
	"providerSites":                     {"p", 0},
	"renewalComplete":                   {"key(entityKey guarantorVerifiedAt hasGuarantor leaseAppKey leaseappAlive noticeMoveOutAt signalsSubmittedAt signedAt status tenancyEndedAt tenant termsSetAt termsTermMonths) p!leaseAppKey", 0},
	"renewalsRead":                      {"key(cancelReason cycleEnd entityKey guarantorMethod guarantorVerifiedAt hasGuarantor leaseAppKey noticeGivenAt noticeGivenBy noticeMoveOutAt rentAmount signedAt status tenancyEndedAt tenancyLeaseEnd tenantKey tenantNameEnv termMonths termsSetAt unitAddress) p!cancelReason", 0},
	...
	"wellnessBookers":                   {"p", 0},
	"wellnessArrearsReminders":          {"p", 0},
	"wellnessBookingChangeNotices":      {"p", 0},
