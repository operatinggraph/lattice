...
func TestEvaluateForEntry_CompositeKeyProducer_DeliversAllKeyColumns(t *testing.T) {
	...
	results, _, _, err := threaded.evaluateForEntry(ctx, threaded.ruleState(), liveEntry)
	require.NoError(t, err)
	...
	legacy, _, _, err := legacyPipeline.evaluateForEntry(ctx, legacyPipeline.ruleState(), liveEntry)
	require.NoError(t, err)
...
func TestEvaluateForEntry_CompositeKeyGrant_RetractsOnTombstone(t *testing.T) {
	...
	results, _, _, err := p.evaluateForEntry(ctx, p.ruleState(), tombstone)
	require.NoError(t, err)
	require.Len(t, results, 1)
