...
func TestEvaluateForEntry_PlainAnchorTombstone_Retracts(t *testing.T) {
	...
	results, _, err := p.evaluateForEntry(ctx, p.ruleState(), liveEntry)
	require.NoError(t, err)
	...
	results, _, err = p.evaluateForEntry(ctx, p.ruleState(), tombstoneEntry)
	require.NoError(t, err)
	require.Len(t, results, 1)
	...
	results, _, err = p.evaluateForEntry(ctx, p.ruleState(), secondaryEntry)
	require.NoError(t, err)
