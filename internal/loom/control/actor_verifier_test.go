...
// wire for Loom: with an ActorVerifier configured, a signed actor JWT on the
// HeaderActor value resolves to the verified vtx.identity.<sub> before
...
func TestControl_ActorVerifier_ValidTokenResolvesVerifiedActor(t *testing.T) {
	...
	_, err = nc.RequestMsg(controlauth.NewActorRequestMsg(control.ListSubject(), token), 2*time.Second)
	require.NoError(t, err)
