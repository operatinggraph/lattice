...
// controlauth.CapabilityKVChecker (cmd/loom aborts startup if that
...
// internal/weaver/control.StubCapabilityChecker.
...
// hits it (cmd/loom wires a real controlauth.CapabilityKVChecker), so a denial
// from it signals a wiring regression.
...
func (d *denyAllChecker) Authorize(_ context.Context, actor, op, targetID string) error {
	d.logger.Warn("loom control capability checker unconfigured: DENY",
