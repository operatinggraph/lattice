...
- **A consumer definition is never edited in place — a filter narrowing or a server-immutable field is a
  delete-then-create in BOTH directions** — messages pending under the old filter never redeliver under the new
  one (widen-then-drain or recreate); JetStream refuses to update `DeliverPolicy` *or* `OptStartSeq` on an
  existing consumer (`nats-server` 2.14 `server/consumer.go:2435,:2438`), so deleting only when a caller names a
  position leaves the *unpositioned* attach unable to recreate a positioned durable at all. Minted: the JetStream
  filter-narrowing incident; cold-sign-in Fire 2 (`b44b667b`). Check:
  `TestNatsTransport_UnpositionedAttachAfterAPositionedDurable`; the narrowing arm has none yet.
