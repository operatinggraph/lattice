...
**A perEntry lens writes only the entries an event changed, and `Reproject` refuses the two
retractions the rewrite was silently fencing.** A `cap-read.<domain>.<actor>.<anchor>` producer
re-evaluates every reached actor whole, and the guarded adapter never skips an identical row
(its watermark advance is the only ordering it can see). So the pipeline decides one level up:
`multiEntryRetractions`, which already lists the actor's stored keys for the prefix diff, reads
back the bodies of the keys it is about to rewrite in one batched `adapter.RowsReader.GetRows`
and marks each fresh entry whose stored body is `rowsEquivalent` — the sweep's own converged
predicate — as `EvalResult.Unchanged`; `writeResults` then writes nothing for it, audits nothing,
and stamps only the freshness clock, and `Reproject` folds it `converged` without its own read
(perentry-unchanged-entry-withholding-design.md §4). A tombstone is never withheld, an entry the
store does not hold — absent, tombstoned, unparseable — is always written, a failed read-back
writes that actor's entries unconditionally and moves a counter, and the mechanism is armed per
call only for the perEntry family on a guarded, read-back-capable target with no Secure decryptor.

What the unconditional rewrite was doing beyond ordering — and what its removal has to
replace — is on the **presence** axis. A withheld entry keeps the watermark of the write that
last changed it, and the ordering token still orders every writer (each captures its token
before it evaluates, so an older evaluation can never outrank the write it races). But
`Reproject` reads edges from the `refractor-adjacency` index, a separately cursored durable, so
its evaluation can read an anchor absent that Core KV already holds; today's rewrite re-stamped
every busy actor's entries at the head and so incidentally declined such a tombstone. Two
explicit refusals in `Reproject`'s delete arm replace that accident: a dropped-entry tombstone
the prefix diff derived from the index's edge view lands only when the adjacency bootstrapper's
applied cursor (`consumer.Bootstrapper.AppliedSeq`, the contiguous floor of what the shared
durable has retired — a Nak'd message caps it until it retires) has reached the reconciliation
token, and a pass that a rebuild opened under (the rebuild generation moved, or a window is
still open) abandons every remaining write. Both fold `blocked`/`unknown` and the next pass
retries; neither applies to a retraction whose absence came from a live Core KV read (the
missing-actor branch, a doc-mode lens's document), which the index never served.
...
| Audit | `health.AuditWriter` | `lattice.refractor.audit.<lensId>` | Per-**committed**-projection audit append (see the Audit subjects row): a write that stored no row appends no entry, and neither does a perEntry entry the write loop withheld as already stored — that decision is counted on the lens entry's `entriesWithheld` instead. |
...
forward or allow-listed as writer-owned with a reason), *a hand-maintained struct round trip's omitted field is fail-OPEN because the zero value is the admitting answer* (`TestRuleState_RoundTripCarriesEveryField`, which discovers the field universe from `rulestate.go` at test time and fails by field name unless each field is read into the snapshot and written back through the same `Pipeline` field). *An index whose entries are read from one place and gated from another must agree about absence — a present-but-EMPTY set and a missing one are the same answer, and a corrupt member of a set read fails only where it is used* (both halves mandated test shapes: `TestAnchorHopIndex_EmptyExpansionIsUnresolved` for every absence gate over a resolved-set field, asserting the resolved and the empty vector against the same index; `TestReadableAnchors_MalformedBodyDeniesOnlyThatAnchor` / `TestPrefetch_CorruptBodyFailsOnlyWhereItIsUsed` / `TestNatsKVAdapter_GetRows_ChunksStripsAndSurvivesACorruptMember` for every batch or set reader; the standing rule for the reader — `len(x) == 0`, never `x == nil`, wherever "no answer" and "the answer is nothing" must behave alike — stays in force).
...
- **A fence you remove was doing unstated work on a SECOND axis — re-derive the ordering token's coverage before trusting it, and a replacement refusal's CURSOR needs a re-arm path and a family gate.** Minted: perEntry unchanged-entry withholding (2026-09-05). The unconditional rewrite of every entry looked like pure ordering (the watermark) and was also an incidental presence fence — `Reproject` reads edges from a separately-cursored index, and a busy actor's head-stamped watermark was what declined its stale tombstones; a first draft added a per-actor lock the ordering token already made unnecessary, and the design's own §14 candidate line (*a serialization added to protect a skipped write should first be re-derived against the ordering token the write was carrying*) is this entry's first half. The second half came from the build's three cold reviews, all on the replacement refusal: its cursor was a maximum over retired sequences (a Nak'd message with twenty Acks behind it read as applied), its poll read the drained check before the head it then claimed, its one-shot head-raise could leave a quiet process refusing every retraction for its lifetime, and it fired on every `Delete` in the arm — the missing-actor tombstones (a live Core KV read) and every doc-mode lens included — so lenses that could not withhold paid an over-grant for a fence they never lost. Check: for any "don't do X" a design lifts, list every consumer whose verdict X was incidentally stamping and state the axis each one reads (ordering vs presence); for any new fail-closed cursor, write its state table with a re-arm path (a refusing cursor whose only recovery is a restart converts a transient lag into a standing refusal) and name the producer whose absence verdict it actually covers — gate the refusal on that producer's mark, not on the arm it sits in. Pinned by `TestReproject_IndexBehindSparesEveryRetractionTheIndexDidNotDecide`, `TestAppliedSeq_IsAContiguousFloorNotAMaximum`, `TestPollReady_ReadsTheStreamHeadBeforeTheDrainedCheck`. Corollary sighted on the same item: a struct-tag `omitempty` silently overrides a field doc's absent-vs-zero contract — the tag, not the prose, is what the reader receives; write the doc from the tag.
  **Corollary (adjacency edge-ordering guard, 2026-09-07) — a guard that DECLINES needs three things the
  guard itself cannot show you, and all three were proven missing by mutations that left the suite green.**
  (a) *Distinguishable from a no-op:* the decline was replaced by a fall-through that skipped the mutation
  but still rewrote the identical document — green, while churning the bucket revision and manufacturing CAS
  conflicts for the two other writers of that node. Check: assert the target's KV **revision** is unchanged
  across a declined write, never only that the content is right. (b) *Observable:* the decline was a bare
  `return nil` while the precedent it cited (`SeqGuarded`'s `DeclinedByWatermark` → `VerdictBlocked`) is
  deliberately loud — a guard nobody can count is a guard nobody can tell is switched off. Check: a counter
  plus a line naming the offered value and the floor that refused it. (c) *Pinned at its PRODUCER:* all four
  sites threading the ordering value could be set to `0` with the whole 26-package tree green, because every
  pin hand-built the value at the consumer. This re-commits the standing checklist's plumbing rule, so the
  check is now a **mandated test shape**: one pin per producer asserting the persisted value **equals the
  delivering message's own sequence**, read from the fixture's own publish — not merely non-zero, and never
  from a hand-built event. A lint is the wrong instrument here (the defect is an absent test, not a
  detectable code shape); the shape is the gate.
  **Same item, fourth sighting of *a soundness claim's stated REASON is load-bearing*** — in one diff: an
  eviction bound justified by a time argument whose two premises (`MaxDeliver` bounded, a lens's lag bounded
  by its consumer) are both false against the tree; `edgeFloor`'s max justified by a document no binary can
  produce, since both fields land in one change; two pre-apply comments left promising guarantees the change
  had just removed, four lines above their own refutation; and a §1 failure route that `History: 1` defeats,
  the real route being an already-fetched message held 20–190 s in a handler. Check: every rationale sentence
  names the `file:line` that makes it true, and that line is read before the sentence ships.
