...
| **Actions** | `triggerLoom` (→ `StartLoomPattern` op, never a Go call), `assignTask` (→ `CreateTask` with episode-deterministic `taskId`), `directOp` ✅. External idempotent I/O is `triggerLoom` of a Loom `externalTask` pattern — the **bridge** executes the call (`docs/components/bridge.md`); Weaver never holds an adapter. `proposedOp` ✅ (the Augur's Fire 2b "Augur dispatch") sources its op + params from the ROW — not playbook config — for the primordial `augurDispatch` target only: it materialises an approved proposal's `proposedAction`/`proposedParams` after a dispatch-time re-validation (action vocabulary + default-deny scope to the TRUSTED candidate + live-registry resolution), fires it under a **proposal-scoped deterministic `requestId`** (collapse-only under a sweep reclaim, unlike every other action's episode-scoped id), then submits `RecordProposalDispatch` as a same-dispatch follow-up (a failed follow-up publish self-heals on the next sweep, never Naks the episode). A **plan-shaped** proposal (`proposedSteps`) dispatches **one leg per episode**: the row's `dispatchLeg` selects the step, the requestId and the flip are leg-scoped, the mark pins the leg (`proposalLeg`), and a `proposedOp` mark whose leg the row has passed is released (revision-conditioned, at lane 1 and at the sweep) so the next leg fires as a fresh episode — legs are ordered because every Weaver op rides one `ops.<lane>` the Processor drains serially. `surface` ✅ (FR29 — `orchestration-base`'s `unroutedTasks` target) dispatches **no op and creates no mark**: a row holding the column open joins that column's in-memory open-row set, and the set is reflected into **one** named `IssueCode`/`IssueSeverity` Health-KV issue per (target, gap column) (default severity `warning`) carrying the count of open rows this instance observes — rewritten on every membership change, retired when the last open row closes. The fact is the target's open WORKLOAD, not a per-row fault, so it is target-scoped and consumes none of the per-row issue budget; per-row identity is read from the target's projected row set. The set holds the VIOLATING rows this instance has observed with the column open, and a row leaves it only on one of four witnessed closes: its column READ false (the level-reconciled mark-clearing pass — `clearClosedMarks` special-cases it, a surface gap never having had a mark to clean up — or either of the reconciler sweep's own gap-close legs), the row delivered non-violating (the lens's verdict about every column of it at once), an entity tombstone, or a target teardown. Every one of those turns on a value the leg actually READ: a row merely ABSENT from the read model is not evidence — it may be mid-rebuild — and a column or a `violating` flag projected as a present non-bool states nothing either, so neither ever shrinks the count. Manual-intervention-only; unlike every other action it never touches `ops.<lane>`. An action the playbook names outside this set fails closed (a loud `PlaybookConfigError` Health issue), never a silent skip. |
