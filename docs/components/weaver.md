...
### Decline classes — what a failed delivery does to the message

A lane-1 delivery that cannot dispatch has three dispositions, and which one a failure takes is
decided by **where its fix comes from**
(`_bmad-output/implementation-artifacts/weaver-decline-retry-substrate-native-design.md` §3.2).
A row that Naks stays in the consumer's **pending set** and re-delivers on its floor, re-evaluated
against the *current* registry and the *current* row — so a package edit is taken up with no rebuild
and no re-projection. A row that Acks keeps only its standing Health issue.

| Failure | Disposition | Why |
|---|---|---|
| Unresolved reference (a pattern/operation the deployment has not installed yet) | **Nak, short floor** | Genuinely transient mid-convergence; the short cadence is the retry |
| Admission-control deferral | **Nak, short floor** | Ordinary pacing, not a fault — no mark, no issue |
| `TemplateDataError` — the gap's action template resolves null against this row | **Nak, long floor** | Fault is template × row: one fix path (a template edit) produces no new delivery, so the loop is the only automatic uptake. The other (a re-projection) supersedes the pending message and is taken up at once |
| `PlaybookConfigError` — an un-dispatchable action, a vanished candidate pin | **Nak, long floor** | Only a package edit fixes it, and that edit delivers nothing |
| `GapWithoutPlaybook` — the row opens a column the playbook names no entry for | **Ack, standing `warning`** | See below |
| Row-data errors (a non-bool gap column, an unreadable body, a missing `entityKey` echo) | **Ack, standing `warning`** | Every fix is a re-projection, which delivers on its own — a held pending slot buys nothing |
| Disabled target, unregistered target, spent gap budget | **Ack** | Nothing is owed by the message: a freeze ends by Resume, a registration by the registry, a budget by `resetBudget` |

**`GapWithoutPlaybook` Acks, and that is deliberate.** It looks like the config class — a playbook
edit would fix it, and that edit produces no new delivery — but this exit also has a population
whose fix *never arrives*: a package may project a `missing_*` column **on purpose** with no `gaps`
entry, ORing it into `violating` so the row stays violating without dispatching anything
(`packages/lease-signing`'s `missing_decision` / `missing_manager`, whose closure is a human decision
or nothing at all). A long Nak parks those rows for the whole human-latency window — forever, for
the ones nothing ever closes — each holding a `MaxAckPending` slot and re-running the entire
`clearClosedMarks` preamble every floor for a configuration that is already correct. So the row Acks
and the standing issue is the whole surface: the fault is visible per (target, gap), and a genuinely
missing entry is taken up by the next projection of the row. The sanctioned way to express an
intentionally undispatched column is a `surface` gap, which Acks with its own named issue per
subject.

The severities follow the same reasoning. `GapWithoutPlaybook` and `PlaybookConfigError` are
**`warning`s**, not errors: each self-heals on a package edit while Weaver goes on dispatching every
other target, which Contract #5 §5.2 separates from `unhealthy` ("cannot fulfil its primary
responsibility"). The heartbeat's listing cut compensates by rank rather than by severity — see
`docs/observability/health-kv-schema.md`'s `IssuesTruncated`.

One message carries the whole row, so a row whose gaps disagree takes the **shortest** retry any of
them asked for (`Nak > NakWithDelay > NakWithLongDelay > Ack`): a gap needing prompt re-evaluation
never waits out another gap's config floor, and a gap wanting only the long floor is re-evaluated by
the shorter redelivery anyway, idempotently. Both floors are operator-settable —
`WEAVER_REDELIVERY_DELAY` and `WEAVER_LONG_REDELIVERY_DELAY`.
