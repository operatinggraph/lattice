...
| `lint-gap-params-optional-hop` | A weaver target's `row.<column>` Params value is non-null on every row its gap opens — Weaver refuses a null templated column as a data error (`strategist.go` `resolveRowTemplate`), and a column walked off an OPTIONAL MATCH is null exactly where the hop bound nothing. Classified through the full engine's `OptionalHopColumns` / `ReturnColumnGuardsNonNull`: clean when the column reads no OPTIONAL-only variable, is `coalesce`d to a non-null literal, or the gap's own column carries a `<> null` / ordering conjunct that binds the hop |
| `lint-board` | The backlog is an index, not a journal — row / section / Done-log caps, no build-history narration, no conflict markers, a `📐 needs designer pass` row names its `no-pattern:`; and every component doc's "Review keeps catching" dossier (`docs/components/*.md`) holds at most the 12 entries it states for itself (an entry is a top-level `- **` bullet under that heading; a class is mechanized into a gate and struck, or same-root classes folded, never the cap raised — `_packages.md` reached 20 entries / 27 KB before this counted), a dossier past 20 KB warned to its owner; `--selftest` proves the cap on a fixture and replays the minting revision when history holds it |
...
| `lint-refusal-courtesy` | Every site that dispatches an op a package script can refuse with a STATE code — a vertical FE's `app.js` function naming the op, or Facet's descriptor form for any described op — declares, per code, the courtesy it gives (`hide` / `disable` / `drop` / `cap` / `prefill` / `confirm`) or an honest `none` / `unreachable` with its reason, once the op has two or more sites; a declaration naming a code the op cannot raise, or an op the function does not name, is stale. A refusal added to one form otherwise leaves its sibling form — or another client — a dead end (café payment cap 2026-09-05, wellness guest picker 2026-09-13, café `ItemUnavailable` reaching Facet through edge-manifest's picker 2026-09-15). Codes are derived per op from the describing package's scripts (`go.starlark.net/syntax`: the dispatch block's `fail(` literals plus every helper reachable through the call graph, a code carried in a helper parameter resolved from the literal at the call site); exempt are the codes whose courtesy is not per-form — `InvalidArgument`, `WrongClass`, `AuthDenied`, `PermissionDenied`, `NotFound`, `Unknown*`, `NotA*`; JS sites are read through goja (a string literal or member naming the op, the `KNOWN_CATALOG_OPS` array excluded), descriptors through go/ast; `--list` prints the census, `--selftest` the mutation battery |
...
| `lint-workplace-staff-vector` | Every op whose DDL script's `execute()` branch reaches `require_workplace`/`enforce_workplace`/clinic-domain's `enforce_workplace_confined` (directly, or through one level of same-script helper indirection — wellness's `prepare_booking_common`), AND whose own `pkgmgr.PermissionSpec` grants it at scope=any to a role other than `operator` (`nonOperatorAnyGrants`, the same structured grant table `scripts/verify-package-*.go` reads — an op with no such grant is unreachable by any non-operator actor by construction, since `actor_holds_operator` exempts the operator before the guard ever runs, and is printed `SKIP <pkg> <Op>: operator-only grant` rather than silently dropped; the evidence check runs first, so a genuinely-reachable op like maintenance-domain `ResolveWorkOrder` — operator-only in `PermissionSpec` but reachable through an orchestration-base ephemeral task grant — is never wrongly skipped), is proven, in its own package's `_test.go` files, by a non-operator staff actor's submission — `actor_holds_operator` returns BEFORE the worksAt walk, so a suite whose every vector for an op runs as the operator measures none of that op's confinement reads (café `Charge`, 2026-09-13; wellness `JoinWaitlist`, 2026-09-15, commit `7507be53`). Parses every shipped script with `go.starlark.net/syntax`, associating each `if ot == "<Op>"` (or `or`/`in`-chained) branch — found anywhere in `execute`, nested ifs/fors included — with whether it reaches a guard call. Evidence is a `_test.go` function resolving as a non-operator "staff cap-doc func" (its own `Roles:` field, or a `.PlatformPermissions = append(...)` mutation traced back to one that has such a field, does NOT name the operator role — `bootstrap.RoleOperatorKey`) granting `{OperationType: "<Op>", Scope: "any"}` — deliberately not scoped to `workplace_confinement_test.go` by filename, since real per-op vectors live under whatever name the author chose (clinic-domain's `correct_appointment_status_test.go`'s `casCapDoc`, maintenance-domain's `authcontext_target_forgery_test.go`'s inline `mdTechCapDoc()`-derived mutation) — or an explicit `// staff-vector: <Op> — <why>` declaration. Proves LISTING, not that a `Test*` function actually submits the op and asserts on the outcome (BOUNDARY, stated in the gate's own header). `--list` prints the governed census; `--selftest` replays both minting incidents from git history |
