...
| **Health KV signals** (Contract #5) | Health KV `health.processor.<instance>.*` | Heartbeat every 10s; per-op metrics (OpsConsumed / OpsCommitted / OpsDuplicates / OpsRejected / OpsMalformed); step-3 latency; step-5 script wall latency (with the per-execution live-read/listing means and the cumulative timeout count); auth trace records; claim-attempt outcomes; alerts under `health.alerts.security.*` |
