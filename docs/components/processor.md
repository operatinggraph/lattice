...
| 6 | **Validate** | `Validator.Validate` — checks `permittedCommands` (operation type must be in the DDL's list), `sensitiveAspectScope` (script may not create underscore-prefixed aspects except system-reserved ones), and key-pattern checks. `DDLViolation` → term, reply with `DDLViolation` code. The Contract #1 §1.5 `instanceOf` type-authority walk (`step6_resolve_ddl.go`, `maxInstanceOfHops` = 4, visited-set cycle guard) resolves each hop's target from the current atomic batch, the hydrated working set, or an on-demand read, ignoring tombstoned `instanceOf` links. |
