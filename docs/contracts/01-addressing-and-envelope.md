...
**Abstract vertex types.** A `meta.ddl.vertexType` meta-vertex whose root `data.abstract` is `true` declares an **abstract** vertex type: a type name that participates in the type taxonomy but has no instances. No key may use an abstract type name in any type segment, and no document may carry it as a `class`; the Processor rejects either at commit, **except on `tombstone`**, which is exempt from both gates. The exemption preserves the invariant rather than weakening it: a tombstone carries no document and can never make an instance **live**, so the set of *live* abstract-typed instances only ever shrinks through that path — and it is the corrective action that must stay available when a live concrete type is flipped to abstract while instances still exist. (Precisely: a tombstone of an *absent* key does materialize a KV entry at that key, already marked deleted. Such an entry names no live instance, every guard tests liveness, and a later non-tombstone mutation on it meets both gates unchanged.) An abstract type declares no `.script` and no `.permittedCommands`. Concrete types are joined to it by `lnk.meta.<concreteTypeMetaId>.subtypeOf.meta.<abstractTypeMetaId>`, whose transitive downward closure is the set of concrete types the abstract name covers.
