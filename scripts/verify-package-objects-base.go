...
// verify-package-objects-base.go — assertion tool for
// `make verify-package-objects-base`.
//
// Connects to a running Lattice NATS instance and checks that the objects-base
// package has been correctly installed. Asserts:
//
//	1 object DDL meta-vertex (vtx.meta.<NanoID>) with class=meta.ddl.vertexType
//	8 DDL aspects: .canonicalName=object, .permittedCommands (3 ops),
//	               .description, .script, .inputSchema, .outputSchema,
//	               .fieldDescription, .examples (each with a valid aspect envelope)
//	3 permission vertices (AttachObject, DetachObject, TombstoneObject), scope any
//	3 grantedBy links (each op → operator)
//	1 package vertex (vtx.package.<NanoID>) + 1 manifest aspect (name=objects-base)
//
// Run via: go run ./scripts/verify-package-objects-base.go
package main
...
const (
	objectsPackageName  = "objects-base"
	objectsDDLCanonical = "object"
	objectsCoreKVBucket = "core-kv"
)

var objectsExpectedOps = []string{"AttachObject", "DetachObject", "TombstoneObject"}
...
func main() {
	...
	coreKV, err := js.KeyValue(ctx, objectsCoreKVBucket)
	if err != nil {
		fmt.Fprintf(os.Stderr, "ERROR: cannot open Core KV bucket %q: %v\n", objectsCoreKVBucket, err)
		os.Exit(1)
	}
	...
	fmt.Printf("verify-package-objects-base: scanning %d Core KV keys...\n", len(allKeys))

	// 1. object DDL meta-vertex.
	objDDLKey, err := pkgverify.FindMetaByCanonical(ctx, coreKV, allKeys, objectsDDLCanonical)
	if err != nil || objDDLKey == "" {
		fail("object DDL meta-vertex", fmt.Sprintf("vtx.meta.*.canonicalName=%q not found: %v", objectsDDLCanonical, err))
	} else {
		ok(fmt.Sprintf("object DDL meta-vertex exists: %s", objDDLKey))
	}

	if objDDLKey != "" {
		// 2. class.
		if env, err := pkgverify.GetEnvelope(ctx, coreKV, objDDLKey); err != nil {
			fail(objDDLKey+" class", fmt.Sprintf("cannot read: %v", err))
		} else {
			if cls, _ := env["class"].(string); cls != "meta.ddl.vertexType" {
				fail(objDDLKey+" class", fmt.Sprintf("got %q want meta.ddl.vertexType", cls))
			} else {
				ok(objDDLKey + " class=meta.ddl.vertexType")
			}
			if isDeleted, _ := env["isDeleted"].(bool); isDeleted {
				fail(objDDLKey+" isDeleted", "vertex is tombstoned")
			} else {
				ok(objDDLKey + " isDeleted=false")
			}
		}

		// 3. canonicalName.
		cnKey := objDDLKey + ".canonicalName"
		if env, err := pkgverify.GetEnvelope(ctx, coreKV, cnKey); err != nil {
			...
			if val != objectsDDLCanonical {
				fail(cnKey, fmt.Sprintf("value=%q want %q", val, objectsDDLCanonical))
			} else {
				ok(cnKey + " value=object")
			}
			if err := pkgverify.CheckAspectEnvelope(env, cnKey, objDDLKey, "canonicalName"); err != nil {
		...
		// 4. permittedCommands.
		pcKey := objDDLKey + ".permittedCommands"
		if env, err := pkgverify.GetEnvelope(ctx, coreKV, pcKey); err != nil {
			...
			allPresent := true
			for _, op := range objectsExpectedOps {
				if !cmdSet[op] {
					fail(pcKey, fmt.Sprintf("missing command %q", op))
					allPresent = false
				}
			}
			if len(cmds) != len(objectsExpectedOps) {
				fail(pcKey, fmt.Sprintf("command count=%d want %d", len(cmds), len(objectsExpectedOps)))
				allPresent = false
			}
			if allPresent && len(cmds) == len(objectsExpectedOps) {
				ok(fmt.Sprintf("%s contains all %d commands", pcKey, len(objectsExpectedOps)))
			}
			if err := pkgverify.CheckAspectEnvelope(env, pcKey, objDDLKey, "permittedCommands"); err != nil {
		...
		for _, asp := range []string{"description", "script", "inputSchema", "outputSchema", "fieldDescription", "examples"} {
			...
			if env, err := pkgverify.GetEnvelope(ctx, coreKV, k); err != nil {
				...
				ok(k + " present")
				if err := pkgverify.CheckAspectEnvelope(env, k, objDDLKey, asp); err != nil {
	...
	for key := range allKeys {
		...
		for _, expected := range objectsExpectedOps {
			if opType == expected {
	...
	// 7. Package manifest.
	pkgKey, pkgManifestKey, err := pkgverify.FindPackageManifest(ctx, coreKV, allKeys, objectsPackageName)
	if err != nil || pkgKey == "" {
		fail("objects-base package manifest", fmt.Sprintf("vtx.package.*.manifest[name=%q] not found: %v", objectsPackageName, err))
	} else {
	...
	if pkgManifestKey != "" {
		if env, err := pkgverify.GetEnvelope(ctx, coreKV, pkgManifestKey); err != nil {
			...
			data, _ := env["data"].(map[string]any)
			if name, _ := data["name"].(string); name != objectsPackageName {
				fail(pkgManifestKey+" name", fmt.Sprintf("got %q want %q", name, objectsPackageName))
			} else {
				ok(pkgManifestKey + " name=objects-base")
			}
	...
	fmt.Println()
	if len(failures) == 0 {
		fmt.Printf("verify-package-objects-base: ALL ASSERTIONS PASSED (%d OK)\n", okCount)
		os.Exit(0)
	}
	fmt.Printf("verify-package-objects-base: %d FAILURE(S) (%d OK)\n\n", len(failures), okCount)
	for _, f := range failures {
		fmt.Printf("  - %s\n", f)
	}
	fmt.Printf("\nSuggestion: run `make down && make up && make verify-package-objects-base` to reinstall from clean state.\n")
	os.Exit(1)
}
