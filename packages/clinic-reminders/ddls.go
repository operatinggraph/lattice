...
// (followups.go), the appointment-change notice pair (changenotice.go), the
// notification-outcome replyOp pair for each (notifications.go),
// and the recurring visit-series vertex type + its aspect gates (visitseries.go).
...
func DDLs() []pkgmgr.DDLSpec {
	...
	ddls = append(ddls, changeNoticeDDLs()...)
...
def execute(state, op):
    ...
    if ot == "RecordAppointmentReminder":
        ...
        # far wider than the one engine that dispatches the appointmentReminders
        # directOp playbook. appointmentKey arrives off the payload and is
        # forwarded in the external.notification body the bridge turns into a
        # real message to the patient, so a wider submitter set is a forged
        # send: an arbitrary operator naming any appointment it likes and having
        # the platform notify that patient. First statement in the branch: it
        # also denies the payload-shape and vertex-alive oracles beneath it.
        if op.actor != primordialActor["weaver"]:
            fail("AuthDenied: RecordAppointmentReminder is restricted to Weaver's dispatch actor; got " + op.actor)
