...
func (f *lensFixture) seedManagedApplication(t *testing.T, appName, applicantName, unitName, landlordName string) {
	...
	f.aspect(t, unitName, "listing", "listing", map[string]any{"rentAmount": 4200, "rentCurrency": "USD", "status": "available", "availableFrom": "2026-08-01T00:00:00Z", "leaseTermMonths": 12})
	...
	f.aspect(t, appName, "lateFee", "leaseLateFee", map[string]any{"amountCents": 5000, "recordedAt": "2026-09-18T15:00:00Z"})
...
func TestLandlordLeaseApplicationsRead_ProjectsManagingLandlordAnchor(t *testing.T) {
	...
	require.Equal(t, "2026-08-01T00:00:00Z", v["unit_available_from"], "the listing's availableFrom is the leaseStart fallback the decide hint states")
	require.EqualValues(t, 12, v["unit_lease_term_months"], "the listing's term is the leaseTermMonths fallback the decide hint states")
	...
	require.EqualValues(t, 5000, v["late_fee_cents"], "late_fee_cents reads straight off app.lateFee.data.amountCents, the landlord's own recorded term")
