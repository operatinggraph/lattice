...
func (f *lensFixture) seedApplication(t *testing.T, appName, applicantName, unitName string) {
	...
	f.aspect(t, unitName, "listing", "listing", map[string]any{"rentAmount": 4200, "rentCurrency": "USD", "status": "available", "bedrooms": 2, "bathrooms": 1.5, "availableFrom": "2026-08-01T00:00:00Z", "leaseTermMonths": 12})
...
func TestLeaseApplicationsRead_ProjectsApplicantSelfAnchor(t *testing.T) {
	...
	require.EqualValues(t, 12, v["unit_lease_term_months"], "the listing's term is the fallback the approve signs on where .terms carries none")
