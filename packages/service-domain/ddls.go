...
func serviceDDL() pkgmgr.DDLSpec {
	return pkgmgr.DDLSpec{
		...
		Description: "Service domain DDL. Vertex shape: vtx.service.<NanoID>, root data = {} " +
			...
			"{backgroundCheck, payment, laundry, fitness, clinic} (edge-showcase-app-design.md §7.3 — widened honestly " +
		...
		InputSchema: `{"type":"object","properties":` +
			`{"family":{"type":"string","enum":["backgroundCheck","payment","laundry","fitness","clinic"],"description":"The service family <x> (backgroundCheck|payment|laundry|fitness|clinic). Sets the vertex envelope class service.<x>.template|instance."},` +
		...
		FieldDescription: map[string]string{
			"family":           "The service family <x>, one of {backgroundCheck, payment, laundry, fitness, clinic}. Determines the vertex envelope class (service.<x>.template for CreateServiceTemplate, service.<x>.instance for CreateServiceInstance). Required for the create ops.",
...
# a DDL per family. laundry/fitness/clinic (edge-showcase-app-design.md §7.3, §7.4) are the
...
SERVICE_FAMILIES = ["backgroundCheck", "payment", "laundry", "fitness", "clinic"]

def required_family(p):
    fam = required_string(p, "family")
    if fam not in SERVICE_FAMILIES:
        fail("InvalidArgument: family: must be one of backgroundCheck, payment, laundry, fitness, clinic; got " + fam)
    return fam
