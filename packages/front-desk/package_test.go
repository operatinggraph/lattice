...
// Definition in lockstep (the one-bill/cafe-domain precedent): the install
// reads the Definition, but the manifest is the human-facing declaration,
// and a drift between the two is a silent install hazard.
...
func TestPackage_StructurePins(t *testing.T) {
	if got, want := len(Package.DDLs), 0; got != want {
