...
//	                          sqft?, availableFrom, leaseTermMonths, depositAmount?, status}
...
var Package = pkgmgr.Definition{
	...
	Version:     "0.16.0",
	Description: "LoftSpace listing economics: the .listing + .address aspects on a location unit (SetListing / SetUnitAddress / SetListingStatus / FloorListingAvailability) + the landlord→unit management link (AssignUnitOwner / RemoveUnitOwner). Depends on location-domain; introduces no vertex type. Five projection lenses (availableListings, applicantRosterRead, landlordUnitsRead, landlordWorkOrdersRead, objectIdentityAttachmentsRead); applicantRosterRead is a PROTECTED Postgres identity roster (Contract #6 §6.14 RLS, D1.5) and a SECURE LENS (Contract #3 §3.10: the sensitive identity name decrypts at projection time into the RLS-protected table) — each row's authz_anchors carries the identity's own NanoID plus the managing landlord and covering buildings of any unit it has a live lease application against (mirroring cafe-domain's cafeIdentitiesRead and lease-signing's landlordLeaseApplicationsRead anchor shape), on top of the reserved WildcardAnchor grant; landlordUnitsRead is a PROTECTED Postgres model of every unit a landlord manages, independent of any lease application (portfolio-pulse occupancy), anchored on the managing landlord plus every covering building (mirroring landlordLeaseApplicationsRead) so a front-desk staffer's building grant resolves it too; landlordWorkOrdersRead is a PROTECTED Postgres model of the maintenance-domain work orders raised at a landlord's managed units — locatedAt a unit, manages to the landlord, an OPTIONAL scopedTo task fan for the open-task count — an OPTIONAL reportedBy/residesIn walk closing back on the order's own unit for reported_by_resident, same anchor shape as landlordUnitsRead, units only (a building-located order has no manages walk to anchor on); objectIdentityAttachmentsRead is a PROTECTED Postgres owner-anchored read model of objects-base's object/AttachObject vertices, self-scoped to an identity's own attached documents — the owner-anchored counterpart objects-base's own object-anchored objectAttachments lens cannot be without giving up its type-agnosticism.",
