...
// wellness-ledger — it already depends on wellness-domain (for bookingRef
// validation) and can read booking data directly, so no separate domain-side
// package or cross-package dependency is needed.
//
// No missing_account gap: wellness's existing billing (any front-desk-driven
// charge) already assumes a member's wellnessaccount exists via the standing
// CreateAccount flow; a no-show'd member with no account yet simply doesn't
// converge until one is opened, same as today's billing (mirrors clinic's
// identical rationale, targets.go's doc comment).
//
