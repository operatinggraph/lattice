...
// (LoftspaceCreateAccount), `transaction` (DebitAccount, CreditAccount), and the
// `ledgerAccountGuard` aspect-type declaration (the lease-anchored
// uniqueness guard LoftspaceCreateAccount writes).
...
func accountDDL() pkgmgr.DDLSpec {
	return pkgmgr.DDLSpec{
		...
		InputSchema: `{"type":"object","properties":` +
			`{"leaseAppKey":{"type":"string","description":"vtx.leaseapp.<NanoID> of the lease this account is for (LoftspaceCreateAccount; required, validated alive). The account gets its own independently-minted NanoID; uniqueness (one account per lease) is enforced via the leaseapp's .ledgerAccount guard aspect, not the account's own id."}},` +
		...
		FieldDescription: map[string]string{
			"leaseAppKey": "Full vtx.leaseapp.<NanoID> key of the lease the account is opened for. LoftspaceCreateAccount validates it is alive, mints the account under a fresh independent NanoID, writes the leaseapp's .ledgerAccount guard aspect (one account per lease) and the heldFor link (account→leaseapp).",
		},
