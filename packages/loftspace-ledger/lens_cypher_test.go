...
func lensKVs(t *testing.T) (adjKV, coreKV *substrate.KV) {
	...
	_, err = js.CreateKeyValue(ctx, jetstream.KeyValueConfig{Bucket: "adj-lsled-cypher-test"})
	require.NoError(t, err)
	_, err = js.CreateKeyValue(ctx, jetstream.KeyValueConfig{Bucket: "core-lsled-cypher-test"})
	require.NoError(t, err)
	adjKV, err = conn.OpenKV(ctx, "adj-lsled-cypher-test")
	require.NoError(t, err)
	coreKV, err = conn.OpenKV(ctx, "core-lsled-cypher-test")
	require.NoError(t, err)
	return adjKV, coreKV
}
