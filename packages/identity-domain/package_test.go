...
func TestPackage_DDLsAndOps(t *testing.T) {
	if got := len(Package.DDLs); got != 18 {
		t.Fatalf("expected 18 DDLs (identity + ssn, dob, name, email, phone, claimKey, linkKey, credentialBinding, idpBinding + "+
			"indexes, duplicateOf, boundTo + actorRevocation, gateway.actorRevoked, gateway.actorUnrevoked + unbindIdentityCredentials + "+
			"tombstoneOrphanedCredentialIndex), got %d", got)
	}
