...
//
//   - the 5 roles (consumer, frontOfHouse, backOfHouse, provider —
//     user-facing; identityProvisioner — system-only, granted to the
//     Gateway's own identity by a one-time ops action, never primordially)
//     — role vertex + canonicalName/description aspects + a canonical-name
//     index vertex, with deterministic NanoIDs;
...
var Package = pkgmgr.Definition{
	...
	Roles: []pkgmgr.RoleSpec{
		...
		{CanonicalName: "provider", Description: "An external service provider (practitioner, instructor, vendor) bound to a provider entity via an identifiedBy link."},
