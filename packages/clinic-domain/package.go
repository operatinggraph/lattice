...
//	                                                    .selfday<yyyymmdd><providerId> {} (patientSelfDayClaim — one open self-booked
//	                                                                    visit per provider per UTC day; the key is the lock)
//	vtx.provider.<id>     class=provider     root {}   .profile {fullName, specialty, credentials?, bio?}
//	vtx.appointment.<id>  class=appointment  root {}   .schedule      {startsAt, endsAt, remindAt, reason?, selfBooked?, movedAt?, movedBy?}
//	                                                    .status        {value ∈ scheduled|confirmed|checkedIn|completed|cancelled|noShow, note?, at?, by?}
//	                                                    .displacement  {displaced, checkedFor?, at, from?, to?, reason?} (does the provider's
//	                                                                    current .timeOff cover this visit? — written clear by the booking
//	                                                                    writers, evaluated per time-off write by EvaluateAppointmentDisplacement)
...
//	                   @at reminder re-arms, and stamps movedAt/movedBy) / SetAppointmentStatus (upsert .status{value, note?, at, by}) /
