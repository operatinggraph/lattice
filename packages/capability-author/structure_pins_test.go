...
func TestPackage_StructurePins(t *testing.T) {
	...
	for i, want := range []string{"capabilityAuthorPending", "capabilityProposals", "capabilityAuthorContext"} {
	...
	wantPerms := []struct{ op, scope string }{
		...
		{"RecordCapabilityProposal", "any"}, {"ReviewCapabilityProposal", "any"},
		{"MarkCapabilityProposalApplied", "any"},
	}
