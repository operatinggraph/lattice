...
// Rule-engine proof of consoleOperatorReadGrants, driven through the `full`
...
// carrying anchor_id '*' — a wildcard over the Protected plane. The package's
// other tests check only structure (the spec literal, the adapter kind, parse
// success), and neither of the two things that actually matter here is visible
// that way:
//
//   - the `role.canonicalName.data.value = 'consoleOperator'` filter is the
//     ONLY thing standing between a scoped console role and handing that
//     wildcard to every role-holder in the graph. A filter that silently
//     matched nothing fails safe (Loupe's reads go RLS-empty); a filter that
...
func (f *coFixture) key(name string) string {
	return "vtx." + f.types[f.ids[name]] + "." + f.ids[name]
}

func (f *coFixture) aspect(t *testing.T, ownerName, local, class string, data map[string]any) {
	t.Helper()
...
func (f *coFixture) edge(t *testing.T, name, fromName, toName string) {
	t.Helper()
	ctx := context.Background()
...
func (f *coFixture) role(t *testing.T, name, canonical string) {
	t.Helper()
	f.vtx(t, name, "role")
	f.aspect(t, name, "canonicalName", "canonicalName", map[string]any{"value": canonical})
}

func (f *coFixture) project(t *testing.T, spec string) []ruleengine.ProjectionResult {
	t.Helper()
	now := time.Now().UTC().Format(time.RFC3339)
	eng := full.New()
	cr, err := eng.Parse(spec)
	require.NoError(t, err, "consoleOperatorReadGrants must parse on the full engine")
...
// coGrantWorld seeds one consoleOperator holder alongside three identities that
// must NOT receive the wildcard: a demoOperator — this package's near-identical
// sibling, and the one role a copy-paste between the two specs would wrongly
// admit — a holder of an unrelated package role, and an identity holding no
// role at all.
func coGrantWorld(t *testing.T) *coFixture {
	f := newCoFixture(t)
	f.role(t, "consoleRole", "consoleOperator")
	f.role(t, "demoRole", "demoOperator")
	f.role(t, "otherRole", "frontOfHouse")

	f.vtx(t, "console", "identity")
	f.vtx(t, "demo", "identity")
	f.vtx(t, "frontDesk", "identity")
	f.vtx(t, "roleless", "identity")

	f.edge(t, "holdsRole", "console", "consoleRole")
	f.edge(t, "holdsRole", "demo", "demoRole")
	f.edge(t, "holdsRole", "frontDesk", "otherRole")
	return f
}

func TestConsoleOperatorReadGrants_GrantsOnlyTheConsoleRoleHolder(t *testing.T) {
	f := coGrantWorld(t)

	rows := f.project(t, consoleOperatorReadGrantsSpec)
	...
	require.Equal(t, "cap-read.consoleOperator", row["grant_source"],
		"the grant_source must stay disjoint from the kernel's cap-read.root so the two producers never overwrite each other")
}

func TestConsoleOperatorReadGrants_ProjectsNothingWithoutTheRole(t *testing.T) {
	f := newCoFixture(t)
	...
	require.Empty(t, rows,
		"absence of the consoleOperator role must mean absence of the grant — the read plane is default-deny. got %v", rows)
}

func TestConsoleOperatorReadGrants_ActivatesWithItsDefaultKeyColumns(t *testing.T) {
	eng := full.New()
	compiled, err := eng.Parse(consoleOperatorReadGrantsSpec)
	require.NoError(t, err)
