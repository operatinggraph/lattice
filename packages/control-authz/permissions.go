...
// Permissions returns the 23 ctrl.<component>.<verb> platform permissions,
...
// Grant matrix:
//
//	ctrl.weaver.{read,disable,enable,revoke,resetConfidence,resetBudget,replayTarget} → control-operator
	...
	for _, verb := range verbs {
		perms = append(perms, pkgmgr.PermissionSpec{
			...
			Note: "Authorizes control-operator, or any consumer / frontOfHouse / backOfHouse / provider identity acting on its own Personal Lens interest set (bound server-side, §3.4), to invoke the refractor control plane's " + verb + " op. " +
				"[no-op-meta: client-agent-op — a control-plane verb the client's sync manager invokes on its own behalf. No human dispatches it, so there is no form for a descriptor to render.]",
