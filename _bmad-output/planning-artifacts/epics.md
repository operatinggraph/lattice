...
NFR-S7: Permission revocations are effective within the p99 CDC-to-projection lag ceiling (< 500ms)
...
- **KV Bucket taxonomy (all buckets):**
  ...
  - Weaver Operational KV: `weaver.state.>` — Weaver-internal dispatch state
...
*(All mapped in FR Coverage Map above — none forgotten, correctly out of Phase 1 scope)*
...
- **Read-path authorization for Lens targets** — direct reads from Postgres-backed (or future ES/stream-backed) Lens targets bypass the Capability Lens write-path boundary (NFR-S2). Phase 1 MVP assumes trusted developer/operator readers only. Phase 2 must add row-level enforcement — Postgres RLS driven by actor identity, or a Gateway read-proxy pattern. Final approach decided at Phase 2 architecture sprint.
...
- **Refractor pipeline key-shape adaptation** *(surfaced by Story 2.2 gap analysis §2.5 / §3.13, Deviation 13 — Epic-3-blocking carry)* — `pipeline.parseCoreKVKey` still recognizes only legacy Materializer `node_<label>_<id>` keys; Contract-correct `vtx.<type>.<id>` and `lnk.*` keys are unrecognized and skipped. Pre-Epic-3 hardening prerequisite for Story 3.2's seeded-graph projection demonstrations. Refactor target: `substrate.ClassifyKey` + `substrate.ParseVertexKey`.
...
2. **Projection lag window exposure**: A test actor whose role was just revoked attempts an operation during CDC-to-projection lag. Processor step 3 reads stale Capability KV but the `projectedAt` timestamp is checked against the hard staleness ceiling (5× NFR-P3); for normal lag the stale entry is used (acceptable per NFR-P3); for excessive lag `AuthFreshnessExceeded` is returned. Test asserts NFR-S7 honored and stale-but-allowed operations are observable in the auth-trace (Story 3.5).
