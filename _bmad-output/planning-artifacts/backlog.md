...
| **BUG ★★★ → ✅ Resolved (NOT an engine bug) + ★★ operationName-null FIXED** | ✅ Done (Steward, 2026-06-25). The filed `assignedTo`-trigger bug did **not** reproduce — four lines of evidence proved the trigger is sound: (1) a new realistic-ordering e2e (`refractor_mytasks_assignedto_e2e_test.go` — identity written FIRST, then real Core-KV link envelopes drive CDC) **passes**; (2) the engine path is correct (`myTasks` is `actorAggregate` → `evalLinkFanOut` seeds both endpoints + builds adjacency; the *"no handler registered"* log is normal non-actor-consumer noise; sibling `capabilityEphemeral` uses the same `(identity)<-[:assignedTo]-(task)` pattern, shipping); (3) `cmd/refractor` wiring is byte-identical to the test; (4) **LIVE clean-stack run** projected BOTH dispatched userTasks for the pre-existing applicant within ~2s. Original symptom = the stale-accumulated dev-stack artifact (same as the cleared Refractor stale-stack flag). **Real defect found + fixed alongside (★★):** operationName projected null in the real flow because the lens hopped `op.canonicalName.data.value`, but a userTask's `forOperation` points at the op DDL meta whose name is `data.operationType` on the root (package ops carry NO canonicalName aspect; the old e2e *manufactured* one). Fix: lens projects `operationName ← op.data.operationType`; Loupe operator inbox (`cmd/loupe/tasks.go`) gets the matching root-operationType fallback; both guarded by tests. All gates green; lead-reviewed. | `implementation-artifacts/mytasks-assignedto-trigger-bug.md`, `packages/orchestration-base/lenses.go`, `cmd/loupe/tasks.go` |
...
| Applicant switcher — **identity roster lens** | 📋 Backlog (★, S) — replace the persisted identity-key field with a proper read-model of selectable applicant identities (a roster lens projection). The all-identity roster is noisy/over-broad; the scoped version is really a Personal-Lens/read-path-auth concern (deferred), so v1 stays a typed key. | `cmd/loftspace-app/web/` |
| lease-signing `leaseApplicationComplete` lens — **multi-bgcheck row collision** | 📋 Bug (★★, M — upgraded from S; needs an engine capability) — the dedicated freshness `OPTIONAL MATCH (applicant)<-[:providedTo]-(bg:service) WHERE completed+fresh bgcheck` (lenses.go:198–199) is NOT aggregated, so an applicant with >1 completed-fresh bgcheck (multiple applications on the same identity, or accumulated re-dispatches from the freshness re-open cycle) yields >1 row per anchor → `guardOutputKeyCollision` trips → the lens errors + goes yellow. The "at most one bgcheck providedTo the applicant" premise is false (providedTo is on the *identity*, not the application). **Refined diagnosis (Steward, 2026-06-25):** the clean fix — compute `freshUntil` as `max(validUntil)` within the existing aggregation `WITH` instead of a row-expanding second match — is BLOCKED: the full cypher engine supports only `collect`/`count` aggregators (executor.go `evalFunctionCall`/`finalizeAggregator`), no `max`/`min`. And `freshUntil` must be the LATEST validUntil (so Weaver's @at re-open timer doesn't fire while a later fresh bgcheck still counts), so a `collect`-then-pick-first is wrong without ordering. **So this needs a `max`/`min` aggregator added to the full engine first** (ISO-8601 strings compare lexicographically == chronologically, so a string-max reducer suffices) — a core read-path change warranting its own fire + the `make test-lease-convergence` heavy e2e (the freshness eager-reopen leg depends on `freshUntil`) + 3-layer review. Found 2026-06-25 via Loupe (test pollution exposed it; cleared by teardown, but the fragility is real). | `packages/lease-signing/lenses.go`, `internal/refractor/ruleengine/full/executor.go` |
