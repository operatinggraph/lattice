...
| Close assignTask tasks when their gap is satisfied | 📐 **Design proposed (Steward, 2026-06-25) — mechanism Winston-ratified, build-ready behind ONE §10.8 contract flag → Andrew.** **Verified live bug:** after `SignLease` wrote `.signature` (closing `missing_signature`), the §10.8-spawned `SignLease` task stayed `status:"open"` — nothing reconciles an assignTask task when the underlying fact lands via another path; the shipped LoftSpace Tasks tab (reads `my-tasks` `status='open'`) then shows a permanently-stale "Sign your lease" after signing. **Design:** extend Weaver `clearClosedMarks` — on a closing gap, read the §10.3 mark before deleting; if `Action=="assignTask"`, derive the task id from the mark's create-revision (`deriveEpisodeTaskID` — Weaver already knows the id) and dispatch an **idempotent `CompleteTask` directOp** (`reads=[taskKey]`, deterministic requestId for redelivery-collapse; benign `TaskAlreadyInState` treated as success), then delete. Reuses existing primitives (no new engine capability; the lens/scan alternatives rejected — known-key-reads-only DDL + no cross-bucket join). The ONE Andrew gate: §10.8's action table is gap-OPEN driven; this adds a gap-CLOSE-driven dispatch — a frozen-contract wording call. Build is its own fire (Weaver core-orchestration → 3-layer review + `make test-lease-convergence`). Ref: `implementation-artifacts/task-closure-on-gap-satisfaction-design.md`. | LoftSpace (all) | platform (orchestration) | ★★ | S–M |
