...
| **BUG ★★★ — `my-tasks` lens doesn't project on task assignment** | 📋 Filed (Steward, 2026-06-25; reproduced on a CLEAN stack) — the identity-anchored `my-tasks` lens reprojects on identity-anchor CDC (`holdsRole`/`applicationFor`/aspect writes) but **NOT** on the inbound `lnk.task.<id>.assignedTo.identity.<id>` mutation (Refractor logs *"link mutation observed but no handler registered"*, `pipeline.go:504`). In the real flow the identity exists long before a task is assigned, so a newly-assigned task **never projects** → the per-identity task inbox (LoftSpace Increment C, FR28 role-queue, future clinic inbox) is empty despite live open assigned tasks (confirmed: 2 open tasks assignedTo the applicant, 0 lens rows). The e2e (`refractor_mytasks_e2e_test.go`) **masks it** by writing the identity vertex *after* the `assignedTo` edge. Core read-path engine → needs design → **3-layer review** → fix (register inbound `assignedTo` as a reprojection trigger + materialize the inbound edge in adjacency + realistic e2e). **Increment C code is correct** (reader unit-tested, both completion ops verified through the real Processor); this is the upstream data source. | `implementation-artifacts/mytasks-assignedto-trigger-bug.md` |
