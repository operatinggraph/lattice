...
| Conventions-linter — edit-time hook *(agentic-ops)* | ✅ The 24 pre-existing `// Story N …` history-comments are swept and `STRICT=1 go run ./scripts/lint-conventions.go` is wired as a CI gate (`.github/workflows/ci.yml`). **Remaining:** a changed-file `PostToolUse` hook (stdin-JSON wrapper) so violations are also caught at edit-time, before CI. | ★ | XS |
| Version-control the agentic-ops role-skills *(agentic-ops)* | `.claude/` is gitignored, so role-skills (`lamplighter`, and the planned `steward`/owner skills) live there un-tracked — not reproducible/shareable. **Rec:** keep canonical defs in a tracked repo dir (e.g. `agents/`) + a `make install-skills` step that links them into `.claude/skills/`; leave the bmad tooling skills local. | ★★ | S |
