...
| lease-signing `leaseApplicationComplete` lens — **multi-bgcheck row collision** | 📋 Bug (★★, S) — the dedicated freshness `OPTIONAL MATCH (applicant)<-[:providedTo]-(bg:service) WHERE completed+fresh bgcheck` is NOT aggregated, so an applicant with >1 completed-fresh bgcheck (multiple applications, or accumulated re-dispatches) yields >1 row per anchor → `guardOutputKeyCollision` trips → the lens errors + goes yellow (`errorCount` climbs). The `lenses.go` "at most one bgcheck providedTo the applicant" premise is false. Fix-forward: collapse that match to one row (`max(validUntil)` / `collect`). Found 2026-06-25 via Loupe (test pollution exposed it; cleared by teardown, but the fragility is real). | `packages/lease-signing/lenses.go` |
