...
**Reliability candidate — Refractor not projecting fresh CDC in a long-lived dev stack (Steward, 2026-06-25, seen while live-verifying Increment C).** On the running `up-full`+`install-loftspace` stack, a freshly-created lease application (`CreateLeaseApplication`, links committed) never projected into the `leaseApplicationComplete` lens, and subsequent committed ops (`RecordIdentityPII`, `SignLease` — aspects landed in Core KV) produced **no** `refractor.log` activity after startup; the `my-tasks` bucket stayed empty despite 18 live open+assigned task vertices (confirmed via Loupe). Restarting the refractor (adjacency bootstrap re-ran) did **not** reproject the existing anchors. Lenses report `active`, the write path + Weaver/Loom dispatch work — but the Refractor's CDC consumer appears to not advance / not reproject in this accumulated stack, so the Weaver-target read models go stale (would also degrade My Applications + the system map). **Unclear if a code bug or an artifact of this particular restarted/long-lived dev deployment** — needs a clean-stack repro (a Lamplighter/owner pass: fresh `make up-full`, create one application, assert it projects within N s; if it fails clean, it's a Refractor read-path reliability bug). Not blocking Increment C (its reader + completion paths are verified headlessly + through the real Processor).
