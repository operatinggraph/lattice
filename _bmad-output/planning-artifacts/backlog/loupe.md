...
- 2026-07-18 — F16 UX design drafted (Sally), grounded live against the shipped read-models/op DDLs. Key finding: **both** loops are human-gated (Augur `augurDispatchPending` fires on `review.state="approved"`, not `pending`) — Augur is an action tab, not observe-only; its approve is *lighter* than capability's (server-side re-validation, no apply step). Awaiting Winston adjudication; 4 open questions for Andrew in the doc §8 (cap approve/apply forks).
- **Next:** F13 L2-full/L3 stay blocked on Chronicler archive mode (unscheduled, lattice). F16 awaits adjudication (Winston, Andrew-delegated) — F16.1 + F16.3 are zero-cross-lane-dep buildable. The `designAhead`-trio maintenance row is 📋 ready (smallest, most concrete).
