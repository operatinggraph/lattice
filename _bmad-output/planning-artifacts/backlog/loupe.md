...
| **F4 — Health absorption + status vocabulary** | Global alert strip (verbatim `health.alerts.*` incl. stub-auth-active), gates panel + rail (preserves `#sysmap-console` slot), `renderedState` incl. `pending-readpath` (the "7 degraded" fix); retires Health tab. | ★★★ | M | 🎯 📋 ready · [design §14](../../implementation-artifacts/loupe-2-ux-design.md) |
...
- **Next:** Loupe Steward builds F4 (health absorption + status vocabulary).
