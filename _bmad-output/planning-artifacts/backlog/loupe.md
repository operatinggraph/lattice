...
| **F11 — Gateway security console** | `#/component/gateway` page (auth metrics + JWKS key set) + the token-revoke surface (arch-review gap). | ★★ | M | 🚧 blocked-on: revocation build (lattice, design ratified) + Gateway up-full/jwks (lattice) · [UX §2](../../implementation-artifacts/loupe-platform-edges-ux.md) |
...
- **Next:** F11 once its lattice enablers land (revocation build + Gateway up-full/jwks); F12/F13 stay gated on Vault/Chronicler. On the Gateway up-full ship, flip its `designAhead` flag off (XS).
