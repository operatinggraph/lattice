...
|---|---|---|---|---|---|---|
| **The café can't forgive a debt it can never collect** | The only clearing verb is `CreditCafeAccount`, whose `.entry` has no reason ([scripts.go:1240](../../../packages/cafe-ledger/scripts.go:1240)) — a write-off is booked as cash collected; wellness/clinic-ledger's staff-only `reason: waiver` is the precedent. Live: 3 of 5 overdue debtors are unapproved seed leases ($18, 23–39 days) still drawing reminders. | Café | pkg + FE | ★★ | S | 📋 ready · `waiver` reason, refused self-scoped; desk Write-off on the arrears row |
| **A refund of a paid charge is handed back in cash AND spent again as credit** | `RefundCafeCharge` only posts a credit ([scripts.go:1593](../../../packages/cafe-ledger/scripts.go:1593)); nothing records money leaving, `CreditCafeAccount` refuses on a credit, and the desk grid drops credit leases ([ledger.go:157](../../../cmd/cafe-app/ledger.go:157)) so the house owes invisibly. Live: Riley Chen −$35.75 after two refunds, prepaying every later tab. | Café | pkg + FE | ★★ | S | 📋 ready · a staff-only payout debit (or `settledAs: cash` on the refund); desk shows credit |
...
| **A reversal is aged against the wrong charge** | `deriveStatement` FIFOs every credit ([ledger.go:211](../../../cmd/wellness-app/ledger.go:211)), so a refund/waiver that `reverses` a named debit pays off the OLDEST one instead; the lens drops the `reverses` walk. Live: Alex Kim's real 18-day-old $20 class charge reads "due Sep 18" because a Sep 3 refund of a different class paid it. Same shape in café/clinic/loftspace. | Wellness | pkg + FE | ★★ | S | 📋 ready · project the reversed key on the credit row; net reversals by target, FIFO only payments |
