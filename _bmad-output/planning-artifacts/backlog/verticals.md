...
| **An approved lease never moves the tenant in** | `DecideLeaseApplication` records `.tenancy` and `EndTenancy` ends it, but neither wires nor unwires `residesIn` — the left edge of `cap.svc.<actor>` and every whoami anchor; only the seed and the operator write it. Live: Jordan Ellis and Priya Raman have none. A convergence target wires at approval, unwires at `endedAt`. | LoftSpace | pkg | ★★ | M | 🏗️ building · [design](../../../docs/reviews/loftspace-residence-spine-2026-09-16.md) · next: Inc 1 tenancyEnd unwire gap |
