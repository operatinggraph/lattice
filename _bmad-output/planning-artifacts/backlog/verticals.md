...
| **A just-minted applicant with no application is on no staffer's roster** | `applicantRosterRead` anchors an identity on its own key + the landlord/buildings of units it has a live lease application against ([lenses.go:250](../../../packages/loftspace-domain/lenses.go:250)), so a claim secret lost before the first application can be re-issued by nobody. Live: Jordan Ellis + Priya Raman hold applications and are reachable. | LoftSpace | pkg | ★ | S | 📐 needs designer pass · no-pattern: an identity that records the workplace it was minted at (third sighting: the café tab + wellness charge rows) |
