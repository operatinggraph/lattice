...
| **The front desk sees a 12-day-overdue café balance it cannot take a payment against** | `/api/frontdesk-balances` lists two debtors ($3.50 OVERDUE 12d, $4.50) whose lease is `landlordApproved:false`, so [app.js:685](../../../cmd/cafe-app/web/app.js:685) disables them in the Resident tab's picker — the only home of Record Payment. Live: a staff `CreditCafeAccount` on that account cleared every server-side check, failing only on a deliberately-zero amount. | Café | FE | ★★ | XS | 📋 ready · one shared `fillLeaseSelect` disables for a reason that only governs OpenTab |
| **$14.50 of café debt is invisible to every staffer on the platform** | 11 of 57 leases project `coveringLocations: []` — their `appliesToUnit` unit was tombstoned by the 2026-08-23 listing reap while the link stayed live — and `visibleLeases` fails closed, so the arrears list silently shows $73.25 of the real $87.75 and `/api/ledger` 403s. The resident still sees it and can self-pay. | Café | pkg + FE | ★★ | S | 📋 ready · mirror `menuCatalog`'s `missingLocation` onto `cafeLeaseWorkplaces` + an unattributable arrears bucket · overlaps the LoftSpace reaped-unit row |
...
- **Rotation to date:** LoftSpace ×29, Clinic ×29, Café ×20, Wellness ×16.
