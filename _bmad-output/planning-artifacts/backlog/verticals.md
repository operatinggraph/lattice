...
| **A staffer clears their own café debt** | The staff leg of `CreditCafeAccount{waiver}`, `RefundCafeCharge`, `PayoutCafeCredit` checks `worksAt` only; the arrears grid draws *Write off* on the staffer's own row. Refuse a clearing verb on the actor's own account (clinic/wellness share the shape). Live: Sam Okafor wrote off 1¢ of their own balance. | Café | pkg + FE | ★★ | S | 🏗️ building · [design](../../../docs/reviews/cafe-self-clearing-2026-09-18.md) · next: SelfClearing in three ledgers + FE |
