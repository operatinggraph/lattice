...
| **A checked-in patient is swept to no-show** | The desk records arrival (`checkedIn`); if the provider never closes the visit, the past-due sweep marks it `noShow` at endsAt and the record says the patient never came ([lenses.go:28](../../../packages/clinic-reminders/lenses.go:28), [ddls.go:3866](../../../packages/clinic-domain/ddls.go:3866)). Recorded arrival is not a documentation lapse the patient caused. | Clinic | pkg | ★★ | S | 🏗️ building · [design](../../implementation-artifacts/clinic-visit-record-integrity-design.md) · next: Inc 1 |
| **A visit that never happened can be documented** | `RecordEncounter` reads neither status nor clock — a cancelled, no-show or future visit takes a clinical note and a follow-up that arms reminders; only the FE hides the button ([ddls.go:4047](../../../packages/clinic-domain/ddls.go:4047)). Live: `p9rDJNfJgRDRZYUnePst` documented 09-03 for a 09-15 visit. | Clinic | pkg | ★★ | S | 🏗️ building · [design](../../implementation-artifacts/clinic-visit-record-integrity-design.md) · next: Inc 1 |
| **A follow-up is cleared by any later visit** | The worklist hides a follow-up once the patient has any later visit — any provider, any reason ([app.js:3473](../../../cmd/clinic-app/web/app.js:3473)) — while the reminder fires whether or not one is booked ([followups.go:293](../../../packages/clinic-reminders/followups.go:293)). Live: Riley's Sports-Medicine follow-up (due 09-29) is hidden by a badge-check visit with Dr. Classic Demo. | Clinic | FE + pkg | ★★ | S | 🏗️ building · [design](../../implementation-artifacts/clinic-visit-record-integrity-design.md) · next: Inc 1 |
| **The clinic desk can't see a debtor, and a charge can't name its visit** | The roster carries no balance and the ledger is a flat list — no per-charge open/overdue state, a waiver names no charge, a copay names no visit (wellness `frontdesk-arrears` + café `reversesKey` aging are the precedent). `appointmentRef` means "no-show fee": a desk copay attached to a completed visit opens `missing_reversal` and is credited back ([lenses.go:153](../../../packages/clinic-ledger/lenses.go:153)). | Clinic | pkg + FE | ★★ | M | 📋 ready |
...
- **Rotation to date:** LoftSpace ×33, Clinic ×33, Café ×23, Wellness ×20.
