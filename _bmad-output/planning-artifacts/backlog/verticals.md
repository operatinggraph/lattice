...
| **`AttachObject`/`DetachObject` stay hand-built with a named fix path unbuilt** | The attach pair needs an upload-ceremony affordance + owner-anchored read surface (`signInMethods`-pane precedent). (`CreateLocation`'s `ClassChoices` half shipped `aa04a7a5` — Done log.) | Cross-vertical | pkg + FE | ★ | S | 🏗️ building · Inc-A shipped ([design §22](../../implementation-artifacts/staff-descriptor-rendering-design.md)) · next: Inc-B owner-anchored lens for DetachObject's Dispatch |
