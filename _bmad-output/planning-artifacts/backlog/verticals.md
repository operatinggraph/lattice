...
|---|---|---|---|---|---|---|
| **Facet picker gives one op-agnostic courtesy** | `entityRefCandidates` drops rows on a single `available` column, so `CreateBooking` cannot drop a full session without hiding it from `JoinWaitlist`, which needs it; the sibling-form class cannot reach Facet for any per-op state. | Cross-vertical | Sally + FE Engineer + pkg | ★ | M | 📐 needs designer pass · no-pattern: per-op entity-ref candidate filter in the descriptor vocabulary (FORK-1) · [census](../../../docs/reviews/verticals-refusal-courtesy-gate-2026-09-15.md) |
