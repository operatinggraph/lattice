...
| **Every resident's class + visit badge on the café front desk is blank** | The composition surface the vertical exists to show renders nothing: `front-desk-bookings`/`front-desk-visits` hold 36 and 63 retracted rows and zero live ones. 11 of 12 `residentRate` bookings are `noShow` and the 12th is tombstoned; every `residentVisit` appointment was swept to `noShow`. Nothing keeps a forward-dated booking or visit alive. | Café | pkg | ★★ | S | 📋 ready · [seed-classic-demo.go](../../../scripts/seed-classic-demo.go) backfill idiom |
