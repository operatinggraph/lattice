...
| **A settled tab bills a self-order nobody made** | `Settle` (resident, desk, the 24 h sweep) posts lines carrying `orderedAt` and no `servedAt`; the queue drops a non-open tab and `MarkLineServed`/`VoidCharge` refuse `TabNotOpen` after, so the kitchen loses the ticket and the resident pays. Refuse or void unserved lines at settle. Live: 4 tabs, 5 lines, $17.50. | Café | pkg + FE | ★★ | M | 🏗️ building · [design](../../../docs/reviews/cafe-settle-unserved-lines-2026-09-18.md) · next: Inc 1 package refusal + sweep void |
| **A staffer clears their own café debt** | The staff leg of `CreditCafeAccount{waiver}`, `RefundCafeCharge`, `PayoutCafeCredit` and `Settle{paidCents}` checks `worksAt` only ([scripts.go:1202](../../../packages/cafe-ledger/scripts.go:1202)); the arrears grid draws *Write off* on the staffer's own row. Refuse a clearing verb on the actor's own lease's account (the wellness/clinic ledgers share the shape). Live: Sam Okafor (front of house, 32 d overdue) wrote off 1¢ of their own balance. | Café | pkg + FE | ★★ | S | 📋 ready |
| **The house limit resets on settle** | `house_tab_limit` bounds one tab's `totalCents` ([ddls.go:1612](../../../packages/cafe-domain/ddls.go:1612)); a resident settles (no payment changes hands) and reopens in two taps, and the credit hold arms only after a reminded 30-day due date. Bound the open balance + tab, or a per-day tally. Live: settle 20:35:54 → new tab 20:44. | Café | pkg | ★★ | S | 📋 ready · [tab-limit design §7](../../../docs/reviews/cafe-house-tab-limit-2026-09-16.md) descoped a balance limit to the hold's lane — re-decide |
| **The POS picks a resident from 58 leases in NanoID order** | `computeLeases` sorts by lease key ([leases.go:46](../../../cmd/cafe-app/leases.go:46)) and `fillLeaseSelect` renders one flat `<select>` ([app.js:873](../../../cmd/cafe-app/web/app.js:873)): 49 disabled seed rows interleaved with 6 usable, no open-tabs-first, no type-to-find. Live: the one open tab sat 19th. | Café | FE | ★★ | S | 📋 ready |
| **The café statement stamps every entry in raw UTC** | `postedAt` renders verbatim ([app.js:2515](../../../cmd/cafe-app/web/app.js:2515)) beside a localized *Due 9/30/2026* and *Opened Sep 17, 2026, 1:40 PM*; the `localDateTime` formatter covers the cards only. | Café | FE | ★ | XS | 📋 ready |
| **A moved class strands its members' hour on the old time** | `ReassignSession` migrates the studio's and instructor's slot cells and leaves the bookers' ([ddls.go:4698](../../../packages/wellness-domain/ddls.go:4698)): the old hour stays `BookerConflict` for every seated member with no booking to show for it, the new hour is unguarded, and cancel/release tombstone cells on the current span only. Live: Riley Chen refused on Sep 19 18:00 (Notice Probe, moved 18→19, called off). | Wellness | pkg | ★★ | S | 📋 ready |
| **A class handed to a sub, un-led, or moved to another room tells nobody** | `missing_move_notice` keys on `startsAt` alone ([lenses.go:277](../../../packages/wellness-reminders/lenses.go:277)); `/api/bookings` carries no instructor, so My Classes cannot even show the change. Live: PO Probe Swap lost Sam Okafor, Riley's row read `missing_move_notice: false`. | Wellness | pkg + FE | ★★ | S | 📋 ready |
...
- **Rotation to date:** LoftSpace ×35, Clinic ×35, Café ×26, Wellness ×23.
