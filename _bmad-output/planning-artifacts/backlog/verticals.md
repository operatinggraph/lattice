...
| **A signed lease with no rent offer is never billed rent** | 4 of 8 signed leases have no ledger account and no rent clause — live: 20 Riverside Walk, rent $1,750, signed 2026-07-31, balance $0. `leaseRentSettlementSpec` gates `missing_account` on `requestedRent` present; apply form leaves it optional, unit's listed rent was never the fallback. | LoftSpace | pkg | ★★★ | S–M | 🏗️ repair op shipped (lease-signing 0.31.15) · next: run the backfill script (sandbox-blocked this fire, operator capability-kv gap) |
