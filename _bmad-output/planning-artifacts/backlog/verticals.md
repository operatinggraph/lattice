...
| **A patient with 38 no-shows books like anyone else** | The booking prompt reads arrears only ([app.js:3935](../../../cmd/clinic-app/web/app.js:3935)); no read model counts a patient's recent no-shows (`count(CASE …)` precedent: [cafe-domain lenses.go:450](../../../packages/cafe-domain/lenses.go:450)). A `noShowCount` over the last 90 days on the roster row and in the booking prompt. Live: Riley Chen 38 in 90 d, 17 booked ahead. | Clinic | pkg + FE | ★ | S | 🏗️ building · [design](../../../docs/reviews/clinic-no-show-history-2026-09-17.md) · next: Inc 1 lens |
| **A settled tab bills a self-order nobody made** | `Settle` (resident, desk, the 24 h sweep) posts lines carrying `orderedAt` and no `servedAt`; the desk's queue drops a non-open tab ([app.js:1408](../../../cmd/cafe-app/web/app.js:1408)) and `MarkLineServed`/`VoidCharge` refuse `TabNotOpen` after, so the kitchen loses the ticket and the resident pays; the only remedy is a refund. Refuse or void unserved lines at settle. Live: 4 settled tabs, 5 unserved lines, $17.50. | Café | pkg + FE | ★★ | M | 📋 ready |
...
- **Rotation to date:** LoftSpace ×35, Clinic ×35, Café ×26, Wellness ×22.
