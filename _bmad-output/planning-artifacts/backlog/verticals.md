...
| **A wellness class's ledger account never opens** | No `vtx.wellnessaccount` has ever been opened for a booker, so both wellness-ledger settlement targets still charge nobody. | Wellness | pkg | ★★ | S | 🚧 blocked-on: CreateAccount browser-call permission gap (below) |
| **CreateAccount can't be called from any of the 4 ledger apps' browsers** | `CreateAccount` grants `["operator"]` only in every ledger package (e.g. `packages/wellness-ledger/permissions.go:16`) — front-desk staff hold no such role (`scripts/seed-showcase.go:620-627`), yet `clinic-app`/`loftspace-app` FE already call it as staff (`cmd/clinic-app/web/app.js:2710`) — 403s for any account a seed script didn't pre-open. Needs a confinement design (no workplace dimension to scope by, unlike CreateSession's frontOfHouse grant). | Cross-vertical | pkg | ★★ | S | 📋 ready |
| **One instructor can teach two classes at once** | The slot lock is claimed on the studio hub only (`studioSlotClaim`), so a live probe scheduled one instructor into two overlapping 18:00–19:00 classes at two studios and booked the same member into both; clinic-domain's `providerSlotClaim` + `patientSlotClaim` are the exact mirror to add on the instructor + booker hubs. | Wellness | pkg | ★★ | M | 📋 ready |
| **The studio can't onboard its own instructors** | `CreateInstructor` / `BindInstructorIdentity` / `SetInstructorProfile` / `TombstoneInstructor` reach no UI, so the one live instructor exists only because `scripts/seed-showcase.go` minted him and hiring a teacher takes a code change; clinic-app already ships the mirror (`CreateProvider` + `BindProviderIdentity`, `web/app.js:1214`). The three lifecycle ops are operator-only by design (`permissions.go`), which the surface has to respect. | Wellness | Sally + FE Engineer | ★★ | M | 📋 ready |
