...
| **A reversal is aged against the wrong charge** | `deriveStatement` FIFOs every credit ([ledger.go:211](../../../cmd/wellness-app/ledger.go:211)), so a refund/waiver that `reverses` a named debit pays off the OLDEST one instead; the lens drops the `reverses` walk. Live: Alex Kim's real 18-day-old $20 class charge reads "due Sep 18" because a Sep 3 refund of a different class paid it. Same shape in café/clinic/loftspace. | Wellness | pkg + FE | ★★ | S | 🏗️ building · [design](../../../docs/reviews/ledger-reversal-aging-2026-09-13.md) · next: inc 1 |
