...
| **Front desk has no way to see which booked appointments a provider's time off now conflicts with** | `SetProviderTimeOff` writes only `.timeOff` — nothing tells front desk which of a provider's existing bookings now need a reschedule call. | Clinic | pkg | ★ | S | 🏗️ building · derived in the FE from the two projections it already holds (providers `timeOff` × staff appointments), the shape `hasLaterVisit` shipped · next: badge + Follow-ups worklist group + editor preview, goja-pinned predicate |
...
| **A term-wide time move of a recurring class still takes one `ReassignSession` per occurrence** | `TombstoneSessionSeries` calls a run off in one act, but moving every remaining occurrence (a new weekday/time) is N submits, each re-claiming its own slot cells; `ReassignSession`'s claims are not derivable from a series key. | Wellness | pkg + FE | ★ | S–M | 🏗️ building · `ReassignSessionSeries` walks `partOf`, shifts every still-upcoming occurrence at the confirmed studio by one delta, releases and claims the cell sets as a batch · next: op + pins + roster form |
