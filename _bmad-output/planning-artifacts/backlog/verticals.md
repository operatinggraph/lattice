...
| **Every losing applicant on a leased unit is still dispatched RecordIdentityPII + SignLease** | The four applicant gaps carry no `unitStatus = 'leased'` conjunct ([lenses.go:745](../../../packages/lease-signing/lenses.go)) — only `missing_listingLeased` does, the in-file precedent — so once one application leases the unit Weaver keeps asking every rival for their SSN and a signature on a unit they cannot get. Live: 13 of 14 applications on one unit hold both tasks open, none declined, no terminal not-selected disposition. | LoftSpace | pkg | ★★★ | S–M | 📋 ready |
...
| **The applicant storefront is empty and cannot refill** | `seed-classic-demo` lists exactly one of 11 units, so the first successful lease drops `/api/listings` to 0 rows; 5 units carry no `.listing` aspect at all and nothing re-lists. A browsing applicant sees the reference vertical with nothing to apply for. | LoftSpace | pkg | ★ | XS–S | 📋 ready |
...
| **Weaver's dispatch loop redispatches 6 gap/action pairs across two targets without ever observing a close** | `LensEffectMismatch` (standing since 2026-08-21) on `clinicSiteBackfill` + `leaseApplicationComplete`'s missing_site/bgcheck/onboarding/payment/signature columns — per `weaver-planner-mandate-design.md` §3.4 points at a stale/wrong guard, a wrong lens column, or a no-op remediation; two also `GapBudgetExhausted`. May share a root with the row above. | Clinic + LoftSpace | pkg | ★★ | S–M | 🚧 blocked-on: [lattice.md](lattice.md) `[Weaver] inflight_<g>` |
...
- **Rotation to date:** LoftSpace ×24, Clinic ×22, Café ×13, Wellness ×10.
