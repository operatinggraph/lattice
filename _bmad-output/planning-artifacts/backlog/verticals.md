...
| **A relisted unit revives every losing rival** | `lost_to_rival` is a live derivation (unit leased ∧ no decision): when the winner's tenancy ends and `missing_relist` flips the unit to available, every rival's gaps re-open, the card reverts to "In review" and Weaver re-dispatches SignLease/RecordIdentityPII (52 on 12 Classic Demo Ave). A recorded terminal loss is a new `.decision` value with a product fork (revive vs close). | LoftSpace | pkg | ★★ | M | 📐 needs designer pass · no-pattern: recorded terminal loss on a leaseapp |
