...
| **A tenant with an assigned signing task is refused at the signature** | `SignRenewal` fails closed without `.applicationSignals` ([renewal_scripts.go:415](../../../packages/lease-signing/renewal_scripts.go:415)); the planner reads the missing profile as `hasGuarantor=false` and assigns the sign task ([renewal_targets.go:110](../../../packages/lease-signing/renewal_targets.go:110)). 5 of 6 live tenancies have no profile. | LoftSpace | pkg + FE | ★★★ | M | 📋 ready · a `submitProfile` assignTask leg + `signalsPresent` column; the inbox self-voices `SetApplicantProfile` |
