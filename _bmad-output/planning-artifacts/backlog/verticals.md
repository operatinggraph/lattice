...
| **A visit the desk cancels or moves tells the patient nothing** | A staff `SetAppointmentStatus(cancelled)` and a `RescheduleAppointment` write `.status`/`.schedule` in silence ([ddls.go:3923](../../../packages/clinic-domain/ddls.go:3923), [:3699](../../../packages/clinic-domain/ddls.go:3699)); only the reminders ever go out. Wellness's mirror shipped `f139d128`. Live: 23 cancelled, none noticed. | Clinic | pkg | ★★ | M | 🏗️ building · [design](../../../docs/reviews/clinic-status-clock-and-change-notices-2026-09-17.md) · next: Inc 2 notices lens + op |
| **An appointment's status has no time** | `.status` is `{value, note?, fee?}` ([ddls.go:4039](../../../packages/clinic-domain/ddls.go:4039)); no transition stamps when, so the desk cannot see how long a checked-in patient has waited and a cancel has no recorded moment to audit against the 24 h window. `at = op.submittedAt` on every transition (wellness's `bookedAt` idiom), projected; "checked in 12 min ago" on the card. | Clinic | pkg + FE | ★★ | S | 🏗️ building · [design](../../../docs/reviews/clinic-status-clock-and-change-notices-2026-09-17.md) · next: Inc 1 the stamps |
