...
| **Every front-desk POS write fails about half the time, mid-order** | A barista gets `ScriptTimeout: script exceeded wall budget 250ms`, no retry. The staff path walks confinement live inside the wall ([ddls.go:765](../../../packages/cafe-domain/ddls.go:765)); the resident's identical op is `authTargetValidated` and skips it — paced 1/s, same host: staff Charge 9/20 vs resident 2/20. `b997ff2a` already removed the clinic-shaped redundancy here. | Café | pkg (design) | ★★ | S–M | 📋 designer · pkg-vs-platform fork · detection: [lattice.md](lattice.md) wall row |
| **The house tab is a running balance that never becomes a bill** | Nothing in cafe-domain, cafe-ledger or one-bill carries a due date, statement period or overdue state — the only `pastDue` in Café is the 24h stale-TAB sweep, which closes a tab, not a balance. Live: $47.50 stands on one account, oldest unpaid debit 2026-08-01, with no surface saying it is late and nothing ever asking for it; the resident can pay, capped at the balance, but is never prompted. | Café | pkg + FE | ★★ | S | 📋 ready |
...
- **Rotation to date:** LoftSpace ×27, Clinic ×26, Café ×17, Wellness ×13.
