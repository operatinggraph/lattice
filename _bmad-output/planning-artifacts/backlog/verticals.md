...
| **The clinic desk can't see a debtor, and a charge can't name its visit** | The roster carries no balance and the ledger is a flat list — no per-charge open/overdue state, a waiver names no charge, a copay names no visit (wellness `frontdesk-arrears` + café `reversesKey` aging are the precedent). `appointmentRef` means "no-show fee": a desk copay on a completed visit opens `missing_reversal` and is credited back. | Clinic | pkg + FE | ★★ | M | 🏗️ building · [design](../../implementation-artifacts/clinic-ledger-debtor-and-charge-context-design.md) · next: Inc 1 |
