...
| **No lease takes a security deposit** | No listing carries a deposit and no approval bills one; every live lease is rent lines only. `depositAmount` on the listing, a `missing_deposit` gap minting a `oneTime` clause at approval, held apart from rent on both statements (return rides move-out). | LoftSpace | pkg + FE | ★★ | M | 🏗️ building · [design](../../../docs/reviews/loftspace-security-deposit-2026-09-15.md) · next: Inc 1 |
...
| **A clinic debtor is never reminded** | Overdue is derived per read off the app clock ([ledger.go:204](../../../cmd/clinic-app/ledger.go:204)) — no `.arrears` on the account, no reminder, no episode; the patient learns only by opening Billing. Live: Riley Chen $25.00, 22 days overdue. Precedent: [wellness](../../../docs/reviews/wellness-arrears-reminder-2026-09-15.md) · [loftspace](../../../docs/reviews/loftspace-rent-arrears-2026-09-15.md); a clinic flags at check-in rather than refusing care — the design decides the hold. | Clinic | pkg + FE | ★★ | M | 📋 ready |
