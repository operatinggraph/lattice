...
| **Retire studio: offered to the desk that cannot, unguarded for the operator who can** | Retire renders for every staff hat but `TombstoneStudio` grants `operator` only ([permissions.go:114](../../../packages/wellness-domain/permissions.go:114)) — opaque toast; no upcoming-classes refusal in the op ([ddls.go:1985](../../../packages/wellness-domain/ddls.go:1985)), so a retired studio keeps booked, charged classes. Refuse `HasUpcomingClasses`, grant frontOfHouse like CreateStudio. | Wellness | pkg + FE | ★★ | S | 🏗️ building · next: op + grant + FE |
