...
| **A tenant with an assigned signing task is refused at the signature** | `SignRenewal` fails closed without `.applicationSignals` ([renewal_scripts.go:415](../../../packages/lease-signing/renewal_scripts.go:415)); the planner reads the missing profile as `hasGuarantor=false` and assigns the sign task. 6 of 7 live tenancies have no profile. | LoftSpace | pkg + FE | ★★★ | M | 🏗️ building · [build note](../../implementation-artifacts/loftspace-lease-renewal-goal-authored-target-design.md) · next: package leg, then the inbox profile form |
