...
| **A tenant cannot give notice and a lease cannot end early** | `EndTenancy` refuses `NotYetEnded` ahead of `leaseEnd` and nothing records a move-out date, so a tenant who leaves mid-term is billed by the rent clause to `validUntil` and the unit stays `leased`. A recorded notice / early end (tenant self, landlord `manages`) shortens the clause's term, ends the tenancy at the recorded date, relists. | LoftSpace | pkg + FE | ★★ | L | 🏗️ building · [design](../../../docs/reviews/loftspace-tenancy-notice-2026-09-15.md) · next: Inc 1 `ShortenClauseTerm` |
