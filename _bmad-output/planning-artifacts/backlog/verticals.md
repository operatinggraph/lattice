...
| **Book Appointment leaks a prior identity's raw patient NanoID** | `clinic.patient` (`app.js:12`) is one global localStorage key, never cleared on logout, read before whoami resolves — switching signed-in identity leaks the prior identity's raw patient NanoID into Book Appointment's Patient field (`shortKey()` fallback, `app.js:302`; live-reproduced). Never render a bare vtx key; scope+clear the selection per identity, revalidate against the new identity's own roster. | Clinic | FE | ★★★ | S | 📋 ready |
| **Patient self-booking write path still 403s** | `clinic-patient-self-service-booking-design.md`'s Checkpoint (capability-mode JWT / `authContext.target` wiring) was ready but never filed. Live-confirmed still 403s: patient-self `CreateAppointment` → `AuthContextMismatch: platform scope=self requires authContext.target`. Also yields the missing `isPatientSelf()` needed to lock "Select patient" to a fixed self-display. | Clinic | FE | ★★★ | M | 📋 ready · [Checkpoint](../../implementation-artifacts/clinic-patient-self-service-booking-design.md) |
| **clinic-domain patient-self-anchor bridge: stale docs + a test that can't catch a regression** | `lenses.go:531-537` + `patients.go:94-105` doc comments still claim "no per-patient self-anchor exists," contradicted by the identifiedBy read-grant bridge already shipped (`lenses.go:225-232`). `staff_patients_rls_test.go` (78-81, 136-144) hand-seeds the old empty-`authz_anchors` shape instead of deriving from the real `clinicPatientsReadSpec` cypher — would not catch a regression in the shipped self-anchor path. | Clinic | pkg | ★★ | S | 📋 ready |
| **Self-order picker offers items `Charge` will always refuse** | `Charge`'s `location_covers` guard accepts only items `servedAt` the tab's building; the offer side doesn't match — `menuCatalog` carries no location column ([lenses.go:48](../../../packages/cafe-domain/lenses.go)) and `/api/menu` filters nothing, by design ([menu.go:53](../../../cmd/cafe-app/menu.go)). Live: both catalog items → `AuthDenied: … not served at tab's building`. Project the item's `servedAt` place, or mirror `edgeEntityMenuItems`' residence-chain walk. | Café | pkg + FE | ★★★ | M | 📋 ready |
...
| **Every vertical app's "Signed in as" prints a raw NanoID, not a name** | 3 of 4 apps never attempt resolution: `clinic-app` (`app.js:219-220`), `cafe-app` (`app.js:256-258`), `wellness-app` (`app.js:253-257`) all render `shortKey(identityId)` straight into the header, live-confirmed in each. `loftspace-app` is the outlier and the pattern to mirror — `renderSignedInAs`/`nameFor` (`app.js:796-816`) resolves via its own RLS-scoped roster, falling back to the raw key only on a miss (that residual gap should close too). | Cross-vertical | FE | ★★★ | M | 📋 ready |
...
- **Rotation to date:** LoftSpace ×16, Clinic ×15, Café ×6, Wellness ×2.
