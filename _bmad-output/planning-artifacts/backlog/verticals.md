...
| **Staff self-guards treat a tombstone as present** | The shipped café/clinic `scope=self` ownership probes test `kv.Read(link) == None`, but a tombstone hydrates as a DOCUMENT — so a soft-deleted `applicationFor`/`identifiedBy` link reads as present. Latent only because nothing tombstones those links today; F4's guard uses the correct `== None or .isDeleted` form. | Cross-vertical | pkg | ★★ | XS | 📋 ready · consumer: any future op that tombstones an ownership link |
