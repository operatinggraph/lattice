...
| **[maintenance] `ResolveWorkOrder` reads `.resolution` before its auth guard** | The terminal/idempotent branch reads and branches on the work order's own `.resolution` aspect before the §3.4.1 resource bind + workplace confinement run. Neither path writes, so it is a read oracle rather than a write bypass: a caller outside the building learns resolved-vs-open, and `.resolution.notes` via the error text. Fix mirrors clinic `SetAppointmentStatus` — bind identity before terminal branching. | Cross-vertical | pkg | ★ | S | 📋 ready |
| **[wellness] `BindInstructorIdentity` grant comment contradicts its own spec** | `permissions.go`'s doc comment says the op grants `frontOfHouse`; the spec below grants `operator` alone. Metadata drift that reads as a wider grant than exists. | Wellness | pkg | ★ | XS | 📋 ready |
