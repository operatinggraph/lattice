...
|---|---|---|---|---|---|---|
| **A recurring visit is credited on the clock, never on a booking** | `AdvanceVisitSeries` rolls at `nextDueAt` whether or not a visit was booked ([visitseries.go:909](../../../packages/clinic-reminders/visitseries.go:909)); Book *handles* the occurrence but nothing records it, so "Due now" empties at Weaver latency. Live: Riley's series at occurrence 2, no visit at the 08-15 grid point. | Clinic | pkg + FE | ★★★ | M | 🏗️ building · [design §9](../../implementation-artifacts/clinic-recurring-visit-series-design.md) · next: Inc 1 visit-gated lens + re-anchoring advance |
