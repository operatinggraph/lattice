...
| **A reported issue never becomes work; a tenant cannot report one; their landlord never sees it** | `ReportIssue` mints no task — the one app-reported order has waited 49 days; the op is staff-only; the landlord console lists no orders. A `missing_task` gap queues `ResolveWorkOrder` to `backOfHouse`; a `consumer` self grant behind the residence probe + Report an issue on the tenant card; a landlord lens of orders at units they `manages`. | LoftSpace | pkg + FE | ★★ | M | 🏗️ building · [design](../../../docs/reviews/loftspace-maintenance-loop-2026-09-17.md) · next: Inc 1 |
