...
| **Persona worlds — Provider archetype + sign-in-first verticals** | Fourth human archetype: `provider` role + `identifiedBy` binding from each vertical's provider entity to a real login; all four FEs become sign-in-first skins over the discovered capability set — pickers + any-subject mints deleted, per-actor submits, grants audited. | Cross-vertical | Sally + FE Engineer + pkg | ★★★ | XL | 🏗️ building · [design](../../implementation-artifacts/persona-worlds-design.md) · W1 Inc 2a front-desk grants SHIPPED · next: Inc 2b hat-gating + provider hat |
| **Clinic front-desk follow-up-series grant** | `StartVisitSeries` (Follow-ups tab) is `operator`-only in the **clinic-reminders** package (separate version + confinement helpers from clinic-domain); a real `frontOfHouse` session gets `AuthDenied`. Add the `frontOfHouse` grant + workplace confinement (mirror clinic-domain's `CreateAppointment`), consumer: front-desk Follow-ups tab. | Clinic | pkg | ★★ | S | 📋 ready · [design §7.1](../../implementation-artifacts/persona-worlds-design.md) |
| **Vertical Package Standard — canonicalize the reference corpus** | Write THE standard a vertical package builds to, knowing everything 2026-07 knows: persona-complete roles/bindings/grants + producers on both read paths, op-metas for every user-facing op, read-posture-clean scripts, tiered read boundary, verify script + structure pins. W1–W4 briefs cite it; per-package fires decide converge-vs-rewrite against it. | Cross-vertical | pkg | ★★★ | L | 📐 awaiting-Andrew · [standard](../../implementation-artifacts/vertical-package-standard.md) |
...
| **CreateBooking has no double-book / past-time guard** | `wellness-domain` `CreateBooking` (`ddls.go:1002-1082`) claims a free seat but never checks the booker already holds one on that session (unlike clinic's `PatientDoubleBook` slot-claim / café's `OpenTabAlreadyExists`), nor that `session.schedule.startsAt` is still future (unlike clinic's `ScheduleInPast`). Live-confirmed via a duplicate booking on an already-ended class (IDs in the brief). | Wellness | pkg | ★★ | S | 🏗️ building · [brief](../../implementation-artifacts/wellness-vertical-design.md) |
