...
| **The front desk composes nothing for either actual resident** | The grid card joins three badges — 🧘 booked class, 🏠 rent/term, 🩺 upcoming visit — and all three are impossible for the two showcase residents: their leases carry no `.tenancy`, the precondition clinic/wellness need before writing the `residentVisit`/`residentRate` links both front-desk lenses anchor on (0 of each live), and their units get no `SetUnitAddress`/`SetListing` (only applicant units 3–4 do). | Café | pkg | ★★★ | M | 📋 ready |
| **The one-bill statement has no reader** | `one-bill`'s two lenses project rent + café transactions into the shared `one-bill-history` bucket ([design §Inc 3](../../implementation-artifacts/cafe-ledger-design.md)), and no application reads that bucket — cafe-app's Resident "Café ledger" still reads `cafeLedgerHistory` alone (`ledger.go:143`), loftspace-app its own. The Inc-3 payoff (café charges on the same lease statement as rent) reaches no user. | Café | pkg + FE | ★★ | S–M | 📋 ready |
...
- **Rotation to date:** LoftSpace ×18, Clinic ×17, Café ×8, Wellness ×4.
