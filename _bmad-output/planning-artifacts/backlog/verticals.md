...
| **The executed lease still doesn't name its tenant** | The applicant's name is a sensitive aspect on a linked identity, out of reach of the subject-rooted egress template, so the document renders the identity key. Direction (Andrew, 2026-09-01): snapshot the party names onto the application as a point-in-time sensitive aspect (the `underwritingParties` idiom), one entry per applicant identity (roommates, cosigners), egressed through the shipped path. | LoftSpace | pkg | ★★ | S | 📋 ready · [why](../../implementation-artifacts/declared-path-reads-design.md) (held; banner) |
...
| **The café's stale-tab auto-settle timer never arms** | `cafeStaleTabSettlement` computes `freshUntil` (`cafe-domain/lenses.go:366`) but its target's `BodyColumns` (`:69`) omit it, so Weaver never schedules the `@at` and a stale tab settles only when an incidental write re-projects its row. Add the column to the target body and pin that the timer arms on a stale tab. | Café | pkg | ★★ | XS | 📋 ready · found by the expiry design's census ([why](../../implementation-artifacts/expiry-as-a-recorded-fact-design.md) §7) |
