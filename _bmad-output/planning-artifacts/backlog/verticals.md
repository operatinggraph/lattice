...
| **Clinic projects patient/provider `fullName` into open KV** | Three open nats-kv lenses (`lenses.go:365,405,424`) carry person names any localhost P5 reader can dump unauthenticated — the leak class D1.5's Protected roster was built to close; wellness's keys-only posture (`lenses.go:90-93`) is the rule (Standard S3/S8). Fix: open lenses project bare keys; names stay Protected/Vault. | Clinic | pkg | ★★★ | S | 📋 ready |
