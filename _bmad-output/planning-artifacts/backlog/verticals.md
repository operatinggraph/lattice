...
| **App read boundaries skip the revocation kill-switch** | All four apps' readauth postures build `NewAuthenticator(verifier, nil)` — a revoked credential keeps *reading* until token expiry (writes die at the Gateway immediately). Apps already open the bindings bucket per request; open `token-revocation` beside it and wire the checker. | all | FE | ★★ | XS–S | 📋 ready · context: [authN design §12.1](../../implementation-artifacts/external-actor-authn-binding-design.md) |
