...
| **A new op's first Weaver dispatch races its permission's projection** | An installed target dispatches within a second of the package commit, before the capability lens has projected the new op's grant, so every first attempt is `AuthDenied` and each row waits a full 30-min mark lease before the sweep retries (café arrears, 2026-09-06: 8 of 9 rows). Weaver cannot tell a not-yet-projected grant from a missing one. | Cross-vertical | platform | ★ | S | 📐 needs designer pass · no-pattern: a projection-aware first dispatch (install ordering or a decline class for a grant projected after the deny) · [Inc 4](../../implementation-artifacts/cafe-ledger-design.md) |
...
| **A just-minted applicant with no application is on no staffer's roster** | `applicantRosterRead` anchors an identity on its own key + the landlord/buildings of units it has a live lease application against ([lenses.go:250](../../../packages/loftspace-domain/lenses.go:250)), so a claim secret lost before the first application can be re-issued by nobody. Live: both unclaimed tenants are reachable. | LoftSpace | pkg | ★ | S | 📐 needs designer pass · no-pattern: an identity that records the workplace it was minted at (third sighting: the café tab + wellness charge rows) |
...
- 2026-09-06 · `b5c8e7a8` + `bdfb344d` · A resident who owes the café is told so — `cafeArrearsReminders` off a recorded `.arrears` due fact the ledger ops maintain, `EvaluateCafeArrears` recomputes the FIFO head and sends one reminder per episode through the bridge; live: 5 overdue accounts sent, 2 timers armed, grid + statement show it.
