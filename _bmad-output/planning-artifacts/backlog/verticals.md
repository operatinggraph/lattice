...
| **One applicant with N lease applications gets N identical onboarding tasks** | `missing_onboarding` is a per-application gap dispatching per-applicant work ([targets.go:97](../../../packages/lease-signing/targets.go:97)), so an applicant holding 4 applications gets 4 open `RecordIdentityPII` cards; `inflight_onboarding` reads the projected row and cannot serialise a same-wave fan-out. | LoftSpace | pkg | ★★ | S–M | 🏗️ building · [design](../../implementation-artifacts/duplicate-human-task-fanout-design.md) §7 + build note · next: Inc 1 re-anchor in worktree |
