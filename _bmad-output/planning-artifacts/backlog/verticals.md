...
| **A losing applicant is never told the unit went to someone else** | A rival parks terminal-not-violating (decision null, unit leased) and the card reads "In review — complete the open steps below" ([app.js:1832](../../../cmd/loftspace-app/web/app.js:1832)); the inbox keeps their expired `SignLease`/`RecordIdentityPII` tasks completable ([app.js:2331](../../../cmd/loftspace-app/web/app.js:2331)), which the op refuses. Live: 52 on 12 Classic Demo Ave. A `lostToRival` column + banner; expired tasks read-only. | LoftSpace | pkg + FE | ★★ | S | 🏗️ building · next: `lost_to_rival` on both application read lenses + banner + expired tasks read-only |
| **A rent charge names no billing period or due date** | The debit entry is `{type, amountCents, postedAt}` ([scripts.go:578](../../../packages/loftspace-ledger/scripts.go:578)); the statement reads "rent $2,125 posted 09-13" with no month covered and no due date, though the reversal memo already names the period. Live: Jordan's 09-13 debit is Sep 6–Oct 6. `DebitAccount` carries `periodStart/periodEnd` (+ a due date) from the clause; the ledger and one-bill render them. | LoftSpace | pkg + FE | ★★ | S | 🏗️ building · next: `.entry` gains periodStart/periodEnd/dueAt from the clause grid; ledgerHistory + oneBillRentEntries project them |
