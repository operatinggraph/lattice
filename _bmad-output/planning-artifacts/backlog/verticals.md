...
| **A rogue-claimed patient login has no operator undo** | `UnbindPatientIdentity` and `RotateClaimKey` both require the identity still `unclaimed`, so a chart bound to a login claimed by someone other than the patient stays bound. Repair: an operator unbind of a claimed identity + credential unlink + re-issue to the patient, provenance-checked (minter, binder, claiming credential). | Clinic | pkg | ★★ | S | 📋 ready · [doctrine](../../../docs/components/_packages.md) |
