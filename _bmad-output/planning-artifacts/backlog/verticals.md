...
| **`seed-showcase.go` books Osei outside his own hours** | `seedRileyClinicWorld`'s Osei appointment (`futureDayAt(1, 14)`) rotates across weekdays with the reseed date, and live it hit `FATAL CreateAppointment: OutsideHours` on a Thursday-UTC run — Osei carries no `SetProviderHours` call in this file, so either stale `.hours` state lingers on the shared dev stack from an earlier probe, or some other path constrains him; root cause not yet traced. Confined to Clinic's seed script, unrelated to the café menu fix in this same fire. | Clinic | pkg | ★ | XS | 📋 ready |
