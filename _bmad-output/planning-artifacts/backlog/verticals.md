...
| **A reversal is aged like a payment** | The ledger names no `reverses` relation ([scripts.go:437](../../../packages/loftspace-ledger/scripts.go:437), README "not yet needed"), so `arrears_head` retires the OLDEST charge with a correction credit. Live: Jordan's 09-05 charge reversed 09-13 leaves his 08-06 rent unpaid, yet the statement and the landlord's Rent-owed read 13 days overdue, not 43. clinic-ledger's `reverses` walk is the precedent. | LoftSpace | pkg | ★★ | M | 📋 ready · [arrears design §2](../../../docs/reviews/verticals-arrears-resumable-replay-2026-09-16.md) |
| **A late fee is a memo on a manual charge** | `LoftspaceRecordCharge{memo: "Late fee"}` is the whole mechanism ([permissions.go:156](../../../packages/loftspace-ledger/permissions.go:156)); the clause `purpose` token admits `lateFee` and nothing reads it ([scripts.go:87](../../../packages/semantic-contracts/scripts.go:87)). Live: Riley 48 days overdue, reminded once, charged $0. A `purpose=lateFee` clause billed once per arrears episode. | LoftSpace | pkg + FE | ★★ | M | 📋 ready · [policy precedent](../../../docs/reviews/cafe-house-tab-limit-2026-09-16.md) |
