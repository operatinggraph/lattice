...
| **The wellness console can't name anyone it serves** | `wellnessIdentitiesRead` is self-anchored only, so a `worksAt` staffer reads exactly one row — their own (live: `count:1`). The instructor's attendance sheet renders `vtx.identity.MQsm…` and the book-a-member picker offers 6 members as `MQsm… — lease Lh1r…`; wellness is the last vertical whose staff console is unnamed. Mirror café's workplace fan-out (`92439169`) / clinic's (`a9c1e7c0`). | Wellness | pkg + FE | ★★★ | S | 📋 ready |
