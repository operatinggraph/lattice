...
| **A class's instructor and time are frozen at creation** | No op reassigns `ledBy` or moves a session — `CreateSession` binds both once. 11 of 17 live sessions carry no instructor, so `SetBookingAttendance`'s ledBy+identifiedBy guard leaves them permanently unmarkable and no instructor can call them off. The only recourse for a sub or a moved class is `TombstoneSession`, which releases every booking — covering a teacher cancels the class on its members. | Wellness | pkg | ★★ | M | 🏗️ building · [doc](../../implementation-artifacts/wellness-vertical-design.md) · next: FE UI |
...
| **The seeded clinic never completes a visit** | All 13 seeded appointments sit at `scheduled` (8 already past) and Document-visit only appears once one is `completed`, so the post-visit half is dark on arrival: 0 encounters, an empty Follow-ups worklist, and `clinic-ledger`'s `noShowSettlement` target never converged (0 ledger transactions). `seed-showcase.go` also assigns both providers to Riverside but never `SetSiteProfile`s it, so `clinicSites` is empty — Sites and the Book site filter offer no real clinic. | Clinic | pkg | ★★★ | M | 📋 ready |
