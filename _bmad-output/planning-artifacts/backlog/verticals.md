...
| **46 auto-marked no-shows are permanently uncorrectable — nobody can say "the patient was here"** | Every terminal→terminal move is refused ([ddls.go:3010](../../../packages/clinic-domain/ddls.go:3010)) and a terminal card renders zero buttons ([app.js:4173](../../../cmd/clinic-app/web/app.js:4173)), so the sweep's unattended verdict is final on 46 of 101 live appointments; the desk's correction fails `TerminalStatus`. | Clinic | pkg | ★★★ | S–M | 📋 ready · add the explicit re-open/correction op |
| **The follow-up worklist clears itself on visits the patient never attended** | `hasLaterVisit` ([app.js:2713](../../../cmd/clinic-app/web/app.js:2713)) treats any later non-cancelled appointment as "booked" and the default filter hides the card: live, 7 of 7 follow-ups are cleared by a `noShow` and the desk reads "0 of 7". None carries a `followUpDate`, so `followUpReminders` cannot fire either. | Clinic | FE + pkg | ★★★ | S | 📋 ready · a no-show must not clear; require `followUpDate` |
| **A provider's time off cancels nothing already booked, and the sweep then blames the patient** | Live: time off over 2026-09-17 left all 4 booked appointments `scheduled` and notified nobody, while a new booking there was rejected `ProviderUnavailable`; `pastDueAppointments` ([pastdue.go](../../../packages/clinic-reminders/pastdue.go)) has no availability term, so each becomes an auto no-show. | Clinic | pkg | ★★ | S | 📋 ready · surface conflicts at time-off; exempt blacked-out slots |
| **The 4 stale cluster-A onboarding cards need a live CancelTask pass** | Inc 2 of the onboarding re-anchor: 4 open `RecordIdentityPII` tasks predate the Inc 1 fix and won't self-clear. Grounded 2026-09-01: the scoping identity is a stranded pre-rotation fossil (`lattice bootstrap verify`'s own STRANDED OPERATOR ROLE notice), not live admin — safe to cancel. | LoftSpace | platform | ★ | XS | 🚧 blocked-on: live-op execution (this session's write guard declined it) · 4 ready-to-paste `lattice op submit CancelTask` calls in the [design](../../implementation-artifacts/duplicate-human-task-fanout-design.md) build note |
