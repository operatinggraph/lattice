...
| **The class schedule's Book button is mostly dead** | `scheduleCard` ([app.js:501](../../../cmd/wellness-app/web/app.js)) gates only on `full`, so it offers Book on classes `CreateBooking` always refuses — live, 11 of 13 listed classes had already started (`SessionInPast`), and one the member already holds a seat in is refused `DoubleBooked` with its button enabled. Mirror the app's own staff path: `renderBookMember` (app.js:731) hides the control for `started \|\| full` for this exact reason; the booked set comes from `/api/bookings`. | Wellness | FE | ★★★ | S | 📋 ready |
