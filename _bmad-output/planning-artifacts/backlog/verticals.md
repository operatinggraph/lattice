...
| **Wellness attendance ops** | An instructor can read their own class roster and call the class off, but cannot mark who actually showed — no `Attend`/`NoShow` op exists in wellness-domain. §7.3's instructor hat names attendance alongside own-roster and cancel-own-session, which W3 Inc 1 shipped. New op + guard + grant, mirroring the ledBy+identifiedBy confinement `TombstoneSession` already uses. Consumer: the instructor Roster tab (shipped, `d3a7cc7b`). | Wellness | pkg | ★★ | M | 📋 ready |
