...
| **A guest's debt outlives the booking that made them visible** | Lease-less coverage comes from `wellnessBookers` (live bookings, no status filter) and `CancelBooking` tombstones the booking where its no-show twin keeps the vertex under a terminal status, so a forfeit stands on someone no desk can reach. Census 2026-09-05: 0 of 4 debtors in this shape. | Wellness | pkg + FE | ★ | S+S | 📋 ready · forfeiting late cancel keeps the vertex as `status=forfeited` (no seat); six consumer sites named · [triage §3](../../../docs/reviews/verticals-designer-triage-2026-09-10.md) |
| **The front desk is sent to collect from two residents it cannot name** | A seed reap tombstoned 10 leases' units; `TombstoneLocation` neither cascades nor refuses and no op re-points `appliesToUnit`, so `cafeIdentitiesRead`'s workplace fan-out ([lenses.go:429](../../../packages/cafe-domain/lenses.go:429)) binds nothing: a staffer reads 46 of 118 names. | Café | pkg | ★★ | S | 📋 ready · `ReassignLeaseUnit` (lease-signing, operator-only; the `SetMenuItemLocation` repair shape) + seed backfill · [triage §2](../../../docs/reviews/verticals-designer-triage-2026-09-10.md) |
...
| **A new op's first Weaver dispatch races its permission's projection** | A target installed with its op dispatches before the capability lens projects the grant: every first attempt is `AuthDenied`, then a 30-min mark lease before the sweep retries (café arrears: 8 of 9 rows). | Cross-vertical | platform | ★ | S | 📐 needs designer pass · no-pattern: a projection-aware first dispatch (install ordering, or a decline class for a grant projected after the deny) · [Inc 4](../../implementation-artifacts/cafe-ledger-design.md) |
| **The desk mints the login it then connects to an existing chart** | `BindPatientIdentity` confines its target to an `unclaimed` identity, but the front desk minted it and holds its claim secret, so a staffer could claim the login afterwards and read the patient's rows via `patientIdentityReadGrants`. Registration extends that trust to an empty chart; this reaches an existing one. | Clinic | pkg | ★★ | S | 📐 awaiting-Andrew · trust-model call on the `frontOfHouse` grant ([permissions.go](../../../packages/clinic-domain/permissions.go)) |
