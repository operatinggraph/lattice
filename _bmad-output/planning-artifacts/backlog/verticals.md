...
|---|---|---|---|---|---|---|
| **A full class offers Join waitlist, not Book, in Facet** | `edgeEntitySessions` projects no seat state, so Facet offers `CreateBooking` on a full session (→ `SessionFull`) and `JoinWaitlist` on an open one; the per-op predicate exists (`Dispatch.VisibleWhen`), the column does not. | Cross-vertical | pkg + FE Engineer | ★ | M | 🏗️ building · [triage](../../../docs/reviews/verticals-designer-triage-2026-09-15.md) · next: Inc 1 lens column |
