...
| **Approval signs a lease on terms the applicant never reviewed** | `.tenancy` comes from the listing's `availableFrom`+`leaseTermMonths` ([scripts.go:878](../../../packages/lease-signing/scripts.go:878)), never the application's `.terms`; the review-before-signing panel ([app.js:1960](../../../cmd/loftspace-app/web/app.js:1960)) states neither. Live: a 09-15 ask bills from 08-23. | LoftSpace | pkg + FE | ★★★ | M | 🏗️ building · [design](../../implementation-artifacts/loftspace-lease-term-and-tenancy-end-design.md) · next: Inc 1 Decide derives from `.terms` |
| **A lease that ends without renewal never frees the unit** | The only listing flip is `leased` on approval ([targets.go:124](../../../packages/lease-signing/targets.go:124)); past `leaseEnd` with no signed renewal the unit stays `leased`, no Relist ([app.js:4261](../../../cmd/loftspace-app/web/app.js:4261)), both cards silent. Relist at the recorded lapse + an ended state. | LoftSpace | pkg + FE | ★★★ | M | 🏗️ building · [design](../../implementation-artifacts/loftspace-lease-term-and-tenancy-end-design.md) · next: Inc 2 `tenancyEnd` target + `EndTenancy` |
...
| **A renewed tenant's home still shows the old lease** | No tenant-facing lens projects `.tenancy`; the card shows application-time terms and the Renewed card's "term ends" is `cycleEnd` — the OLD end ([app.js:2874](../../../cmd/loftspace-app/web/app.js:2874)). Live: Jordan Ellis reads $2,050 · ends 2026-09-06 while billed $2,125 through 2027-09-06. | LoftSpace | pkg + FE | ★★ | S | 🏗️ building · [design](../../implementation-artifacts/loftspace-lease-term-and-tenancy-end-design.md) · next: Inc 3 the card states the recorded lease |
