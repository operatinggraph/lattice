...
| **My Classes never says which studio to show up at** | `wellnessBookings` projects name/time/rate only (`lenses.go:317`, no studio walk), so `myClassCard` (`app.js:676`) renders a booking with no place — live, Riley holds seats at three different studios and three of them run a class named "Vinyasa Flow". The Schedule card already shows `studioName`, and the session carries both `atStudio` and `atLocation`. | Wellness | pkg + FE | ★★ | S | 📋 ready |
...
| **The classic-demo wellness world duplicates itself every seed run** | `seed-classic-demo.go` pins the clinic patient + provider precisely so a rerun cannot render indistinguishable duplicates, but leaves the studio + session unpinned and unguarded (`:213-241`) — live, the public `/api/studios` lists six "Classic Demo Studio" rows and three identical "Vinyasa Flow" classes at the same 2026-08-01T19:00Z slot. | Wellness | pkg | ★ | XS | 📋 ready |
...
- **Rotation to date:** LoftSpace ×18, Clinic ×17, Café ×8, Wellness ×5.
