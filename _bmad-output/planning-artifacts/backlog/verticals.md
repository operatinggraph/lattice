...
| **Six identical staff members manage Unit 3** | `ensureStaff`'s idempotency probe (`seed-showcase.go:593`) skips any `frontOfHouse` candidate that also holds `consumer`, but signing in EARNS `consumer` (seedStaff's own doc says so) — the guard self-defeats after the first sign-in and every later run mints another "Dana Whitfield". Live: 6 of her, dated 07-20 → 07-30, all `manages` Unit 3; the pinned `seedLandlordWorld` stays at 1. | LoftSpace | pkg | ★★ | XS | 📋 ready |
...
| **The classic-demo world duplicates itself every seed run** | `seed-classic-demo.go` pins the clinic patient + provider so a rerun cannot render indistinguishable duplicates, but leaves the LoftSpace unit+listing (`:81-96`) and the studio+session (`:213-241`) unpinned and unguarded — live, LoftSpace's entire browsable inventory is seven identical "12 Classic Demo Ave" $2200 1-beds, and `/api/studios` lists six "Classic Demo Studio" rows with three identical "Vinyasa Flow" classes at one slot. | Cross-vertical | pkg | ★★ | XS | 📋 ready |
