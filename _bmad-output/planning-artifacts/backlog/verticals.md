...
| **A relisted unit advertises its previous tenant's move-in date** | `missing_relist` flips status only; `availableFrom` keeps the old move-in and a bare application is approved from it — a tenancy starting before the last ended. Live: 50 Riverside Walk reads 2025-09-06 under a 2027-01-31 notice. | LoftSpace | pkg + FE | ★★ | M | 🏗️ building · [design](../../../docs/reviews/loftspace-unit-turnover-2026-09-17.md) · next: Inc 1 `FloorListingAvailability` |
...
| **A moved-out, declined or lost applicant can never apply to that unit again** | The `(applicant, unit)` guard link is freed only by withdraw and reassign; `EndTenancy`, a decline and `RecordApplicationLoss` leave it alive, so `CreateLeaseApplication` refuses `DuplicateApplication` forever. | LoftSpace | pkg | ★★ | S | 🏗️ building · [design §4](../../../docs/reviews/loftspace-unit-turnover-2026-09-17.md) · same fire as the row above |
...
| **A visit inside a provider's later-declared time-off is reminded, then swept to no-show** | `SetProviderTimeOff` writes the provider only ([ddls.go:2293](../../../packages/clinic-domain/ddls.go:2293)); the reminder's gate reads status alone ([lenses.go:28](../../../packages/clinic-reminders/lenses.go:28)), the patient's card hides the conflict ([app.js:5372](../../../cmd/clinic-app/web/app.js:5372)), the sweep closes it as a no-show. Record the displacement on the visit at the time-off write; reminder + sweep read it; patient told. Live: 0. | Clinic | pkg + FE | ★★ | M | 📋 ready |
| **A visit the desk cancels or moves tells the patient nothing** | Only the 24 h reminder, the follow-up and the arrears reminder ever go out; a staff `SetAppointmentStatus(cancelled)` and a `RescheduleAppointment` more than 24 h out write `.status`/`.schedule` in silence ([ddls.go:3923](../../../packages/clinic-domain/ddls.go:3923), [:3699](../../../packages/clinic-domain/ddls.go:3699)); a move inside 24 h is re-reminded at once. Wellness's mirror shipped `f139d128`. Live: 23 cancelled, none noticed. | Clinic | pkg | ★★ | M | 📋 ready |
| **An appointment's status has no time** | `.status` is `{value, note?, fee?}` ([ddls.go:4039](../../../packages/clinic-domain/ddls.go:4039)); no transition stamps when, so the desk cannot see how long a checked-in patient has waited and a cancel has no recorded moment to audit against the 24 h window. `at = op.submittedAt` on every transition (wellness's `bookedAt` idiom), projected; "checked in 12 min ago" on the card. | Clinic | pkg + FE | ★★ | S | 📋 ready |
