...
| **Facet's captured-once login JWT never refreshed — permanent zombie at 30m** | Live-verified (facet.log, 5+ hr WARN loop): `devTokenTTL`=30m token frozen at Acquire broke NATS reconnect (permanent, nats.go's repeated-auth-error abort), Gateway-submit writes (silent forced sign-out), and browser-native's `getToken`; even a fresh re-login handed back the same dead engine. | Facet | pkg | ★★★ | M | ✅ shipped `e1ddf8a2` |
| **Facet sessions are immortal while a tab stays open** | Token-refresh residual: the refresher renews unconditionally (20m interval + visibility, no idle gate) and the Go-host engine re-mints while a feed holder is attached — an abandoned tab keeps a session alive forever. Add a client-side idle gate on the refresh cadence + the terminal auth-death bounce to `/login` (Loupe's 401 reaction; the "expiry has no UX" residual), both modes, stateless. Consumer: hosted live demo — host up days, shared tabs. | Facet | FE | ★★ | S | 📋 ready |
...
| **Facet surfaces ops whose dispatch target can't resolve** | Live-verified in-browser: wellness "Book a class" from the template sheet fails (`session: required vtx.session.<NanoID>; got vtx.identity.…`) — `dispatch.targetField` (`session`; clinic `provider` same shape) expects "the entity being viewed," but Facet has no session/provider browse surface and the showcase world seeds no sessions. Hide/disable ops whose targetField can't resolve from context; booking beats then need a browse surface + seeded sessions. | Facet | FE + pkg | ★★ | M | 📋 ready |
| **Facet café OpenTab asks the user to paste their own leaseAppKey** | Live-verified in-browser: the "Open a house tab" descriptor form renders `leaseAppKey` as a required free-text field, but the signed-in identity's own leaseapp is already in its manifest (the Sign Lease task names it). Auto-fill self-anchored keys the manifest already carries instead of asking the visitor to paste a vertex key. | Facet | FE | ★★ | S | ✅ shipped `4cc6e906` |
...
| **Display names across the graph ("fewer NanoIDs")** | Andrew PO pass: raw NanoIDs are the primary label across Facet (header, Unit/Building chips, leaseapp tasks, "Unnamed" Me). Four display classes (metas/business/identity/relational) + the floor rule "a bare NanoID is never a primary label" — see design. | Cross-vertical | FE + pkg | ★★★ | M–L | 🏗️ building · [design](../../implementation-artifacts/display-name-convention-design.md) · N1 pkg shipped (location `.presentation` + me-anchor names); next N2 renderer floor + row context, then N3 self-name |
