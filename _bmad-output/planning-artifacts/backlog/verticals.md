...
| **Patient/identity chrome leaks raw NanoIDs across identities** | `clinic.patient` (`app.js:12`) is one global localStorage key, never cleared on logout, read before whoami resolves — switching identity leaks the prior identity's raw patient NanoID into Book Appointment's Patient field (`shortKey()` fallback, `app.js:302`; live-reproduced). `/api/whoami` never resolves a name either — header "Signed in ·" always shows a raw NanoID. Never render a bare vtx key; scope+clear the selection per identity, resolve real names both places. | Clinic | FE | ★★★ | S | 📋 ready |
...
- **2026-07-27:** Clinic (Andrew-directed) — repro'd a reported patient/NanoID display leak live; root-caused to an unscoped localStorage key + whoami never resolving a name + no self-booking write path (403, Checkpoint unfiled); filed 3.
- **Next:** Café.
