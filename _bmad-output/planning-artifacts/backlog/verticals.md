...
| **A DDL script reading an op's own `targetField` can outrun its declared `Reads` undetected** | Only `orchestration-base` has a guard test (`dispatch_reads_guard_test.go`) pinning each op's script-read keys against its declared `Reads`/`OptionalReads`; clinic-domain, wellness-domain, service-domain, clinic-reminders don't — wellness's `SetInstructorProfile` shipped the gap live, caught only by manual review. | Cross-vertical | pkg | ★★ | S | 📋 ready · [origin](../../implementation-artifacts/staff-descriptor-rendering-design.md) §16 |
