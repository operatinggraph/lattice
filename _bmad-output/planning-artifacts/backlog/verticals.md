...
| **Vertical Package Standard — canonicalize the reference corpus** | THE standard a vertical package builds to (S1–S8): persona-complete roles/bindings/grants + producers on both read paths, op-metas for every user-facing op, read-posture-clean scripts, tiered read boundary, verify script + structure pins. W1–W4 briefs cite it. | Cross-vertical | pkg | ★★★ | L | 🏗️ building · [standard §7](../../implementation-artifacts/vertical-package-standard.md) · S7 + structure-pins clean, S1 debt now identity-domain only · next: 7 identity-domain S1 (§3.4) + 5 lens-cypher |
