...
|---|---|---|---|---|---|---|
| **Rent is billed from the day of approval to forever, never from leaseStart to leaseEnd** | The rent clause is minted at approval and charges on a null `chargeValidUntil`, then every lapse ([lenses.go:269](../../../packages/semantic-contracts/lenses.go:269)) — nothing reads `.tenancy`. Live: Priya Raman moves in 2026-09-08 and already owes $4,800 (debits 08-03, 09-02); Jordan Ellis's term ended 09-06, billed 09-05 for the month after. | LoftSpace | pkg | ★★★ | M | 📋 ready · clause carries the term; timer-recorded start/end facts |
| **A signed renewal's new rent never reaches the bill** | `SignRenewal` extends `.tenancy.leaseEnd` only ([renewal_scripts.go:489](../../../packages/lease-signing/renewal_scripts.go:489)); the clause keeps its original `amountCents` and `.terms.rentAmount` is read by the display lens alone. Live: Jordan's renewal quotes $2,125 against a $2,050 clause. | LoftSpace | pkg | ★★★ | S | 📋 ready · re-price / supersede the rent clause at signature |
