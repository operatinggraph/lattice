...
| **A guest's debt outlives the booking that made them visible** | Lease-less coverage comes from `wellnessBookers` (live bookings, no status filter) and `CancelBooking` tombstones the booking where its no-show twin keeps the vertex under a terminal status, so a forfeit stands on someone no desk can reach. Census 2026-09-05: 0 of 4 debtors in this shape. | Wellness | pkg + FE | ★ | S+S | 🏗️ building · [triage §3 + brief](../../../docs/reviews/verticals-designer-triage-2026-09-10.md) · next: pkg increment (CancelBooking late branch → `forfeited`), then FE |
