...
| **Maintenance work-order producer** | The op set that mints role-queued maintenance work orders (report → `vtx.task` queuedFor a maintenance role) — the domain content the staff-worlds offline beat (F5, basement tech) claims and completes; the claim/complete/offline mechanism ships in the staff-worlds fires, this row is only the producer. | Cross-vertical | pkg | ★★ | S | 📋 needs-design: `CreateTask`-gated, so needs a completion op + tech-resolvable subject + role (reuse `backOfHouse`). consumer: [F5](../../implementation-artifacts/facet-staff-worlds-design.md) |
