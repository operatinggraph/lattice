...
| **A posted charge has no receipt** | A statement line is "+$8.00 — Latte, Croissant"; the settled tab's `lines` (per-line price, who ordered, voids) are already on `/api/tabs`, keyed by the entry's `tabKey`, but only an open or still-posting tab renders them. Join them so a resident (and the desk) can open any charge to what it was for. | Café | FE | ★ | S | 🏗️ building · brief in the build commit · next: receipt join in the ledger list |
| **The desk can't take an item off the menu for the day** | `RetireMenuItem` tombstones and a re-add mints a new key; a sold-out latte needs an availability flag `menuCatalog` projects, both pickers grey out and a self-order `Charge` refuses `ItemUnavailable`, plus a desk toggle. | Café | pkg + FE | ★ | S | 🏗️ building · brief in the build commit · next: `SetMenuItemAvailability` + `.price.available` |
