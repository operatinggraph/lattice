...
| **`worksAt_covers` pages `containedIn` and drops the cursor** | Same bug class `actor_holds_operator` just closed (below), same S10-pinned-helper shape, still open: 9 copies discard `kv.Links`' cursor over a `WORKPLACE_PARENT_PAGE_LIMIT=20` walk. Lower risk than the role case (a containment tree rarely nests 20+ parents on one node) but same fail-closed-but-wrong defect. Mirror the just-shipped fix (cursor-following loop + a paired `MAX_*_PAGES` guard constant). Consumer: staff nested 20+ levels into a workplace hierarchy. | Cross-vertical | pkg | ★ | S | 📋 ready |
...
| **Package-standard census covers 15 of 29 packages** | §3's routing was "mechanical against the census scorecards", but the census pinned 15 packages while 29 existed at that commit — 14 were routed by nothing. S1/S6/S7 are no longer the gap (those gates run over all 29 with zero exemptions); what stays unmeasured for the 14 is **S2–S5/S8** — grants, read boundary, guard idioms, D3 — which no gate can see. Consumer: the §3.3 sweep's completion claim. | Cross-vertical | pkg | ★★ | M | 🏗️ building · re-auditing all 29 vs S2-S5/S8 |
