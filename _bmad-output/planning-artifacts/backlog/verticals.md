...
| **A mis-keyed house-tab payment is unbounded, uncorrectable, and hides the resident from collections** | `CreditCafeAccount`'s descriptor promises the amount cannot exceed what is owed, "server-verified" ([opmetas.go:63](../../../packages/cafe-ledger/opmetas.go:63)); it is not. Live: $50 posted against $14.25, account now −$35.75 and off the arrears grid, with no op that undoes it. | Café | pkg + FE | ★★ | S | 📋 ready · cap the credit at the balance, or give overpayment a reversal |
| **The café can relocate and retire a menu item but can never reprice or rename one** | `.price` is write-once (`PermittedCommands: ["CreateMenuItem"]`, [ddls.go:416](../../../packages/cafe-domain/ddls.go:416)) and the only other catalog ops are `RetireMenuItem` + `SetMenuItemLocation`, so a price rise or a typo costs a retire plus a fresh key. Live: 4 items, prices frozen since mint. | Café | pkg + FE | ★★ | XS–S | 📋 ready · a `SetMenuItemPrice` mirroring `SetMenuItemLocation` |
