...
| **Vertical Package Standard — canonicalize the reference corpus** | THE standard a vertical package builds to (S1–S8): persona-complete roles/bindings/grants + producers on both read paths, op-metas for every user-facing op, read-posture-clean scripts, tiered read boundary, verify script + structure pins. W1–W4 briefs cite it; per-package fires decide converge-vs-rewrite. | Cross-vertical | pkg | ★★★ | L | 🏗️ building · [standard](../../implementation-artifacts/vertical-package-standard.md) · next: Inc 1 S1 descriptors |
...
| **Op-metas still declare `authContext: standing` for the five landlord ops** | `DecideLeaseApplication`'s op-meta says `standing`, and no `OpDispatchSpec` names an authContext for SetRenewalTerms / VerifyGuarantor / CancelRenewal / SetListingStatus at all, so Facet's descriptor-driven renderer sends none and lands on the standing path. Consumer: Facet's landlord/front-desk worlds. | Cross-vertical | pkg | ★★ | S | 🏗️ building · subsumed by Package Standard §3.3 Inc 1 · [§6](../../implementation-artifacts/vertical-package-standard.md) |
