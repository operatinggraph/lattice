...
| **Front desk can't read the patient roster** | `/api/staff/patients` returns 0 for every `worksAt`-anchored front-desk identity (live: 4 read appts=9, series=2, patients=**0**) — `read_clinic_patients` anchors each row on its patient's NanoID alone, unlike `read_clinic_appointments`, which also carries the `atSite` building `cap-read.staff` grants. The wildcard holders that DO read it carry no `worksAt`, so the FE hides every staff tab from them. Empty roster ⇒ no switcher, no book-on-behalf. Fix: anchor `clinicPatientsRead` on the workplace. | Clinic | pkg | ★★★ | M | 📋 ready |
| **`/api/residents` is clinic-app's one unconfined read** | It serves every lease applicant's identity NanoID + landlord-approval flag to any signed-in session (live: a patient hat got all 8) — `computeResidents` over raw `weaver-targets`, no actor scoping. café-app and wellness-app both filter the identical picker through `resolveSubjectHats`/`visibleLeases`; clinic-app has zero occurrences of it — the sweep that added it (`48a06798`) scoped itself to "wellness+café". Every other clinic read is scoped: `/api/ledger` cross-patient 403s. | Clinic | pkg + FE | ★★ | M | 📋 ready |
| **No signable patient account has a visit series or a ledger** | `seed-showcase.go` binds one patient (Riley) + one provider (Osei) and books appointments, but all 4 visit series and the single `clinicaccount` belong to patients carrying no `identifiedBy` link — so both patient-facing tabs render empty for every account that can sign in (live: `/api/my-visit-series` count=0 and `/api/ledger` balance 0 for both bound patients). Same class as the LoftSpace seed blockers fixed in `b3458cb5`. | Clinic | pkg | ★★ | S | 📋 ready |
| **No showcase resident can order anything** | `seed-showcase.go` never calls `CreateMenuItem`, so the self-order picker is empty for all 10 showcase leases (live) — the mechanism itself is fine (a probe item `servedAt` the building appeared instantly, then retired clean). Only `seed-classic-demo.go` seeds a menu, and with no `alive()` guard it re-mints on every run (Latte×2, Croissant×2). Same class as `b3458cb5`. | Café | pkg | ★★★ | S | 📋 ready |
| **The Café's composition layer is installed by nothing** | `front-desk` (3 lenses, 3 endpoints, 3 FE joins) and `one-bill` sit in no install target — not `install-cafe`/`up-cafe`/`install-showcase-domains` — so live all three front-desk surfaces return `[]` and the grid drops every class/rent/visit badge, indistinguishable from "this resident has none": the handler reads a missing bucket as no-rows, the FE swallows it in bare `catch (_)` ([app.js:497-517](../../../cmd/cafe-app/web/app.js)). `one-bill-history` has zero readers. | Café | pkg + FE | ★★★ | M | 📋 ready |
