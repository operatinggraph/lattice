...
| **No way to demonstrate that Facet survives going offline** | Offline-first is the Edge's headline claim and nothing lets a viewer see it. Per [UX §6/§11](../../implementation-artifacts/facet-app-ux.md): a truthful toggle pauses the host engine's host↔NATS traffic — reads keep serving from bbolt, writes queue and drain on release. | Cross-vertical | Sally + FE + platform | ★★ | M | 🏗️ building · §11 · next: gates
