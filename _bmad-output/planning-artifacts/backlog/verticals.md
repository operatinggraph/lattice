...
| **A self-order is half a loop — nobody is told to make it** | A resident's phone order lands as a bare line on one of N open-tab cards: no `orderedAt`, no made/served state, no new-order signal, the desk polls Refresh. Live: `Latte · by Riley Chen`, indistinguishable from one served an hour ago. | Café | pkg + Sally + FE Engineer | ★★★ | M | 📋 ready · line `orderedAt` + a served mark on `.status.lines`, a Front Desk orders queue oldest-first |
