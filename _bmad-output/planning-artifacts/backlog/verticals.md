...
| **A moved visit keeps its arrival** | `RescheduleAppointment` leaves `.status` untouched ([ddls.go:3602](../../../packages/clinic-domain/ddls.go:3602)) and the desk offers Reschedule on `checkedIn`/`confirmed`: a checked-in visit moved to next week reads "checked in" a week early, the sweep exempts it, so a later no-show is never recorded or billed. A move returns it to `scheduled`. | Clinic | pkg + FE | ★★ | S | 🏗️ building · [design](../../../docs/reviews/clinic-visit-record-integrity-2026-09-16.md) · next: Inc 1 clinic-domain |
