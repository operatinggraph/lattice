...
| **Payload-probe reads answer ahead of their ownership guard (sweep)** | A payload-match probe fails distinguishably before the `authContextTarget` guard, so a scope=self consumer brute-forces it — the maintenance oracle, reachable. Worst: lease-signing `WithdrawLeaseApplication` (`scripts.go:662,671`) leaks who applied where; also clinic `RescheduleAppointment` (`ddls.go:2283,2286`, PHI) + `SetAppointmentStatus` (`:2422`), wellness `CancelBooking` (`:1453`) + `TombstoneSession` (`:1153`), orchestration-base `ClaimTask` (`:401,433`). | Cross-vertical | pkg | ★★ | M | 📋 ready |
