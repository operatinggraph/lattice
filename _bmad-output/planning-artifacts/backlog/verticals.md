...
| **edgeCatalog retries a poison event forever** | Root-caused to the Refractor hot-reload path: a Personal Lens edited after its process started lost its business key columns, so every write was rejected and retried forever. Fixed; live loop cleared (142k errors → 0). | Facet | pkg + platform | ★★★ | S–M | ✅ fixed · tail: hot-reload path proven by test, not live — `reinstall-package` never bumps a lens spec revision, so no MATCH update could be triggered |
