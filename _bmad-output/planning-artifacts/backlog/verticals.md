...
| **The studio's grid runs dry and nothing says so** | 0 of 43 sessions is upcoming — the member's Schedule tab now renders empty — `CreateSessionSeries` mints a fixed `occurrenceCount` batch with rolling recurrence a declared non-goal ([ddls.go:439](../../../packages/wellness-domain/ddls.go:439)), and no lens flag, task or horizon signal tells staff the schedule is about to end. | Wellness | pkg + FE | ★★ | S | 📋 ready |
...
| **Nobody at the front desk can mark a member present** | `SetBookingAttendance` grants `[operator, provider]` only — [permissions.go:58](../../../packages/wellness-domain/permissions.go:58) excludes frontOfHouse as "unconfined authority", yet the sibling `CancelBooking` on the same session is granted to it and confined by `require_workplace(session_locations(session))`. One instructor exists; 30 of 43 classes are unled, so 20 of 25 bookings have no marker but break-glass operator. Live: 25 of 25 bookings `noShow`, zero `attended`. | Wellness | pkg + FE | ★★ | S | 📋 ready |
| **Every no-show line on a member's bill reads the same** | [lenses.go:200](../../../packages/wellness-ledger/lenses.go:200) projects the literal `'No-show fee' AS memo`, while the sibling class-price settlement passes `row.sessionName` ([targets.go:123](../../../packages/wellness-ledger/targets.go:123)) and the op's own schema documents `"No-show fee — Vinyasa Flow"`. The cypher already reads `bk.status.data`, where `className`/`classStartsAt` are carried forward. Live: 11 of 15 lines on one $335 statement are identical `$25.00 No-show fee`. | Wellness | pkg | ★★ | XS | 📋 ready |
| **Booking a yoga class downloads the whole deployment's op catalog** | `/api/op-catalog` serves all 100 descriptors (106 KB, 7× the schedule payload) to every signed-in member including a plain one; the FE names 15. It carries clinic (`CreatePatient`, `RecordEncounter`), loftspace (`SignLease`), café (`OpenTab`) and platform-admin (`RecordIdentityPII`, `RotateClaimKey`) ops with full input schemas. Same unfiltered handler in all four `cmd/*-app/op_catalog.go`. | Cross-vertical | FE + pkg | ★ | S | 📋 ready |
