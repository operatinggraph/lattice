...
| **15 appointments carry no site and `clinicSiteBackfill` closes the gap for none** | Every one is Dr. Patel's, who practises at ONE live site — the replay dispatched all 15 and `BackfillAppointmentSite` no-op'd each: it counts `practicesAt` links, and her second link names a tombstoned building. A package bug, not a Weaver gap. | Clinic | pkg | ★★ | S | 🏗️ building · [§17](../../implementation-artifacts/weaver-decline-retry-substrate-native-design.md) · next: the op counts live sites only, clinic-domain 0.34.26, replay again |
...
- 2026-09-13 · `(docs)` · 8 bookings `booked` on ended classes — resolved live before any replay: `pastDueBookings` shows 0 violating, every 07-20…08-23 booking is `noShow`; the gate was the fire classifier's, not Andrew's.
