...
| **A recurring class stops when its count runs out** | `occurrenceCount` (≤52) is the series' whole life; Evening Flow with Sam ends Sep 23 and Riverside's schedule is then empty until the desk retypes the series. A rolling horizon mints the next occurrence as the last approaches; the studio card's "runs out in N days" is the only guard today. | Wellness | pkg + FE | ★★ | M | 🏗️ building · [design](../../../docs/reviews/wellness-rolling-series-2026-09-16.md) · next: Inc 1 package (horizon + lens + op) |
| **An approved lease never moves the tenant in** | `DecideLeaseApplication` records `.tenancy` and `EndTenancy` ends it, but neither wires nor unwires `residesIn` — the residence spine only the seed and `operator`-only `WireResidesIn` write ([permissions.go:24](../../../packages/service-location/permissions.go:24)), the left edge of `cap.svc.<actor>` and every whoami anchor. Live: Jordan Ellis and Priya Raman have none; seed-wired Riley and Sam reach Maple Laundry, they don't. A convergence target wires at approval, unwires at `endedAt`. | LoftSpace | pkg | ★★ | M | 📋 ready |
| **A reported issue never becomes work** | `ReportIssue` mints the work order and deliberately not its task ([package.go:12](../../../packages/maintenance-domain/package.go:12)); only the seed submits `CreateTask`. Live: 17 of 18 work orders are seed-minted with tasks; the one reported through the app ("Kitchen tap is dripping", 07-29) has had no task for 49 days. A `missing_task` gap on an unresolved, unqueued order queues `ResolveWorkOrder` to `backOfHouse` at the building. | LoftSpace | pkg | ★★ | S | 📋 ready |
| **A tenant cannot report a problem with their own home, and their landlord never sees it** | `ReportIssue` grants `operator` + the staff roles behind the workplace guard ([permissions.go:31](../../../packages/maintenance-domain/permissions.go:31)); the app offers the form to staff only ([app.js:3011](../../../cmd/loftspace-app/web/app.js:3011)); the landlord console lists no work orders. A `consumer` self grant behind the tenancy self probe, "Report an issue" on the tenant card, a landlord lens of orders at units they `manages`. | LoftSpace | pkg + FE | ★★ | M | 📋 ready |
