...
| **A wellness class costs nothing and shows nothing** | `wellness-ledger` is dormant behind the app: `wellnessSessions` projects `priceCents` but the FE neither sets it (CreateSession's payload carries no price) nor renders it, no `vtx.wellnessaccount` has ever been opened (`CreateAccount` reaches no UI — loftspace-app's `openLedgerAccount` mirrors it), so both settlement targets charge nobody and neither ledger bucket is read. | Wellness | Sally + FE Engineer + pkg | ★★★ | M | 📋 ready |
