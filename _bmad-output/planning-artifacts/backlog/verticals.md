...
| **LoftSpace can't work a maintenance queue** | 14 work orders sit `locatedAt` LoftSpace units but `cmd/loftspace-app` wires neither `ReportIssue` nor `ResolveWorkOrder` — no tenant report path, no super queue. 5 open tasks render as disabled cards titled by a bare NanoID; `openTaskRow` ([tasks.go:34](../../../cmd/loftspace-app/tasks.go)) drops the lens's `queuedRole` so none can be claimed. | LoftSpace | pkg + FE | ★★★ | L | 📋 ready — resized M→L: no op-dispatch pattern in loftspace-app, needs net-new FE-to-op wiring. Multi-fire. |
