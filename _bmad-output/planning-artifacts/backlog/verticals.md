...
| **A signed lease with no rent offer is never billed rent** | 4 of 8 signed leases have no ledger account and no rent clause, so rent is never charged — live: 20 Riverside Walk, unit rent $1,750, signed 2026-07-31, balance $0. `leaseRentSettlementSpec` gates `missing_account` on `requestedRent` present ([lenses.go:76](../../../packages/semantic-contracts/lenses.go)); the apply form leaves rent optional ([app.js:1363](../../../cmd/loftspace-app/web/app.js:1363)) and the unit's listed rent is never the fallback. | LoftSpace | pkg | ★★★ | S–M | 📋 ready |
...
| **A submitted application is invisible to applicant AND landlord, and the app calls the projection healthy** | A live `CreateLeaseApplication` committed 12:54Z was absent from `read_lease_applications` and `read_landlord_lease_applications` at 13:03Z; both endpoints answered `projectionHealthy: true`. `withProjectionHealth` reports paused-vs-not only ([projectionhealth.go](../../../cmd/loftspace-app/projectionhealth.go)); Health KV already carries the lens's lag. | LoftSpace | FE + pkg | ★★★ | S | 🚧 blocked-on: [lattice.md](lattice.md) Postgres-lens zero-progress row |
| **Portfolio "service attach rate" reads 0 of 4 while 3 of the 4 leases carry café tabs and wellness bookings** | The composition KPI counts only an UNSETTLED tab or a currently-booked class ([portfolio.go:212](../../../cmd/loftspace-app/portfolio.go:212)); live, all 70 `cafeTabSettlement` rows are `settled` and all 25 wellness bookings are `noShow`, so `front-desk-bookings` holds 0 live keys and the number is structurally 0. A landlord asking who uses building services wants attached-this-period, not right-this-second. | LoftSpace | FE + pkg | ★★ | S | 📋 ready |
...
- **Rotation to date:** LoftSpace ×28, Clinic ×26, Café ×17, Wellness ×14.
