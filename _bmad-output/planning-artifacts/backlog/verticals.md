...
| **A relisted unit revives every losing rival** | `lost_to_rival` is a live derivation (unit leased ∧ no decision): when the winner's tenancy ends and the unit relists, every rival's gaps re-open, the card reverts to "In review" and Weaver re-dispatches their tasks (52 live). The loss becomes a recorded `.decision = lost`. | LoftSpace | pkg | ★★ | M | 🏗️ building · [design](../../implementation-artifacts/loftspace-recorded-application-loss-design.md) · next: Inc 1 package (gap + `RecordApplicationLoss`) |
