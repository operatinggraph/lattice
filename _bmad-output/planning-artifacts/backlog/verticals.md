...
| **A member never gets refunded when the studio cancels their class** | `ReleaseOrphanedBooking` (wellness-domain/ddls.go, the TombstoneSession-drain op) tombstones the booking and releases its seat/waitlist/guard cells but never mints a `wellnessrefund` marker — unlike `CancelBooking`, which does. A booked member whose class the studio calls off keeps paying for it. | Wellness | pkg | ★★ | S | 📋 ready · found auditing `CancelBooking`'s late-cancel-window fire; a studio-cancellation is not a member-initiated cancel, so it's a distinct row, not a scope extension of that one |
