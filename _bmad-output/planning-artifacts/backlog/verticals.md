...
| **No seeded landlord holds `consumer`** | Both `manages`-holding personas are the frontOfHouse staff spine, which `seed-showcase.go` gives no consumer role by design — so their cap docs carry no scope=self row and the landlord hat still cannot SetRenewalTerms / VerifyGuarantor / CancelRenewal / SetListingStatus live. Decide works on the frontOfHouse standing grant. persona-worlds §7.2 already calls `consumer` the signed-in-human role, so this is the seed, not the grants. Consumer: W2 Inc 3's shipped path. | LoftSpace | pkg | ★★★ | S | 📋 ready |
