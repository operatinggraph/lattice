...
| **A landlord's Record charge / Record payment buttons are refused** | Every landlord row gets the form ([app.js:4793](../../../cmd/loftspace-app/web/app.js:4793)) but `DebitAccount` and any-scope `CreditAccount` grant `operator` only ([permissions.go:66](../../../packages/loftspace-ledger/permissions.go:66)); a cheque received or a repair charge cannot be recorded. Live as Nora Vance: `AuthDenied` / `AuthContextMismatch`. Self grants bound through `heldFor→appliesToUnit→manages` (`4790992b`'s `require_manages`). | LoftSpace | pkg + FE | ★★★ | M | 📋 ready |
| **A landlord cannot edit the listing of a unit they manage** | "✎ Edit listing" ([app.js:4678](../../../cmd/loftspace-app/web/app.js:4678)) posts `SetUnitAddress` + `SetListing`, both `operator`-only ([permissions.go:40](../../../packages/loftspace-domain/permissions.go:40)) while `SetListingStatus` has the self grant; a rent change is refused. Live as Nora: `AuthDenied`. Self grants + `enforce_manages` for the two; "Post a listing" stays operator-conferred (the `AssignUnitOwner` decision), so hidden from a plain landlord. | LoftSpace | pkg + FE | ★★ | S | 📋 ready |
| **A tenant cannot give notice and a lease cannot end early** | `EndTenancy` refuses `NotYetEnded` ahead of `leaseEnd` ([scripts.go:1510](../../../packages/lease-signing/scripts.go:1510)) and nothing records a move-out date, so a tenant who leaves mid-term is billed by the rent clause to `validUntil` and the unit stays `leased`. A recorded notice / early end (tenant self, landlord `manages`) shortens the clause's term (`SupersedeClause`), ends the tenancy at the recorded date, relists. | LoftSpace | pkg + FE | ★★ | L | 📋 ready |
| **"Rent owed" never says how late** | The portfolio list ranks balances by amount ([app.js:4196](../../../cmd/loftspace-app/web/app.js:4196)) and neither it nor the statement reads the recorded `dueAt` ([lenses.go:84](../../../packages/loftspace-ledger/lenses.go:84)); a bill posted today and one 40 days overdue read alike, and no debtor is ever reminded. Live: Priya's $2,400 due 09-08 shows under Riley's $3,300 with no age. "Due in N days" / "N days overdue" per lease; one reminder per arrears episode (the café/wellness `.arrears` idiom). | LoftSpace | pkg + FE | ★★ | M | 📋 ready |
| **No lease takes a security deposit** | `DebitAccount`'s description names "a deposit" ([opmetas.go:107](../../../packages/loftspace-ledger/opmetas.go:107)) and `leaseRentSettlement` anticipates a one-time move-in clause ([lenses.go:108](../../../packages/semantic-contracts/lenses.go:108)), yet no listing carries one and no approval bills one; every live lease is rent lines only. `depositAmount` on the listing, a `missing_deposit` gap minting a `oneTime` clause at approval, held apart from rent on both statements (return rides move-out). | LoftSpace | pkg + FE | ★★ | M | 📋 ready |
...
- **Rotation to date:** LoftSpace ×34, Clinic ×33, Café ×24, Wellness ×21.
