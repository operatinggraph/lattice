...
| **Persona worlds — Provider archetype + sign-in-first verticals** | Fourth human archetype: `provider` role + `identifiedBy` binding from each vertical's provider entity to a real login; all four FEs become sign-in-first skins over the discovered capability set — pickers + any-subject mints deleted, per-actor submits, grants audited. | Cross-vertical | Sally + FE Engineer + pkg | ★★★ | XL | 🏗️ building · [design](../../implementation-artifacts/persona-worlds-design.md) · next: W3 wellness Inc 1 — sign-in-first + read auth + 3 hats, then W5 Facet hats |
| **Vertical Package Standard — canonicalize the reference corpus** | THE standard a vertical package builds to (S1–S8): persona-complete roles/bindings/grants + producers on both read paths, op-metas for every user-facing op, read-posture-clean scripts, tiered read boundary, verify script + structure pins. W1–W4 briefs cite it; per-package fires decide converge-vs-rewrite. | Cross-vertical | pkg | ★★★ | L | ✅ ratified (2026-07-24) · [standard](../../implementation-artifacts/vertical-package-standard.md) · next: sweep fire, non-FE packages |
...
| **Op-metas still declare `authContext: standing` for the five landlord ops** | The landlord scope=self path is reachable only from the bespoke LoftSpace FE: `DecideLeaseApplication`'s op-meta says `standing`, and no `OpDispatchSpec` names an authContext for SetRenewalTerms / VerifyGuarantor / CancelRenewal / SetListingStatus at all, so Facet's descriptor-driven renderer sends none and lands on the standing path. Consumer: Facet's landlord/front-desk worlds. | Cross-vertical | pkg | ★★ | S | 📋 ready |
...
| **Café staff read hat is workplace-unscoped** | A `worksAt` anchor to any location anywhere grants the whole café house (every tab, balance, ledger + the front-desk grid). Read/write asymmetry in one vertical: café-domain's staff *writes* are workplace-confined and Facet's staff *reads* grant-confined; this read path has no workplace term. Needs a lease→unit→building join no café lens projects ([W4 as-built](../../implementation-artifacts/persona-worlds-design.md)). Consumer: café staff reads, multi-building. | Café | pkg | ★★ | M | 📋 ready |
