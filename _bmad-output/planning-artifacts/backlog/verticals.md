...
| **A wellness class's ledger account never opens** | No `vtx.wellnessaccount` has ever been opened for a booker, so both wellness-ledger settlement targets still charge nobody. | Wellness | pkg | ★★ | S | 📋 ready — permission gap fixed (`329ac6cf`); the browser still needs to call WellnessCreateAccount from My Classes |
| **CreateAccount can't be called from any of the 4 ledger apps' browsers** | `CreateAccount` grants `["operator"]` only in every ledger package — front-desk staff hold no such role, yet `clinic-app`/`loftspace-app` FE already call it as staff — 403s for any account a seed script didn't pre-open. | Cross-vertical | pkg | ★★ | S | 🏗️ building — wellness shipped `329ac6cf`; clinic+loftspace next, S9-clean rename required (see commit) |
