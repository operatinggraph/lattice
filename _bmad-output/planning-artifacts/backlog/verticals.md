...
| **Vertical Package Standard — canonicalize the reference corpus** | THE standard a vertical package builds to (S1–S8): persona-complete roles/bindings/grants + producers on both read paths, op-metas for every user-facing op, read-posture-clean scripts, tiered read boundary, verify script + structure pins. W1–W4 briefs cite it. | Cross-vertical | pkg | ★★★ | L | 🏗️ building · [standard §6](../../implementation-artifacts/vertical-package-standard.md) · Inc 1–3 shipped, S7 + structure-pins clean · next: 16 S1 metas + 5 lens-cypher |
