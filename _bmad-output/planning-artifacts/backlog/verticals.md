...
| **No seeded landlord holds `consumer`** | The seed's one `manages` holder is the frontOfHouse staff persona, which gets no consumer role by design, so its cap doc carries no scope=self row and the landlord hat cannot SetRenewalTerms / VerifyGuarantor / CancelRenewal / SetListingStatus live. §7.2 calls `consumer` the signed-in-human role, so this is the seed, not the grants. Consumer: W2 Inc 3's shipped path. | LoftSpace | pkg | ★★★ | S | 🏗️ building · [W2 Inc 4](../../implementation-artifacts/persona-worlds-design.md) · next: seed the plain-landlord cell |
