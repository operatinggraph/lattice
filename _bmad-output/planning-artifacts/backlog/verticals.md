...
| **Joining a waitlist charges the full class price before you have a seat** | `classPriceSettlementSpec` ([lenses.go:263](../../../packages/wellness-ledger/lenses.go:263)) has no `bk.status.data.value` predicate, so a `waitlisted` booking is charged like a booked one — live, a $20 waitlist debited $20 12s before promotion. Never promoted = never refunded: `pastDueBookings` is `status='booked'`-gated ([pastdue.go:16](../../../packages/wellness-reminders/pastdue.go:16)) and Cancel dies at SessionStarted. | Wellness | pkg | ★★★ | S–M | 📋 ready · gate the charge on `booked` |
...
| **The front desk can put a class on the grid but never take it off** | `TombstoneSession` deliberately withholds the frontOfHouse grant `CreateSession` gives ([permissions.go:54](../../../packages/wellness-domain/permissions.go:54)) — only the operator or the class's bound instructor may cancel, and 30 of 43 classes have no instructor. Live: the staffer who scheduled a class 5 min earlier got `AuthDenied … (no instructor supplied)`. | Wellness | pkg | ★★ | S | 📋 ready · revisit the exclusion; workplace-confine like `a5077b9a` |
...
- **Rotation to date:** LoftSpace ×28, Clinic ×27, Café ×18, Wellness ×15.
