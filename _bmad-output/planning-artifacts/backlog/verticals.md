...
| **A self-order is half a loop — nobody is told to make it** | A resident's phone order lands as a bare line on one of N open-tab cards: no `orderedAt`, no made/served state, no new-order signal, the desk polls Refresh. Live: `Latte · by Riley Chen`, indistinguishable from one served an hour ago. | Café | pkg + Sally + FE Engineer | ★★★ | M | 🏗️ building · [design](../../../docs/reviews/cafe-self-order-fulfilment-2026-09-16.md) · next: Inc 1 package |
| **Paying at the counter is refused until the Weaver posts** | Settle posts the debit asynchronously; the desk's Record payment (Resident view) is capped at the ledger balance, so `Settle` → `CreditCafeAccount` for the tab total is `PaymentExceedsBalance` for ~9 s live (indefinitely on a paused projection). No one-act settle-and-pay on the tab card. | Café | pkg + FE Engineer | ★★ | M | 📋 ready · a `paidAtSettle` on the tab the settlement playbook posts as a credit after the debit; Pay now beside Settle |
| **The arrears row forgives a debt but cannot collect it** | Front Desk's debtor list offers Write off / Pay out only; taking a payment means Resident view → lease picker → Record payment. The one-click action is the one that loses money. | Café | Sally + FE Engineer | ★★ | S | 📋 ready · Take payment beside Write off, prefilled to the balance |
| **A house tab has no limit** | Self-order is capped only by the credit hold, which arms 15 days + a reminder after the first unpaid debit; a new resident can run any total in the meantime. No recorded tab limit anywhere. | Café | pkg + FE Engineer | ★★ | S | 📋 ready · a recorded limit (precedent: wellness no-show policy), `Charge`/`OpenTab` self path refuse `TabLimitExceeded`, desk warned |
| **Tab and visit times render as raw UTC ISO** | `Opened 2026-09-16T12:37:47Z`, `Visit: 2026-09-25T10:00:00Z` on every POS/Front Desk/Resident card, while due dates on the same cards are localized. | Café | FE Engineer | ★ | XS | 📋 ready · format via the same `toLocaleDateString`/time idiom the due dates use |
