...
| **Paying at the counter is refused until the Weaver posts** | Settle posts the debit asynchronously; the desk's Record payment (Resident view) is capped at the ledger balance, so `Settle` → `CreditCafeAccount` for the tab total is `PaymentExceedsBalance` for ~9 s live (indefinitely on a paused projection). No one-act settle-and-pay on the tab card. | Café | pkg + FE Engineer | ★★ | M | 🏗️ building · [design](../../../docs/reviews/cafe-counter-payment-2026-09-16.md) · next: Inc 1 packages (`Settle{paidCents}`, `missing_payment`, credit `paysFor`) |
