...
| **A tab's total and its lines disagree after an amount-only void** | `VoidCharge` without `lineId` takes any amount off `totalCents` (clamped at 0) and touches no line, so an itemized tab settles below its own receipt — the statement posts "Latte, Croissant" at $3.50 and Today counts a Latte sold against a negative unitemized remainder. Every tab is settled, so the legacy form has no live consumer: retire it (or bound it to the unvoided lines, recorded as a line). Live: 3 (`5Vvfxnj…` $66.50→$0, `AZpDNzx…` $143.50→$0, `TovMPnX…`). | Café | pkg | ★★ | S | 🏗️ building · next: lineId required |
| **An overdue debtor opens a new tab unasked** | The POS picker says "awaiting approval" / "lease ended" but never "owes $47.50 · 31 days overdue" (already on `/api/frontdesk-balances`), and neither `OpenTab` path reads the account's `.arrears` — the desk opened a tab for the 31-day debtor live; a resident self-serves the same way. Picker badge + confirm (the Wellness/Clinic desk pattern) and a credit hold once the reminder has gone out. | Café | FE + pkg | ★★ | S | 🏗️ building · next: `CreditHold` + picker badge |
| **A posted charge has no receipt** | A statement line is "+$8.00 — Latte, Croissant"; the settled tab's `lines` (per-line price, who ordered, voids) are already on `/api/tabs`, keyed by the entry's `tabKey`, but only an open or still-posting tab renders them. Join them so a resident (and the desk) can open any charge to what it was for. | Café | FE | ★ | S | 📋 ready |
| **The desk can't take an item off the menu for the day** | `RetireMenuItem` tombstones and a re-add mints a new key; a sold-out latte needs an availability flag `menuCatalog` projects, both pickers grey out and a self-order `Charge` refuses `ItemUnavailable`, plus a desk toggle. | Café | pkg + FE | ★ | S | 📋 ready |
