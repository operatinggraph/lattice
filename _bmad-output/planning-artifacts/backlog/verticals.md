...
| **TombstoneStudio strands a wellness session's outstanding seats** | `session_locations` denies once a studio is tombstoned (§11); unlike the shipped clinic sibling, a session has no atSite-equivalent fact of its own — needs a location snapshot at CreateSession time, not a resolver tweak. Operators can act; patients can self-cancel. | Wellness | pkg | ★★ | S | 📋 ready |
...
- 2026-07-27 · `0badf04e` · `RescheduleAppointment`'s `provider` field gets `x-entityRef` — Facet's picker resolves it via `edgeEntityProviders`; `patient` stays free-text, covered by the existing D3 edge-plane block on the visit-series trio row
- 2026-07-27 · `6985a24d` · seed-showcase's recovery helpers fail loud on a KV read anomaly, warn on an ambiguous bind — the RevisionConflict/orphan-adopt + ctx-window gaps stay open
