...
| **The patient's bill silently drops lines and shows the wrong balance** | 6 of 60 live statement reads truncated or 502'd — balances of $625/$800/$900 against a true $150. `handleLedger` scans two whole buckets; its `get` closure swallows each `KVGet` error and [computeLedgerHistory](../../../cmd/clinic-app/ledger.go:57) skips the row, then derives the balance from the survivors. `cmd/loftspace-app/ledger.go:216` shares the shape. | Clinic | FE + pkg | ★★★ | S–M | 📋 ready · scope the read to one patient's rows, fail loud per key |
...
| **The clinic's automated no-show sweep always bills $25** | `MarkPastDueNoShow` stamps the 2500 default on any appointment whose `endsAt` passes unattended ([ddls.go:788](../../../packages/clinic-domain/ddls.go:788)) — the behaviour ruled against and fixed for Wellness. 32 charges were hand-waived live on 2026-08-26, 6 identical ones still stand ($150 on Riley Chen), and 24 future appointments are armed to re-bill. | Clinic | pkg | ★★ | S | 📋 ready · mirror wellness-reminders 0.3.3's typed-zero fee + a waiver pass for the 6 standing |
