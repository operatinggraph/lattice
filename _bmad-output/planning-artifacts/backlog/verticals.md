...
|---|---|---|---|---|---|---|
| **First Postgres protected read-model + read boundary (D1.3) — Fires 2–3** | The loftspace consumer of the D1.3 RLS enabler: a protected Postgres lease-applications lens + authenticated `handleApplications` reader, and the landlord/residence audience (a landlord sees only their managed units' applications). | LoftSpace | pkg + FE | ★★★ | L | 🏗️ building · [design](../../implementation-artifacts/first-postgres-read-model-and-read-boundary-design.md) · applicant Fires 2–3 + landlord/residence Inc 1–3 shipped (RLS enforcement LIVE); remaining = the security e2e (revoked-by-tombstone grant ⇒ RLS denies — D1.3's acceptance test, per grant-table-anchor-tombstone-retraction-design.md) |
