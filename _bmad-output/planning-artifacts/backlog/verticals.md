...
| **A waitlister seated inside the late-cancel window is charged and cannot back out** | `CancelBooking` promotes in-batch and the promotion gap runs until `startsAt` ([lenses.go:689](../../../packages/wellness-domain/lenses.go:689)), so a seat handed 63 min out is charged under the 2 h forfeit ([ddls.go:3894](../../../packages/wellness-domain/ddls.go:3894)); no `promotedAt` recorded, the flip is silent. Live: two forfeits, $20, a 1-seat class ran empty. Stop promotion at the window (or restart the clock at seating); badge the seating. | Wellness | pkg + FE | ★★★ | M | 📋 ready |
| **Retire studio: offered to the desk that cannot, unguarded for the operator who can** | Retire renders for every staff hat ([app.js:3360](../../../cmd/wellness-app/web/app.js:3360)) but `TombstoneStudio` grants `operator` only ([permissions.go:114](../../../packages/wellness-domain/permissions.go:114)) — opaque toast; no upcoming-classes refusal in the op ([ddls.go:1985](../../../packages/wellness-domain/ddls.go:1985)), so a retired studio keeps booked, charged classes. Refuse `HasUpcomingClasses`, grant frontOfHouse like CreateStudio. | Wellness | pkg + FE | ★★ | S | 📋 ready |
| **The desk's No-show click bills an undisclosed flat $25 on top of the class price** | The roster button says only "No-show" ([app.js:2903](../../../cmd/wellness-app/web/app.js:2903)) and sends no fee, so `SetBookingAttendance`'s 2500 default applies ([ddls.go:995](../../../packages/wellness-domain/ddls.go:995)): a $0 resident class no-show costs $25, a $15 guest $40, and the studio has no fee policy (`studioProfile` = `{name}`). A studio-level `noShowFeeCents`, the button states the amount, the desk may mark fee-free. | Wellness | pkg + FE | ★★ | S | 📋 ready |
| **A wellness debtor is never told, never reminded, and books unasked** | The member statement says "Balance owed" with no due date or overdue flag ([app.js:488](../../../cmd/wellness-app/web/app.js:488)) though `deriveStatement` computes both; no arrears reminder op/notification exists (café precedent: [notifications.go:53](../../../packages/cafe-ledger/notifications.go:53)); a 3-day-overdue member self-books with no word (live: Alex Kim, $20). Mirror café: reminder recorded + shown, `CreateBooking` refuses `CreditHold` off it. | Wellness | pkg + FE | ★★ | M | 📋 ready |
...
- **Rotation to date:** LoftSpace ×33, Clinic ×33, Café ×24, Wellness ×21.
