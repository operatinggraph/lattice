...
| **Approval signs a lease on terms the applicant never reviewed** | `.tenancy` comes from the listing's `availableFrom`+`leaseTermMonths` ([scripts.go:878](../../../packages/lease-signing/scripts.go:878)), rent from the applicant's offer ([lenses.go:189](../../../packages/semantic-contracts/lenses.go:189)); the "review before signing" panel ([app.js:1960](../../../cmd/loftspace-app/web/app.js:1960)) shows neither. Live: Alex Kim asked 08-21, leased 08-28; listings are available-from 08-23, so a 09-15 ask bills from 08-23. | LoftSpace | pkg + FE | ★★★ | M | 📋 ready |
| **A lease that ends without renewal never frees the unit** | The only listing flip is `leased` on approval ([targets.go:124](../../../packages/lease-signing/targets.go:124)); a cancelled renewal parks terminally, so past `leaseEnd` the unit stays `leased`, hidden from Browse, with no Relist ([app.js:4261](../../../cmd/loftspace-app/web/app.js:4261)), the tenant's card never says it ended, and a new applicant is refused `UnitNoLongerAvailable`. Level-triggered relist at the recorded `leaseEnd` lapse + an ended-lease state on both cards. | LoftSpace | pkg + FE | ★★★ | M | 📋 ready |
| **A losing applicant is never told the unit went to someone else** | A rival parks terminal-not-violating (decision null, unit leased) and the card reads "In review — complete the open steps below" ([app.js:1832](../../../cmd/loftspace-app/web/app.js:1832)); the inbox keeps their expired `SignLease`/`RecordIdentityPII` tasks completable ([app.js:2331](../../../cmd/loftspace-app/web/app.js:2331)), which the op refuses. Live: 52 on 12 Classic Demo Ave. A `lostToRival` column + banner; expired tasks read-only. | LoftSpace | pkg + FE | ★★ | S | 📋 ready |
| **A renewed tenant's home still shows the old lease** | No tenant-facing lens projects `.tenancy` (leaseStart/leaseEnd/rentAmount/termStart); the application card shows the application-time terms and the Renewed card says "term ends" `cycleEnd` — the OLD end ([app.js:2874](../../../cmd/loftspace-app/web/app.js:2874)). Live: Jordan Ellis reads $2,050 · term ends 2026-09-06 while billed $2,125 through 2027-09-06. Project the term; the card states the current lease. | LoftSpace | pkg + FE | ★★ | S | 📋 ready |
| **A rent charge names no billing period or due date** | The debit entry is `{type, amountCents, postedAt}` ([scripts.go:578](../../../packages/loftspace-ledger/scripts.go:578)); the statement reads "rent $2,125 posted 09-13" with no month covered and no due date, though the reversal memo already names the period. Live: Jordan's 09-13 debit is Sep 6–Oct 6. `DebitAccount` carries `periodStart/periodEnd` (+ a due date) from the clause; the ledger and one-bill render them. | LoftSpace | pkg + FE | ★★ | S | 📋 ready |
...
- **Rotation to date:** LoftSpace ×33, Clinic ×32, Café ×23, Wellness ×20.
