...
| **[Refractor] A lens consumer's ack floor freezes, so its rebuild can never drain** | Live on `clinicProviders`: 678 ack-pending, floor pinned, `num_pending` 0, one DISTINCT message processed per 5m AckWait cycle — same cadence before and after the prefetch fix, so it is delivery starvation, not a redelivery storm. The pump hardening and the health signal shipped; the starvation itself is unexplained. | ★★ | S–M | 🏗️ building · [design](../../implementation-artifacts/lens-consumer-ack-window-design.md) §5b · next: why the 678 went un-acked |
