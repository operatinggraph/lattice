...
|---|---|---|---|---|
| **[Processor] The NFR-S6 tail: delete the release-quantum masking, equalize the work instead** | 277 lines of engine machinery mask a timing oracle whose cause is 18+15 early returns in identity-domain's own script, plus one Vault RPC a tombstoned `.claimKey` skips. Equalize the script, drop `Sensitive` from a sha256-of-256-bits, delete the floor. Net: −277 lines, −1 op-name map, −50ms per rejection, no new mechanism. | ★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/nfr-s6-release-quantum-payload-design.md) |
