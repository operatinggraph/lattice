...
| **[Loom/Weaver] A dispatcher cannot declare its op's class-(e) enumerations** | A `kv.Links` walk is declared through `ContextHint.Enumerations`, expressible by neither Loom `systemOp` submit nor Weaver `directOp` (`GapActionSpec` has no field). | ★ | XS–S | 🏗️ owner: claude/great-lamport-krohi2 · [brief](../../implementation-artifacts/erasure-orchestration-design.md) · next: Inc C1 Loom+Weaver field |
