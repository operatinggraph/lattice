...
| **[Refractor] Post-claim auth-grant latency is unbounded by design** | Auth-plane actorAggregate lenses (`capabilityRoles` incl.) intake the full broad Core-KV stream serially — ~2 events/sec through a ceremony burst — so grant visibility is queue-position luck, 5–20s observed, no upper bound. Facet's `isTransientAuthLag` retry papers over it client-side. | ★★ | L | 🏗️ 🔭 Andrew: §6.2 tie-rule edit staged UNCOMMITTED · [design](../../implementation-artifacts/auth-plane-projection-latency-design.md) · next: ratify, then Inc 2a before Inc 2 |
