...
| **[Processor] The confinement/authority-walk wall fix (row above) is café-only** | Measured: the walks are 2–8 listings; the wall was crossed by the per-listing unit cost on the Refractor's flood days (139 ms → timeout → 30 ms, same op + actor). No walk primitive: ships step-5 wall/read telemetry (log + `step5-latency` + summary arm), clinic's doubled `withProvider` listing, the re-measure at head. | ★★★ | S–M | 🏗️ building · owner: claude/serene-meitner-c3uqwk · [design](../../implementation-artifacts/authority-walk-wall-unit-cost-design.md) · next: Inc A + Inc B |
