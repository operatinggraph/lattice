...
| **[Refractor] A package upgrade's lens Output change is refused and the OLD shape keeps projecting** | Hot-reload swaps the adapter/rule but never re-installs the envelope, delete key or sweep plan ([reload.go:114](../../../cmd/refractor/reload.go:114)); no verb re-activates a live lens, and the upgrade reports success. Live: 3 lenses `fault`; `cafeStaleTabSettlement` rows still lack the `freshUntil` `b569fd2c` shipped. | ★★★ | M | 🏗️ building · [design](../../implementation-artifacts/lens-output-reactivation-design.md) · next: Inc 1 `TruncateForReactivation` |
