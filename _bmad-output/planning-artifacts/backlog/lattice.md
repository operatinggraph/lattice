...
| **[Refractor] edgeCatalog's role-branch merge fails closed for a dual-role actor** | A multi-hat actor whose 2+ roles each `grantedBy` the SAME op makes the role Walk itself yield 2 rows per op anchor with disagreeing `role` values; the merge refuses instead of picking one. Live continuous errors since (c) shipped. | ★★ | S–M | 📋 ready · found live, not (d)'s defect · [design §17](../../implementation-artifacts/refractor-shared-keyspace-arbitration-design.md) |
| **[Refractor] A lens spec change re-compiles but never re-projects** | `ClassifyUpdate` returns `MatchChange`, but `reload.go` only calls `UseFullEngine` — `Pipeline.Rebuild`'s one caller (operator control RPC) pkgmgr never invokes. A sweep-enrolled lens free-runs with a standing "Output descriptor changed — not hot-reloadable" error until Refractor restarts. Surfaced by wellness 0.13.0; live-recurring on `capabilityRead` (core) + 3 edge-manifest v0.14.12 grants, stuck ~4h. | ★★★ | M | 📋 ready · Lamplighter 2026-07-29 |
| **[Refractor] A live claim's own consumer grant never projects into Capability KV** | `ClaimIdentity`'s R2 grant lands in Core KV (verified) but `cap.roles.<target>` never appears — no DLQ, absent past a completed sweep. Repro `make test-claim-ceremony` (2/3 live runs, demo box). | ★★★ | M | 🏗️ building · zero-gap local repro is CLEAN (15/15) — needs real distributed timing · next: live Refractor instrumentation (Andrew sign-off), [design §13.7](../../implementation-artifacts/facet-staff-worlds-design.md) |
