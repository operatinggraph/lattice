...
| **[Weaver] A collapse-only reclaim books a phantom `__effect` dispatch** | `bumpEffectDispatch` fires on the sweep-reclaim path for every action (`evaluator.go`), yet the reconciler already treats assignTask/triggerLoom/proposedOp reclaims as phantom churn the consumer collapses (`reconciler.go`). A re-dispatch that is not a new attempt is booked as one, so any remediation outliving the 30m mark lease fills its window with pending entries nothing can close — a false LensEffectMismatch. Gate the reclaim booking on that same collapse-only predicate. | ★★★ | S | 📋 ready |
| **[Weaver] Sweep-won gap closes are never credited to the `__effect` window** | `recordEffectClose` fires only on the lane-1 paths (`evaluator.go`); the sweep drops it — `deleteMark` takes the mark's `action` but only logs it (`reconciler.go`). Any close the once-a-minute sweep wins over CDC goes uncredited, biasing every window toward zero closes and making false `LensEffectMismatch` warnings likelier platform-wide. Dispatch side is pinned by a test; no close-side counterpart. | ★★ | XS–S | 📋 ready |
