...
| **Script-read posture — declared+hydrated vs live `kv.get`/`kv.Links`** | Live Core-KV reads in scripts are the common root of the Loom-guard Processor-side redirect *and* the Edge A′-prediction partiality; evaluate all-reads-declared (`contextHint.reads`) + Processor-hydrated, live `kv.get` as deprecatable debt, `kv.Links` enumeration the irreducible hard case. | ★★ | L | 📋 needs design (umbrella — subsumes the Loom guardless-recovery redirect; surfaced by the Edge party-mode) |
