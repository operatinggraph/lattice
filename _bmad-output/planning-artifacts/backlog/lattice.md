...
|---|---|---|---|---|
| **[Processor] The NFR-S6 tail: equalize the timing where it is made, then delete what has no reason to exist** | The masking quantum hides 18+15 script early returns plus two engine gaps. One is cleanly equalizable (decrypt-and-discard the retained tombstone ciphertext — no declassification needed); the other is substrate-rooted and is not. Deletion of ~190 lines is gated on measuring the residue; the op-name map survives for the wire collapse. | ★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/nfr-s6-release-quantum-payload-design.md) |
