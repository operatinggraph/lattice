...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[Refractor/Weaver] Expiry is a fact on the CHECK, not a clock in the lens — sweep every `$now` lens** | `MarkExpired` only pokes the application's marker so the lens re-reads `$now`; a clock-function row gets no audit, no derivation, a corpus rescan per event. Direction (Andrew): the fired timer records `expiredAt` on the service instance; lenses count `completed AND expiredAt = null`; the application keeps its own lifecycle marker. 8 lens files. | ★★★ | L | 📐 needs designer pass · no-pattern: a fired freshness timer recording the lapse on the checked entity · Shape B fixed |
