...
| **🔭 Contract #2 — a descriptor's read disposition must be a floor the envelope cannot raise, edit uncommitted** | The disposition is a client choice today, so a hand-rolled `contextHint` naming a key under `reads` re-opens the `HydrationMiss` existence oracle on any anti-enumeration op. Extends *weakest wins* from derived-vs-envelope to descriptor-vs-envelope. Demonstrated live as an ordinary consumer. | ★★★ | S | 🔭 flag-for-Andrew · edit staged uncommitted in `main` · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §19.5 |
| **🔭 Contract #3 — soft-deleted sensitive-aspect read disposition, edit uncommitted** | `docs/contracts/03-mutation-batch-event-list.md` §3.10 was silent on how hydrate treats a tombstoned sensitive aspect; the two dispositions now differ (deliver-scrubbed vs refuse) and are documented nowhere frozen. Built and live; the edit awaits Andrew. | ★★ | XS | 🔭 flag-for-Andrew · edit staged uncommitted in `main` |
