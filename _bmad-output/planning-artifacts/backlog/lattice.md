...
| **[Weaver] A declined violating row is Acked once and never revisited** | Never-dispatched gaps invisible forever (26/28 `clinicSiteBackfill` — blocks the verticals clinic row), a config latch that can't retire, false contraction restart-replay claim. Row-sweep design HELD by Andrew: no new enumerator on the durables. | ★★★ | M | 📐 needs designer pass · no-pattern: substrate-native decline retry (Nak'd declines + per-boot lane-1 replay) · [held design + direction](../../implementation-artifacts/weaver-sweep-declared-work-enumeration-design.md) |
