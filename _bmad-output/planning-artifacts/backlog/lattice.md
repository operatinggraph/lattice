...
| **[Weaver] `gapSuppressed`'s cap-fallback shares the action-based external-gap test** | `evaluator.go:896` decides the default-budget fallback by `action != actionDirectOp`, the same narrowing that broke `staleMark` for a `triggerLoom` externalTask gap. Inert today — the lease gaps declare `maxretries_<g>`, so the fallback never fires. | ★ | XS–S | 📋 ready · steward-owned · next run · consumer: the first external gap declaring no `maxretries_<g>` |
| **[Tooling] The lease-convergence harness discards every engine log** | `harness_test.go:192` gives Processor/Loom/Weaver/Refractor an `io.Discard` logger, so an `error`-severity `InflightActionMismatch` fired on every run of this suite and no run could show it. | ★ | XS | 📋 ready · steward-owned · next run · consumer: the next engine defect this suite hides |
