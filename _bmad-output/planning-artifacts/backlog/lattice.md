...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[Refractor/Weaver] Expiry is a fact on the CHECK, not a clock in the lens — sweep every `$now` lens** | `MarkExpired` pokes the application's `freshnessExpiry` marker and the lens re-reads `$now`: a clock-function row gets no audit, no derivation, a corpus rescan per neighbour event. Direction (Andrew, 2026-09-01): the fired timer records `expiredAt` on the service instance's outcome; lenses count `completed AND expiredAt = null`; the application keeps its own lifecycle marker. Sweep the 8 `$now` lens files. | ★★★ | L | 📐 needs designer pass · no-pattern: a fired freshness timer that records the lapse on the checked entity · direction fixed: Shape B |
