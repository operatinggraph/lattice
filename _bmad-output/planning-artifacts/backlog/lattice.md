...
| **[CI] 62 test files still dial an embedded server with a bare `nats.Connect`** | The `natsserver.RunServer` / `testutil.StartEmbeddedNATS` fixture family returns a URL its callers dial bare, so each keeps nats.go's single-shot 2s whole-handshake deadline — the same host-stall exposure `internal/natsfixture` closed for the 42 `NewServer` sites. Route them through `natsfixture.Connect`, then widen the lint from the constructor to the connect. | ★★ | M | 🏗️ building · [mechanism](../../../internal/natsfixture/natsfixture.go) · next: annotate deliberate reject-vector connects |
