...
| **[Refractor] Two clause shapes the full engine accepts and silently miscompiles** | An **anonymous** required `MATCH` binding no NEW variable is dropped rather than filtered; the named-relationship form now filters correctly. `WITH *` reaches the AST as an empty projection list, so `projectItems` rebuilds each row empty and a later pattern re-seeds by whole-bucket scan. | ★★ | XS–S | 🏗️ building · [design](../../implementation-artifacts/lattice-backlog-audit-2026-08-20.md) (fire brief 2026-08-21) · next: parse refusals + tests |
