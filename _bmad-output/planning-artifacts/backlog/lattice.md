...
| **[Pkgmgr] An uninstalled package cannot be reinstalled — only refused** | Reinstall is now a loud refusal naming the occupants and what restores a grant, but the keys themselves stay dead: reviving them needs step 8's package-scope guard to resolve a tombstoned manifest, and Contract #8 §8.2's create-only guardrail with it. | ★★ | M | 📐 needs designer pass · no-pattern: package-revive ownership scope for a tombstoned manifest · [options](../../implementation-artifacts/reinstall-over-uninstall-occupancy-design.md) §5 |
| **[Pkgmgr] The install path lists the whole Core KV bucket twice, per operation** | `checkCoreBucketExists` lists every key to learn the stream exists, then discards it; `findInstalledPackage` lists it again plus a serial `KVGet` per manifest. Both walk the `vtx.op.*` churn namespace against a 45-60s deadline, on every Install/Upgrade/Apply/Uninstall. | ★★ | S | 📋 ready · mirror `readMetaDocs`; existence wants stream-info, not a listing · [why](../../implementation-artifacts/reinstall-over-uninstall-occupancy-design.md) §6 |
...
| **[Pkgmgr] Uninstall erases the same secure-lens history with no attestation** | `Uninstall` tombstones a Secure Lens `.spec`, so `registry_probe` stops seeing holder types whose ciphertext still stands in the target store — the destruction oracle attests coverage it lacks. Upgrade/Apply refuse this; uninstall does not. | ★★ | S | 📋 ready · build from scratch (the prior fire's branch is gone — remote containers are ephemeral) · operator-attested `UninstallOptions`, never the Definition (that would disarm the R3 upgrade gate) |
| **[Edge] The first-paint gate has no identity for the hydrate cycle it gates** | Two defects: `hydrationComplete` carries no `Lens`, so the gate releases on the first marker and can paint a partial world; and `personal.hydrate` is per-**identity**, so a second device's burst satisfies this device's gate. The client-side fix was built and refuted. | ★★ | M | ✅ ratified (Winston-adjudicated) · [design](../../implementation-artifacts/edge-first-paint-gate-identity-design.md) |
...
| **[bootstrap] A package-plane actor can forge a package-origin permission and grant it to itself** | `origin:"package"` is client-supplied and buys the Contract #6 reserved-set exemption; nothing ties a created `vtx.permission.*` to authority the submitter may confer. Reachable from `UpgradePackage`'s create arm and from any package-authored DDL script. Owns the `grantedBy`-revival gap. | ★★★ | L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/package-authority-minting-provenance-design.md) |
| **[natsperm] The `capability-author-context` catalog bucket is app-tier readable** | NL-1's context-lens widening projects lens `.spec` bodies (incl. RLS posture) into a bucket every app-tier `$JS.API.>` holder can read. The vendor-egress half is closed adapter-side; restricting the bucket read needs new machinery + a loupe-charter carve-out. | ★★ | M | 📐 needs designer pass · no-pattern: read-restricted package bucket + loupe-charter carve-out (Andrew) · [why](../../implementation-artifacts/natural-language-weaver-targets-design.md) §For-Andrew |
...
| **[capability-author] One authoring request cannot co-propose a NEW lens + the target that binds it** | `RecordCapabilityProposal` records one `{kind,content}` per request and single-artifact apply resolves `lensRef` only by NanoID or same-Definition name — so a new-lens-plus-target intent can't produce both atomically. NL v1 binds an existing lens instead. | ★★ | M | 📐 needs designer pass · no-pattern: multi-artifact proposal + cross-artifact id resolution · [why](../../implementation-artifacts/natural-language-weaver-targets-design.md) §1 |
...
| **Suite reddens under parallel load, in packages the change never touched** | Rotating membership across unit-1, unit-2 and the convergence job. FOUR mechanisms: (a) `substrate.Connect`'s 2s no-retry handshake, ~45 sites remain; (b) a starvation signature (`found=map[]`); (c) a wall-clock DEADLINE read as correctness; (d) a `privacy-pii-key-envelopes` key-not-found race in `internal/leaseconvergence` — now the dominant signature, predates the CI-speed fires. | ★★★ | M | 🏗️ owner: Whetstone · next: root-cause (d) |
| **Substrate backfill test's live handler races its own ack-floor pin** | `TestBackfillConsumerInactiveThreshold_PreservesDeliveryPositionAndAckFloor`: the consume handler stays attached across the write-back, redelivery resumes at `OptStartSeq=3`, the handler acks 3–4, and `AckFloor` advances past the `==` snapshot pin ([test:160](../../../internal/substrate/backfill_inactive_threshold_test.go)) — CI run 32452945651 read `4, want 2`. Fix: stop/drain the handler before the before-snapshot; keep `==`. | ★ | XS | 📋 ready · owner: Whetstone · tighten, never loosen |
