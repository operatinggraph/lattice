...
| **[Loom] `loom-state` accretes a terminal `instance.<id>` forever, and three paths enumerate the whole bucket** | The cursor is permanent by design; the live harm is enumeration: `ListInstances` is 12× past `KVGetMulti`'s 1,024-subject fast path and hits the 1 MB create-consumer wall near 22k instances. Fix: subject-filtered listings, a `failed` index, completed history from the read model. | ★★ | M | 🏗️ building · owner: claude/relaxed-rubin-q8w7ug · [design](../../implementation-artifacts/loom-instance-enumeration-bounding-design.md) · next: Inc 1 filtered pin listings |
