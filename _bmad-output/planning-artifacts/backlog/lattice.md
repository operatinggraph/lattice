...
| **[identity-domain] A claim rejection's LATENCY still separates already-claimed from wrong-key** | A tombstoned `.claimKey` skips the piiKey KVGet and the AEAD decrypt, so the rejection causes may differ in time even though the wire shape does not. | ★ | S | 📋 ready · measured: no p50 gap, direction inverted — re-measure concurrent before building any floor · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §19.5 |
