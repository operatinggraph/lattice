...
| **[Refractor] Streaming aggregation — the executor materializes the full binding set** | Live pprof at 8.4 GB heap: 6.3 GB under `projectItems→evalAggregatorArgs→evalExpr` + 0.65 GB `cloneBinding` — every binding row and aggregator arg held before `finalizeAggregator` reduces; count/collect over a large MATCH costs GBs per evaluation (took a dev host past 20 GB). Fold incrementally per row + a loud binding-count guard; also bound graceful shutdown, which a monster evaluation wedges past SIGTERM. | ★★★ | M | 📋 ready |
