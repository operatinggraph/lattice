...
| **[Pkgmgr] A package upgrade silently un-tombstones a deliberately revoked grant** | The surviving-key branch emits an `update` (`upgrade.go:292-332`) and step 8's update arm has no aliveness guard (`step8_commit.go:407-447`), so a tombstone is overwritten alive. Keys are version-independent, so any reinstall revives what `RevokePermission` retracted. | ★★ | S–M | 📋 ready · blocks [grant-provenance](../../implementation-artifacts/grant-provenance-runtime-permission-minting-design.md) Inc 1 |
...
| **[Pkgmgr] No live-vs-declared reconciliation for permission vertices** | `VerifyAgainstDefinition` compares manifest-to-Go, never Core KV, so a permission vertex no manifest declares is invisible to every gate. Conditional: the trigger is any future runtime grant channel — with the channel closed the installer is the only writer and a reconciler has no drift to catch. | ★ | S | 📋 ready · seq: behind any runtime grant channel · [why](../../implementation-artifacts/grant-provenance-runtime-permission-minting-design.md) §1.1 |
...
| **[Refractor] A plain lens's neighbour event recomputes its whole row set** | A non-anchor endpoint seeds nothing, so the executor prefix-scans every anchor candidate: N+1 bindings per event, N = the anchor population (1,325 events wedged `clinicProviders`). Needs a scan-root terminus on `HopIndex` plus a standing healer; ≤36 of 60 plain lenses addressable. | ★★ | L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/plain-lens-neighbour-anchor-derivation-design.md) · fork: is detection a sufficient licence to narrow |
...
| **[Refractor] A personal row goes stale when its own D1 grant flips** | `personalEnvelopeFn` gates each row on `cap-read.*`, which lives in Capability KV, not Core KV — so no CDC event reaches the personal pipeline when a grant lands or retracts, and personal lenses get no sweep plan. A revoked grant leaving a row live is the over-grant direction. | ★★ | M | 🏗️ designing (2026-08-11) · next: ground the cap-read trigger seam |
...
| **[Edge] An orphan a purge cannot reap has no server-side backstop** | A revoked credential fails the sign-out reap (the auth callout refuses its connection) and a crashed host never reaps — each strands a durable no client can name again. `InactiveThreshold` is the shape; the shell's 30 min will not port. | ★★ | S–M | 🏗️ building · [design](../../implementation-artifacts/edge-sync-orphan-expiry-design.md) · next: Inc 1 stream policy |
...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[rbac] A second grant channel mints capability grants outside the package plane** | `CreatePermission`/`UpdatePermission`/`GrantPermission` are `scope:any` to operator with no allow-list, so a grant no manifest records and no uninstall retracts is indistinguishable at step 3 from a package one — reaching `ShredRetentionClassKey`, the one shred verb shipping no grant. Fix is provenance, not a deny-list. | ★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/grant-provenance-runtime-permission-minting-design.md) · fork: may a non-package op be grantable |
