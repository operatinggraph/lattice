...
| **[Refractor] `edgeManifestReadGrants` builds a >1M-row cross product per event** | The new binding cap named the lens behind the 8.4 GB profile: rule `RKF4MBBKaEEPktjxRKF4` (edge-manifest read-grant producer) passes 1,000,001 bindings on a single `lnk.session.*.atLocation.*` event and is now refused + redelivered, where before it silently consumed GBs. Its read-grants stop refreshing while refused. Likely an under-anchored walk whose OPTIONAL branches cross-product. Untested hypothesis: the per-row fold may make it viable at a raised cap — measure on a quiet host before re-defaulting. | ★★★ | M | 📋 ready · live since `5527e0e2` |
| **[Refractor] The executor still materializes the whole binding set** | The aggregate half now folds per row, but `applyMatch`/`matchPath` still build the full `[]binding` cross product — measured ~75 MB per 102k rows, so the 1M-row cap it now runs under permits ~730 MB. Consumer: the first legitimate lens the cap refuses. Streaming the binding set (lazy expansion, or push-down into the fold) is the design that removes the ceiling rather than bounding it. | ★★ | L | 📋 designer · residual of `5527e0e2` |
...
- *(older entries rolled to [archive/lattice-done.md](archive/lattice-done.md); newest rolled entry `bcf52101`)*
