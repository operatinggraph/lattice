...
| **[Refractor] A tail expansion from a walk anchor binds in-memory but not in the live projection** | `(op)<-[:permitsOperation]-(psvc)` + collect yields the service list in the ruleengine test store yet null/[] live (spec current in KV, refractor cycled, cold hydrate) — while forward tail hops (`availableAt`, `appliesToUnit`) bind live. Likely the projection loader's subgraph scope excludes vertices no walk reached. Ground the loader, then close with a positive live vector. | ★★ | M | 📋 ready |
