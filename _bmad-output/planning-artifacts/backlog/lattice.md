...
| **[Refractor] Lens health is liveness-only — a frozen row renders green** | 12 orphanedTaskGrants rows sat 12 days stale while the lens card showed green: status/lag/error health cannot see per-row wrongness, and the only live catcher was Weaver's LensEffectMismatch. A sampled projected-vs-recomputed row audit (sweep-side divergence counter surfaced to Health KV) is the missing signal. | ★★ | M | ✅ ratified 2026-08-06 (Winston, delegated) · [design](../../implementation-artifacts/lens-projection-divergence-audit-design.md) · Fire 1 = one fire with sweep-supersession |
...
| **[Refractor] A sweep write after a rule swap's truncate locks the rebuild out of the key** | `Reproject` never checks `supersededRule` (unlike `writeResults`) and stamps the consumer head, so an old-rule sweep write after a reload's truncate recreates the key at watermark H — the ≤-guard then rejects the whole replay beneath it: a revoking MATCH edit silently defeated, reported healed. | ★★ | S | ✅ ratified 2026-08-06 (Winston, delegated) · [design](../../implementation-artifacts/sweep-rule-snapshot-granularity-design.md) · one fire with divergence-audit Fire 1 |
