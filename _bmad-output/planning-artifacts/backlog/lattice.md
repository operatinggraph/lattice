...
| **[Refractor] DiffRetraction silently no-ops on a GrantTable target** | `GrantWriterAdapter` is not a `KeyLister`, and `applyDiffRetraction` returns unchanged when the adapter isn't one — a grant lens opting into DiffRetraction NEVER retracts, silently. Proven live: after an unwire the grant survived and the ex-actor still read the protected row. Needs a `grant_source`-scoped `ListKeys` + fail-closed on the mis-config. | ★★★ | M | 📋 ready · blocks [staff-worlds F3](../../implementation-artifacts/facet-staff-worlds-design.md) grant producer |
| **[Processor] A script `update` wipes the immutable create provenance** | Step 8 re-stamps only `lastModified*` on an update, and `VertexDoc` never exposes `createdAt`/`createdBy`/`createdByOp` — so no Starlark script CAN carry them forward and every script `update` drops all three (Contract #1 §1.3 marks them immutable). `pkgmgr/upgrade.go` works around it client-side; packages cannot. | ★★ | S–M | 📋 ready · found fixing [service-location] Wire revive |
