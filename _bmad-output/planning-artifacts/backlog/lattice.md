...
| **[bootstrap] `UpgradePackage` accepts unvalidated mutations against permission/role vertex classes** | No DDL registers canonicalName `permission`/`role`, so step 6 validation is skipped for those classes and `UpgradePackageDDLScript` full-body-replaces any `vtx.*` past a shape check alone. `consoleOperator` ("does not confer root") holds it at the meta lane — a live, non-root escalation. | ★★★ | S–M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/permission-role-provenance-write-once-design.md) |
...
| **[bootstrap] `UpgradePackage`/`UninstallPackage` mutations aren't scoped to the named package's own manifest** | Neither script verifies a submitted update/tombstone's key belongs to the named package's committed `.manifest.declaredKeys` — any holder can tombstone or (for non-permission/role classes) rewrite any non-protected key while naming an unrelated package. | ★★ | M | 📐 needs designer pass · found 2026-08-14 · [why](../../implementation-artifacts/permission-role-provenance-write-once-design.md) §8(b) |
