...
| **[Pkgmgr] An op-meta tombstone orphans the open tasks that reference it** | A package move tombstoned `RecordIdentityPII`'s op-meta while an open task still linked it `forOperation` — the task becomes an undispatchable zombie (lens projects null, descriptor path dead). No uninstall/upgrade guard covers open referents; needs a designer call: block, migrate links, or auto-cancel. Live repro healed via operator CancelTask. | ★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/opmeta-retirement-open-task-guard-design.md) |
| **[orchestration-base] A closed task's ephemeral grant stays exercisable until expiry** | `capabilityEphemeral`'s three branches filter only `expiresAt > $now` (lenses.go:284-308) and step-3 matches taskKey+opType+target+expiry — status never checked — so CancelTask/CompleteTask do not revoke the grant; a cancelled task's op stays submittable until `expiresAt`. Lens-side `status='open'` filter is the likely shape (myTasks already has it). Surfaced by the op-meta guard design's adversarial pass. | ★★ | S | 📋 ready |
| **[Refractor] Two lenses sharing one IntoKey race per column** | `edgeCatalog` (service path, `viaServices`) and `edgeCatalogRoles` (role path, `viaRole`) both project `manifest.op.<id>`; last writer wins per re-derivation, so a multi-hat actor's op rows flap between column shapes and the service modal blanks. Needs a designed merge/ownership rule for shared row keys. Mitigation shipped: both tails project `viaServices` (edge-manifest 0.14.4). | ★★ | M | 📋 ready |
