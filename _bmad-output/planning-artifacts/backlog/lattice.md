...
| **[Bridge/Processor] Op-status read surface — `lattice.op.status` responder** | Processor-hosted op-status RPC (vault.decrypt pattern); Fires 3–4: Loom probe migration (+§10.6 reconciliation), CLI — the gate for matrix-wide read tightening. | ★★★ | S | 🏗️ building · [design](../../implementation-artifacts/op-status-read-surface-design.md) · next: Fire 3 Loom probe migration |
