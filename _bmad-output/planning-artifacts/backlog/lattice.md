...
| **[Weaver] The `data:` latch is read-driven, so entries strand and `planGap`'s log cannot be damped** | `issueKeyDataEntity`'s clears are keyed to reads, not to facts: a column whose read stops (gap close) strands its entry, and the gap column's own read erases `planGap`'s template fact, re-stamping `since` and defeating arrival-vs-repeat damping. | ★★ | M | 🏗️ owner: `claude/great-lamport-c6lifp` · [design](../../implementation-artifacts/weaver-data-latch-fact-pairing-design.md) · next: Inc 1 fact-scoped template key |
