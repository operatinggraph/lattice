...
| **[Gateway] revocation materializer poison-pill (invalid actor key)** | `RevokeActor` checks only the `vtx.identity.` prefix, so an actor id outside the NATS-KV charset commits + publishes; the materializer's `KVPut` errors → Nak, and with no `MaxDeliver` the event redelivers forever — one bad submit stalls kill-switch sync. Tighten `required_actor` to the NanoID charset AND Ack-and-report an unputtable key. | ★★ | S | 📋 · found by loupe F11 review · `packages/identity-domain/revocation.go` + `internal/gateway/revocation_materializer.go` |
...
| **Weaver planner mandate (dispatcher → solver)** | Remediation stops being a static gap→action lookup: deterministic planner (per-gap candidate selection, then goal-regression synthesis over op-declared effects) executed as content-addressed pinned Loom patterns, plus contraction/oscillation diagnostics and fleet admission control; shadow mode + per-target cutover; the Augur stays the AI boundary. | ★★★ | XL | 🏗️ building · [design](../../implementation-artifacts/weaver-planner-mandate-design.md) · Fires 1-2 done; next: Fire 3 (`internal/weaver/planner` pure library) |
