...
| **[Refractor] A personal row goes stale when its own D1 grant flips** | `personalEnvelopeFn` gates each row on `cap-read.*`, which lives in Capability KV, not Core KV — so no CDC event reaches the personal pipeline when a grant lands or retracts, and personal lenses get no sweep plan. A revoked grant leaving a row live is the over-grant direction. | ★★ | M | 🏗️ building · [design](../../implementation-artifacts/personal-lens-grant-change-trigger-design.md) §14 · next: Inc 1 steps 1-9 |
