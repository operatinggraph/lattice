...
| **contract-wire-error-code-reconciliation** | Contract #2 §2.6's error-code table diverged from the wire both ways — ~7 listed codes are never emitted (Starlark*/schema/scope/sensitivity/MetaLaneCollision; CellMoved deferred) and 6 emitted codes are unlisted (HydrationFailed/ScriptFailed/DDLViolation/ProtectedKey/AuthInfrastructureFailure/ClaimKeyInvalid); §2.9 also overclaims unknown-field rejection vs the lenient ParseEnvelope. Reconcile to the real closed enum (uncommitted edit for Andrew) + a conformance test reading the table. | ★★ | S | 📋 |
...
| **contract7-and-processor-mandate-refresh** | Contract #7 §7.2/§7.7 describe a superseded kernel (5 meta-meta DDLs, processor identity, topology-walk cypher) — stage the alignment edit (→ literal `data.protected` grant + the `cap.<actor>`/`cap.roles.<actor>` split) for Andrew. processor.md/doc.go omit step-6.5 encryption, the §3.2 OCC retry, task auto-completion, and kv.Links; commit_path.go keeps "stubbed 4-10"/"auth (stub)" comments. | ★ | S | 📋 |
...
| **Weaver planner mandate (dispatcher → solver)** | Remediation stops being a static gap→action lookup: deterministic planner (per-gap candidate selection, then goal-regression synthesis over op-declared effects) with contraction/oscillation diagnostics and admission control; shadow mode + per-target cutover; the Augur stays the AI boundary. | ★★★ | XL | 🏗️ building · [design](../../implementation-artifacts/weaver-planner-mandate-design.md) · building: [renewal](../../implementation-artifacts/loftspace-lease-renewal-goal-authored-target-design.md) R2 package |
