...
| **[privacy-base] A pre-narrowing shredded subject earns a clean attestation over live `credentialindex` rows** | That shred tombstoned `boundTo` and left each `vtx.credentialindex.<hash>` standing, so the sweep finds no live link, all five residue arms read zero, and the seal attests `violating=false` over N live rows. | ★★ | S | 📋 ready · CLI + narrow tombstone op; precedent `cmd/lattice/identity/reconcile.go:153`; population capped (producer deleted `54b3c8c7`) · [why](../../implementation-artifacts/erasure-orchestration-design.md) inc 9 |
