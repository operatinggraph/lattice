...
| **[bridge] Egress-unwrap serves identity-custodied sensitive aspects only — a retention-class holder's `$sensitiveRef` permanently fails** | `resolveSensitiveRef` (`egress.go:190`) hard-rejects any non-identity holder; `piiKeyEnvelope` is `MATCH (i:identity)` only. A retention-class-custodied snapshot declares fine Processor-side but never reaches a vendor. Blocks verticals.md's lease-tenant-name row. | ★★ | S–M | 📐 needs designer pass · no-pattern: retention-class-holder envelope lookup for bridge egress · also a privacy call on erasure-surviving PII reaching a vendor |
...
| **[Refractor] A `WITH` refuses per-anchor closure wholesale, so three plain lenses lose narrowing AND retraction** | `anchorProjectionShape` refuses a `WITH` before reading a key column: `leaseApplicationsRead` rescans the corpus per event (~64k pending, ~1 msg/min), it and `renewalsRead` retract nothing at all, `clinicPatientsRead` pays DiffRetraction. | ★★★ | M | ✅ ratified 2026-09-02 (Andrew: build as designed) · [design](../../implementation-artifacts/with-alias-anchor-closure-design.md) |
...
| **CI pipeline speed (continuous)** | Make CI faster without weakening any gate — owned continuously by the **Whetstone**. Eleven parallel jobs; unit sharded 4 ways by measured `go test` time, `internal/natsperm` now its own job (was a unit-4 step). | ★★ | M (ongoing) | 🏗️ continuous (Whetstone) · unit-1 pole cut 277s→170s (ad15f91); natsperm→own job (b558d16), still unproven — every cloud-session push to main triggers zero CI (0 check-runs/statuses; confirmed on `b153d120` and again `b558d163`, 2026-09-03), local/Winston pushes unaffected. Root cause: cloud sessions push via a session-bound OAuth App credential, a separate GitHub identity from the installed "claude" GitHub App the 2026-08-09 fix granted write on (independent identities — [anthropics/claude-code#61189](https://github.com/anthropics/claude-code/issues/61189)); operatinggraph's org-level OAuth App access policy is the leading suspect and isn't readable from any session · next (Andrew): check Settings → Third-party access → OAuth App policy on operatinggraph, approve Anthropic's OAuth App if restricted · then measure |
