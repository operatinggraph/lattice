...
| **[Bootstrap] A re-bootstrap strands the prior epoch's operator grants** | Regenerating `lattice.bootstrap.json` mints a NEW `operator` role; the prior epoch stays live but unreachable. Live: 40 permissions no service actor can reach — all 3 verticals' ledger `CreateAccount`/`DebitAccount`, `AttachObject`, the shred plane. LoftSpace- + Café-requested. | ★★★ | S–M | ✅ ratified (edge revocation + lens retirement + grant re-mint via package plane) · [design](../../implementation-artifacts/primordial-epoch-stranded-authority-design.md) |
