...
| **[Loupe] A `newPackage` proposal is closed over a same-named package it never wrote** | Live (the Studio path is ungated); one ambiguous name+version heuristic decides both the apply 409 and the mark-applied recovery. No primitive missing — the durable install receipt is computed and discarded. | ★★ | M | 🏗️ owner: claude/great-lamport-tasw4c · [design](../../implementation-artifacts/capability-proposal-install-receipt-design.md) · next: Inc 1 thread reply through ApplyResult |
...
| **[Weaver] `GapWithoutPlaybook` long-Naks a column no playbook will EVER name** | `lease-signing` projects `missing_decision`/`missing_manager` into `violating` with no gaps entry, deliberately; nothing ever closes `missing_manager`. Such a row holds a pending slot and re-runs the preamble every floor forever — §4.2's argument for the unregistered-target exit, never applied to row 8. Fix: declare orphan columns `surface` + gate lens `missing_*` ⊆ gaps. | ★★★ | M | 📋 ready · [why](../../implementation-artifacts/weaver-decline-retry-substrate-native-design.md) §3.2 |
