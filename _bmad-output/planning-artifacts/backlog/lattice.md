...
| **[Bootstrap] operator-grant restoration never run — `b153d120` shipped only detection + a manual CLI** | Shipped revocation tooling, not auto-reinstall; its design names `cafe-ledger`/`objects-base`/`privacy-operator-grant`/`identity-domain` as needing a manual reinstall, never run. Live 2026-08-27: LoftSpace `AttachObject` + café `CreateAccount` still `AuthDenied`. | ★★ | XS | 📋 ready · run the documented reinstall for the 4 named packages · [why](../../implementation-artifacts/primordial-epoch-stranded-authority-design.md) |
