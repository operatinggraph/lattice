...
| **[Refractor] A MATCH hot-reload rewrites `plainReprojectLabels` unsynchronized against the consumer** | `useFullEngineBranches` reassigns the map + `plainReprojectAll` — and now `plainReprojectRelations` + `plainRelationsExhaustive` — while `handle` reads them, no lock. The label pair fails closed; the relation pair does not, since `exhaustive` against a nil map skips every link. | ★★ | S | 🏗️ building · [design](../../implementation-artifacts/lens-trigger-relation-narrowing-design.md) §8 · next: adversarial review then merge |
