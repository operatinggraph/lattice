...
| **[Tooling] The history-comment gate misses "this fire"-shaped narration** | CLAUDE.md's most-violated rule, unscreened: 70 live sites / 58 files / 13 packages. Flipping `lint-conventions` and sweeping must land together, and the sweep first needs `lint-package-version`'s comment-only exemption (`commentOnlyGoChange:314`, wired only to the walk-generator branch) extended to in-package files — else 13 packages republish for comment edits. | ★ | M | 📋 ready · precedent: `importOnly`/`commentOnlyGoChange` · surfaced repairing one site in `supervisor_adapt.go` |
