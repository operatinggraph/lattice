...
| **[Loom/Substrate] `loom-state`'s delete tombstones are five sixths of its growth — sweep them** | Every deleted sub-key leaves a permanent tombstone subject — 74k subjects for 12k instances — and nothing purges deletes. A purge sweep collects only delete entries, so the cursor's dedup guard is untouched. Andrew, 2026-09-02: the cursor stays, the tombstones go. | ★★ | M | 🏗️ building · owner: steward-lattice-loom-tombstones · [design](../../implementation-artifacts/loom-state-tombstone-sweep-design.md) · next: brief, then Inc 1 |
| **[Weaver] One per-target issue budget is shared by the WORKLOAD and FAULT populations** | `rowIssueCapPerTarget`=500 spans `gap:`/`data:`/`template:`/`sweep:`; a slot frees per cleared entry, so a static `surface` backlog of 500 open tasks holds the target at cap and every later fault is refused, and a refusal inverts two log seams. Fix: one counted entry per (target, gap column). | ★★ | M | ✅ ratified 2026-09-02 (Andrew: take the trade; contract line lands with the fire) · [design](../../implementation-artifacts/weaver-surface-workload-vs-fault-issues-design.md) |
...
| **[Processor] A documentless tombstone skips every DDL-driven write gate, and no link is ever protected** | Step 6 derives class only from the mutation's own document, so a bare tombstone skips permittedCommands, abstract and custody; step 8 protects no link key. Any package script can tombstone any unprotected root, aspect or link of any package; the corpus relies on it. | ★★★ | M | ✅ ratified · [design](../../implementation-artifacts/stored-class-write-gate-and-kernel-topology-protection-design.md) |
| **[Loom] A re-delivered step-deadline marker fails a parked instance after 24 h** | The probe reads rejected-or-lost from two absences (the 24 h tracker, the outbox) while the wait it backstops is unbounded; a `loom-deadline` durable rebuilt from `DeliverAll` replays every disarmed running instance's marker and fails it. | ★★★ | M | 📐 needs designer pass · no-pattern: an armed/disarmed deadline state on the instance record the probe can read · [why](../../implementation-artifacts/loom-state-tombstone-sweep-design.md) §11.2 |
| **[Bootstrap/Loom] The 1 s marker TTL is the delivery window of every TTL-expiry signal** | `LimitMarkerTTL=1s` on all six `PerKeyTTL` buckets means a `deadline.*` expiry (Loom's only rejected/lost-step signal) and a tracker expiry exist for one second; a consumer down across that second never sees it and no startup scan notices. Decide the value per bucket. | ★★★ | S | 📋 ready · [why](../../implementation-artifacts/loom-state-tombstone-sweep-design.md) §11 |
...
| **[Weaver] An exhausted gap never re-plans, and re-escalates every 30 minutes forever** | Live `renewalComplete`/`vtx.renewal.QomdjY7hAGS6mHvN9d2j`: the gap exhausted 2026-08-29, a day before the terms landed that made `signRenewal`'s `pre` satisfiable — and it never re-planned, re-submitting `CreateAugurReasoningClaim` every ~30 min instead (265 dispatches vs budget 6, zero observed closes); its one proposal still reads `review.state: pending`. | ★★★ | S–M | 📋 ready · LoftSpace PO 2026-09-04 · blocks a [verticals.md](verticals.md) renewal row |
...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[Refractor/Processor] `capabilityEphemeral`'s `$now` is a set-inclusion filter with no fact to record** | It admits a task's grants only while `expiresAt > $now`; dropping the filter projects grants for every open-but-expired task, so `cap.ephemeral.<actor>` grows without bound on the auth plane. Safe today: the Processor re-checks `expiresAt`. | ★★ | S | 📐 needs designer pass · no-pattern: a recorded expiry fact on a task readable by an auth-plane inclusion filter · [why](../../implementation-artifacts/expiry-as-a-recorded-fact-design.md) §2.6 |
| **[Refractor] A Secure plain lens has no orphan healer** | A row orphaned by a missed or failed retraction event is revisited by nothing: the audit refuses a lens with a secure decryptor, the sweep enrols auth-plane actor-aggregate lenses only, and the anchor Delete never re-reads the target. `renewalsRead` today; `clinicPatientsRead` keeps `DiffRetraction` as its healer. | ★★ | M | 📐 needs designer pass · no-pattern: an off-request orphan healer for a Secure plain lens · [why](../../implementation-artifacts/with-alias-anchor-closure-design.md) §13 Inc 3 |
