...
| **[Refractor] Full-engine phantom aggregate row on empty required-MATCH scan** | An unanchored full-scan lens with an aggregating WITH (e.g. `landlordLeaseApplicationsRead`'s `qualified` clone) fabricates one all-null row when its required MATCH binds zero rows, instead of zero rows — null `authz_anchors` fails the protected adapter's non-null check on every CDC touch stack-wide; row never projects even once real data exists. | ★★★ | S–M | 📋 · `internal/refractor/ruleengine/full/executor.go:797` (`projectItems`) |
