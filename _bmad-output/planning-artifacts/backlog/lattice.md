...
| **[Processor] §2.5's floor silently skips the key templates it cannot resolve** | `resolveDescriptorFloor` applies no floor for `{me.*}`/`{entity.*}` and only logs Warn, so those keys stay hardenable by a hand-rolled `contextHint`. The clause reads unconditional; enforcement is not. Live: cafe-domain's `Charge`/`Settle` declare exactly that shape. | ★★ | M | 📐 needs designer pass · no-pattern: `{me.*}`/`{entity.*}` floor-resolution semantics (spike shape-match first, may collapse M→S) · consumer LIVE: cafe `Charge`/`Settle` |
| **[Refractor] Two clause shapes the full engine accepts and silently miscompiles** | An **anonymous** required `MATCH` binding no NEW variable is dropped rather than filtered; the named-relationship form now filters correctly. `WITH *` reaches the AST as an empty projection list, so `projectItems` rebuilds each row empty and a later pattern re-seeds by whole-bucket scan. | ★★ | XS–S | 📋 ready · refuse-at-parse; 0 live lenses (full census incl. generated); mirrors the 4×-shipped `v.fail()` idiom · [why](../../implementation-artifacts/lattice-backlog-audit-2026-08-20.md) |
...
| **[Pkgmgr] A manifest edit evades the §29 destruction oracle — dropped column or renamed lens** | `widenSecureColumnsForUpdate` only widens a still-present narrowed `holderTypes`: deleting the `SecureColumn` entry (zero widen; `mayHoldHolderType` reads no columns as a genuine no) OR renaming the lens canonicalName (fresh historyless key, old ciphertext stands) both skip it. | ★★ | S | 📋 ready · author-declares-intent, fail-closed; Fire 28 carve-out precedent · [why](../../implementation-artifacts/retention-class-key-custody-design.md) §29 R2/R3 |
| **[Edge] The first-paint gate has no identity for the hydrate cycle it gates** | Two defects: `hydrationComplete` carries no `Lens`, so the gate releases on the first marker and can paint a partial world; and `personal.hydrate` is per-**identity**, so a second device's burst satisfies this device's gate. The client-side fix was built and refuted. | ★★ | M | 📐 needs designer pass · no-pattern: per-hydrate correlation-id + gate-keying (§6 names 3 needs) · [why](../../implementation-artifacts/edge-cold-signin-delivery-position-design.md) §6 Fire 4 |
...
| **[privacy-base] A pre-narrowing shredded subject earns a clean attestation over live `credentialindex` rows** | That shred tombstoned `boundTo` and left each `vtx.credentialindex.<hash>` standing, so the sweep finds no live link, all five residue arms read zero, and the seal attests `violating=false` over N live rows. | ★★ | S | 🏗️ building · [design](../../implementation-artifacts/erasure-orchestration-design.md) (fire brief 2026-08-21) · next: build the op + CLI |
...
| **[Processor] Sensitive resolution trusts a mutation's self-reported `class`, never the key's localName** | Step 6/6.5 resolve an aspect's DDL off `Document["class"]` only; nothing checks it against the key's own localName. Omitted/wrong `class` on a sensitive key commits PHI plaintext, cache healthy or not. | ★★ | M | 📐 needs designer pass · no-pattern: declared per-DDL localName to derive expected class · [why](../../implementation-artifacts/ddl-cache-invalidation-fault-signal-design.md) §1 |
