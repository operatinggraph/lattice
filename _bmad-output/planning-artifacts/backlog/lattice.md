...
|---|---|---|---|---|
| **[Contract #2] §2.5 does not state the `derive_reads` contradiction refusal** | The clause enumerates derive_reads' failure modes and a descriptor contradicting its own derivation is not among them; both neighbouring rules resolve toward demote where the Processor now refuses. Behaviour already ships (`44d42a7`); the text does not. | ★ | XS | 🔭 flag-for-Andrew · staged UNCOMMITTED in main (§2.5 diff) · ratify = commit |
| **[Contract #9] NFR-S6 states a rejection's SHAPE but not its TIMING** | §9.3 calls the generic-reply-code collapse anti-enumeration, but constrains only the wire shape; the causes were measured to separate by 0.27–0.70 ms, so the oracle is reachable without defeating the shape. Implementation shipped `624d445`. | ★★ | XS | 🔭 flag-for-Andrew · staged UNCOMMITTED in main (§9.3 diff) · ratify = commit |
| **[Contract #10] §10.3's `inflight_<g>`⇒`maxretries_<g>` MUST reads unqualified** | Its justification (the fresh-`claimId` path has no collapse to pace it) holds only for the external class, so read literally it condemns lease-signing's two human gaps, whose cap was deliberately removed as create-once-forever. The engine has always read it scoped; the text has not said so. | ★ | XS | 🔭 flag-for-Andrew · staged UNCOMMITTED in main (§10.3 diff) · ratify = commit |
...
| **[Contract #2] §2.5's declared-read cost model is a GET per key, not a batch** | Three statements describe hydration as one sequential Core KV GET per declared key and the ceiling's justification counts round trips; step 4 issues one batched read over the union. Text only, no behaviour proposed. | ★ | XS | 🔭 flag-for-Andrew · staged UNCOMMITTED in main (§2.5 diff) · ratify = commit |
| **[Contract #10] §10.3's reserved-key paragraph says the sweep skips all three** | The same table's `__effect` row says those keys are "GC'd by the sweep's orphan legs", so the sentence is already self-contradictory in main; the count leg closing §10.8's never-a-silent-park promise makes it false for `__count` too. Text only, no behaviour proposed. | ★★ | XS | 🔭 flag-for-Andrew · staged UNCOMMITTED in main (§10.3 diff) · ratify = commit |
...
| **[Bootstrap] A re-bootstrap strands the prior epoch's operator grants** | Regenerating `lattice.bootstrap.json` mints a NEW `operator` role; the prior epoch stays live but unreachable. Live: 40 permissions no service actor can reach — all 3 verticals' ledger `CreateAccount`/`DebitAccount`, `AttachObject`, the shred plane. LoftSpace- + Café-requested. | ★★★ | S–M | 🏗️ building Fire 2 · [design](../../implementation-artifacts/primordial-epoch-stranded-authority-design.md) · next: build per Fire 2 fire brief |
