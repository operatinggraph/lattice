...
| **[Packages] Content edits without a version bump never reach running stacks** | A `packages/` semantic change (permission, lens, DDL) at an unchanged manifest version is invisible to live stacks — plain install no-ops it, and nothing enforces the bump. Bit twice, latest: control-authz's `ctrl.refractor.syncgap` grant (2026-07-19 — every warm-resume Facet engine wedged fail-closed on a permission the repo had granted a day earlier). Wants a `lint-conventions` gate: a diff touching `packages/<x>/**` must change that manifest's version. | ★★ | S | 📋 ready |
| **[Refractor] Silent per-user sync wedge — no health signal** | A Facet engine whose sync manager crash-loops (e.g. controlauth denial on `personal.syncgap`) logged WARNs for 20+ min while the user saw a healthy-looking stale world — no connectivity frame, no Health KV emission, nothing Lamplighter-visible. The host knows (restart loop with backoff); surface it: feed a degraded connectivity frame to the FE + emit `health.facet.<instance>`. | ★★ | S | 📋 ready |
