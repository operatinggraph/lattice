...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[Refractor] The executor reads a marked hub whole on every typed hop** | `fetchEdges` drains a marked node's whole link keyspace even for a typed hop, so a hub identity is expanded per evaluation and the plain Postgres pair still moves 1–3 messages per 4 min after the walk fix. Mirror `adjacency.NeighborsByRelation` + the selector-scoped footprint; the validator re-read takes the same scope. | ★★ | M | 📋 ready · [design](../../implementation-artifacts/refractor-hub-walk-and-periodic-load-design.md) §8 |
