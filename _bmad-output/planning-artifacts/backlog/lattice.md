...
| **[CI] 97 `natstest.RunServer` fixtures escape the embedded-server lint** | The connect side is closed, but the gate anchors `natsserver.NewServer` only, so a new hand-rolled `RunServer` fixture still passes. These 80 files already honour `jsstore.Dir`/`Port: -1`; what they lack is `WaitForShutdown` and a single owner. Route server construction through `natsfixture.StartServerWith`, then widen the ctor gate. | ★ | M | 📋 ready · [mechanism](../../../internal/natsfixture/natsfixture.go) |
