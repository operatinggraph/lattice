...
| **[Weaver] An exhausted gap's loud stop dies at restart and cannot be un-parked** | The raise goes through the in-process `issueCache`, and the mark is left to TTL-expire — so the sweep (which enumerates marks) loses the entity and, with no `freshUntil`, nothing re-delivers the row. After a restart: no alert, no dispatch, still suppressed. The count clears only on gap-close, which the suppression prevents. | ★★ | M | 🏗️ owner: claude/great-lamport-s8dzq0 · [design](../../implementation-artifacts/weaver-exhausted-gap-durable-stop-design.md) · next: Inc 2 re-arm dispatch |
...
- *(older rolled to [archive/lattice-done.md](archive/lattice-done.md); newest `dd22e08c`)*
