...
| **[packages] Declare the actor-role confinement walk on the ops outside cafe** | The `{actor} holdsRole out` walk is 39 of the 139 baselined shapes; cafe's 8 ops declare it and their 7 rows are retired. Same mechanical pattern for the rest (clinic, loftspace, wellness, identity, lease-signing), each op's dispatcher and its hand-built test envelopes together. | ★★ | M | 🏗️ owner: claude/great-lamport-altqy0 · [brief](../../implementation-artifacts/descriptor-declared-enumerations-design.md) §8 · next: clinic-domain |
