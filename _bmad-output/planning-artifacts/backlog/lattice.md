...
| **[Pkgmgr] An uninstalled package's permission grant cannot be restored by any reinstall** | `uninstall` tombstones the permission vertex + its `grantedBy` link; a later `install`, and `install --force`, both report `install committed` while both keys stay `isDeleted=true`, so the role never regains the verb. | ★★ | S | 🏗️ building · owner: `claude/quirky-cerf-msmesv` · [why](../../implementation-artifacts/erasure-orchestration-design.md) build note · next: revive arm on the install create path |
