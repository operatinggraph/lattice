...
| **[Processor] A mutation's boolean fields (`isDeleted` etc.) are never type-checked against what readers assume** | A link `update` with `isDeleted` present but non-bool reads live to the Processor's own type assertions while Refractor's typed decoders DLQ/hard-fail — a grant `RevokePermission` can no longer even read. General to every mutation class. | ★★ | S | 📋 ready · found 2026-08-15 (review) · fail closed on a type mismatch · [why](../../implementation-artifacts/permission-role-provenance-write-once-design.md) §15 |
