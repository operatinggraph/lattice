...
| **[Pkgmgr] An AI-authored `weaverTarget` artifact has no static holder of `missing_*` ⊆ gaps** | The artifact validator defers LensRef resolution and the installer never checks the subset, so the CI lint is the invariant's only holder; the runtime holders need an installed lens's columns — live state. | ★★ | M | 🏗️ building (Inc 1+2; Inc 3 seq: §11) · owner: claude/relaxed-rubin-razw23 · [design §14](../../implementation-artifacts/weaver-target-gap-declaration-holder-design.md) · next: Inc 1 lenscolumns |
