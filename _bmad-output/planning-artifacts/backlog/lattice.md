...
| **[Pkgmgr] Uninstall erases the same secure-lens history with no attestation** | `Uninstall` tombstones a Secure Lens `.spec`, so `registry_probe` stops seeing holder types whose ciphertext still stands in the target store — the destruction oracle attests coverage it lacks. Upgrade/Apply refuse this; uninstall does not. | ★★ | S | 🏗️ owner: claude/quirky-cerf-49r72s · [brief](../../implementation-artifacts/retention-class-key-custody-design.md) §31 · next: Inc 1 gate |
