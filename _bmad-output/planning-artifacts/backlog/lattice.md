...
| **[Refractor/edge-manifest] `edgeManifestReadGrants` strands `op`, bound over unrelated chains** | Two `edgeCatalog` walks bind `op:meta` over different chains in one domain, so the base producer strands the name and the engine refuses its hop index (≈4 s/event on the BFS). The own-task walk is a distinct authorization basis — give it its own `ReadGrantDomain`. | ★★ | S | 🏗️ owner: claude/exciting-clarke-pccb08 · [triage §2](../../../docs/reviews/lattice-designer-triage-2026-09-10.md) · next: the task domain split |
