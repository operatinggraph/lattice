...
| **[Refractor] The `WITH`-scope refusal masks the two `cap-read.edgeManifest*` producers** | The generated producer re-binds a name a `WITH` dropped, so derivation refuses it — the second blocker behind the varlength one, and the lenses the live symptom actually names. Inc 2 narrows the refusal to a structural-identity whitelist. | ★★★ | S–M | 🏗️ owner: claude/bold-newton-a48ino · [design](../../implementation-artifacts/varlength-anchor-derivation-design.md) §18 · next: Inc 2 admit + T9/T10 |
