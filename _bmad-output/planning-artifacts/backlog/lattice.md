...
| **[Contract #9] NFR-S6 states a rejection's SHAPE but not its TIMING** | §9.3 calls the generic-reply-code collapse anti-enumeration, but constrains only the wire shape; the causes were measured to separate by 0.27–0.70 ms, so the oracle is reachable without defeating the shape. Implementation built + measured. | ★★ | XS | 🔭 flag-for-Andrew · proposal branch `claude/contract-9-nfr-s6-timing-invariant` (branch-vs-main diff IS the proposal) · ratify = merge |
...
| **[identity-domain] A claim rejection's LATENCY still separates already-claimed from wrong-key** | Hydration does different work per cause, so the three separate in time though the wire shape does not. Measured n=3000: monotone `absent < already-claimed < wrong-key`, 0.27–0.70 ms, load-invariant; ~12–81 requests answer *does this identity exist*. | ★★ | M | 🏗️ owner: claude/great-lamport-ukjho5 · [measured](../../implementation-artifacts/auth-plane-projection-latency-design.md) §19.8 · next: receipt-anchored reply floor |
