...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **protected-flag-create-guard-vector** | Root capability hinges on the easily-forged `identity.data.protected=true` data bit — step-8 exempts create, only identity-domain's `data:{}` convention guards it, and the bit is overloaded (anti-brick + root). Re-designate root onto a seed-only marker + Processor guard; a create-guard + Gate-2/3 vector is the interim floor. Epic-12 carried obligation. | ★★★ | S–M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/root-identity-designation-design.md) · fork: marker shape (rec. reserved envelope field) |
...
| **System-actor package-op grants absent under capability auth** | A kernel system actor's platform read is the fixed 6-op `cap.<actor>` anchor, so every engine-submitted package op (MarkExpired, CreateTask, DetachObject, RecordShredFinalization) authorizes only under the dev stub (`make up` runs `LATTICE_AUTH_MODE=stub`). | ★★ | M | 📋 needs-design (anchor grant set vs cap.roles routing) |
