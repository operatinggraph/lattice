...
| **[Weaver] A Weaver-dispatched op cannot declare a walk hubbed on the dispatching actor** | Gap-action templating admits a literal or `row.<column>`; the Weaver's actor is neither — its identity is a runtime var each deployment generates, so no Definition can embed it and no lens can project it. `ReleaseOrphanedBooking` is the shipped instance. | ★★ | S–M | 🔭 flag-for-Andrew · ratify = merge branch `claude/weaver-actor-hub-token-contract` (main + 1 commit) · [why](../../implementation-artifacts/descriptor-declared-enumerations-design.md) §8.8 |
