...
| **[Processor] A mutation's boolean fields (`isDeleted` etc.) are never type-checked against what readers assume** | A link `update` with `isDeleted` present but non-bool reads live to the Processor's own type assertions while Refractor's typed decoders DLQ/hard-fail — a grant `RevokePermission` can no longer even read. General to every mutation class. | ★★ | S | 🏗️ building · [design](../../implementation-artifacts/permission-role-provenance-write-once-design.md) §16 · next: wire `validateMutationBooleanFields` |
| **[Processor] `docIsProtected` reads a malformed stored `data.protected` as "not protected" (fail-open)** | Same `.(bool)` fail-open shape as the `isDeleted` gate above, one field over. Not reachable today — only bootstrap writes it, and a validly-`true` root already blocks all further writes. | ★ | XS | 📋 ready · found 2026-08-15 · [why](../../implementation-artifacts/permission-role-provenance-write-once-design.md) §16 |
| **[Processor] A malformed `data.sensitive` on a DDL root silently reads as "not sensitive," skipping encryption** | `ddl_cache.go`'s `rootDoc.Data["sensitive"].(bool)` fail-opens like the two rows above, but worse: a sensitive aspect whose DDL loads `Sensitive=false` skips step 6.5 encryption and commits plaintext. Forgery reachability uncensused. | ★★ | S | 📋 ready · found 2026-08-15 · [why](../../implementation-artifacts/permission-role-provenance-write-once-design.md) §16 |
| **[rbac] `vtx.roleindex.<id>` (name→role lookup) is the same unguarded-rewrite class one layer down** | An `update` on `data.roleId` redirects a canonical name to a different role, same shape as the `.canonicalName` vector just closed — no live reader today (`RoleIDs` in-memory map is the real cross-package resolution path), so latent not live. | ★ | XS–S | 📋 ready · found 2026-08-15 (review) · consumer: the first live reader of `vtx.roleindex.*` · [why](../../implementation-artifacts/permission-role-provenance-write-once-design.md) §13 |
