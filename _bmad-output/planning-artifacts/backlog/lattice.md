...
| **[Weaver] The per-row Health budget admits by ARRIVAL, so a routine `surface` family evicts non-re-derivable facts** | `rowIssueCapPerTarget`=500 is one per-target budget over `gap:`/`data:`/`template:`/`sweep:`, released only at zero. A high-volume surface gap fills it and a later `RowDataError` (Acks; never re-delivered) is refused and lost, voiding §10.8's exhaustion-raise for that target. | ★★ | M | 📐 needs designer pass · no-pattern: re-derivability-ranked per-row issue admission · [why](../../implementation-artifacts/weaver-decline-retry-substrate-native-design.md) |
...
| **[Weaver] A Weaver-dispatched op cannot declare a walk hubbed on the dispatching actor** | Gap-action templating admits a literal or `row.<column>`; the Weaver's own actor is neither — `bootstrap.WeaverIdentityKey` is a runtime var each deployment generates, so no package Definition can embed it and no lens can project it. `ReleaseOrphanedBooking`'s is the one shipped instance, its baseline row the residue. | ★★ | S–M | 🔭 flag-for-Andrew · ratify = merge branch `claude/weaver-actor-hub-token-contract` (main + 1 commit) · [why](../../implementation-artifacts/descriptor-declared-enumerations-design.md) §8.8 |
| **[Processor] A walk whose hub is link-discovered cannot be declared on any surface** | Every declaration surface resolves its hub at dispatch time, so a hub that exists only after a prior read (`vtx.building.<id> containedIn out`, reached from a unit's `containedIn` target) is undeclarable by construction — the residue the read-drift baseline's walk rows now measure. | ★★ | M–L | 📐 needs designer pass · no-pattern: chained/link-discovered enumeration hub declaration · [why](../../implementation-artifacts/descriptor-declared-enumerations-design.md) §6 |
...
| **A correctly-bounded confinement walk still blows the wall — no batched-read primitive exists** | cafe/wellness-domain's `worksAt_covers` needs 2–3 sequential `kv.Read`/`kv.Links` round trips per containment hop, all wall-clocked; `starlark_kv.go` exposes only single-key `Read`/`Links`, though the engine's dispatch path already has `KVGetMulti`. Live: café staff Charge 9/20 timeouts vs resident 2/20. Resolves `verticals.md`'s front-desk-POS fork PLATFORM. | ★★ | S–M | 📐 needs designer pass · no-pattern: Starlark `kv.ReadMulti` (or per-op-class wall budget) |
