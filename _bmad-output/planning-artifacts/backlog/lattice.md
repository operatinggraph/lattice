...
| **[Refractor] A commit landing mid-evaluation splits one actor into two rows** | `resolveProperty` point-reads `<node>.<aspect>` live per binding row (no per-`Execute` cache), so a grouping column (`u.listing.data.status`) can resolve to two values in one evaluation ⇒ 2 rows/anchor ⇒ `guardOutputKeyCollision` fails the actor closed and drops the update. ~80k redundant Core-KV reads/app. Root-causes the lease-convergence flake. | ★★★ | M | 📋 ready · fix: memoize `fetchNode` per evaluation · run [30237435839](https://github.com/operatinggraph/lattice/actions/runs/30237435839) |
