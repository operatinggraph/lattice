...
| **[Processor] A withdrawn `Sensitive` or `canonicalName` declaration is still read as live** | `ddl_cache`'s `sensitive` and `canonicalName` readers skip `isDeleted`, so an upgrade that stops emitting either keeps the old value forever. Sibling of the `script`/`permittedCommands` readers fixed in `9dcf1848`; the custody reader beside them filters. | ★★ | S–M | 📋 ready · needs design (honoring a withdrawal stops encrypting a class — a decision about existing ciphertext, not a filter) · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.17 |
...
- 2026-08-10 · `9dcf1848` · [taxonomy] `location` becomes an abstract type over `unit`/`building`/`property` — Fire B complete, first `:location*` consumer, 19 guards migrated, and the upgrade-path tombstone five readers took as live
