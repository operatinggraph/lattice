...
| **Natural-language Weaver targets — real capability-author adapter (NL-1)** | The shipped authoring pipeline (request→dispatch→bridge→review) gains its brain: `internal/reasoning` (official Go SDK) + a real `capabilityAuthor` adapter with an injected deterministic validator, env-opt-in (no key ⇒ fake, byte-identical). Fork: the bridge becomes the first third-party-secret holder + paid API caller. Propose-only. | ★★★ | L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/natural-language-weaver-targets-design.md) |
