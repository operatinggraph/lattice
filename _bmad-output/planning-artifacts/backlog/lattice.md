...
| **[Loom] The deadline probe acts on every empty body, not on the expiry it backstops** | `loom-deadline` probes on any empty-body message — a removal marker as readily as the server's `MaxAge` expiry — and cannot tell the current arm from a previous one; evidence lives 24 h, the wait 30 days. Fix: marker provenance + deadline-key presence + a conditioned fail; `disarmDeadline` deleted. | ★★ | S | 🏗️ building · owner: winston-interactive (no fleet lock) · [design](../../implementation-artifacts/loom-deadline-marker-provenance-design.md) · next: Inc 1 |
