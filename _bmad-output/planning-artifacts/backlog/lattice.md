...
| **[Refractor] Two lenses sharing one IntoKey race per column** | `edgeCatalog` and `edgeCatalogRoles` both project `manifest.op.<id>`; last writer wins per re-derivation, so a multi-hat actor's op rows flap between column shapes. Ratified shape eliminates the N-writers cause: multi-walk lenses (pkgmgr `Walks`) + engine UNION + a disjoint-key guard with no escape hatch. | ★★ | M–L | 📐 awaiting-Andrew (composition re-decision) · [design §13](../../implementation-artifacts/refractor-shared-keyspace-arbitration-design.md) |
