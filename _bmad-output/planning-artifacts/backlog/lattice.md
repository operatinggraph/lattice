...
| **[Refractor/Pkgmgr] A generated read-grant producer commits its whole target per event** | `edgeManifestStaffReadGrants` (plain auth-plane, pkgmgr-generated from the edge-manifest AnchorWalks) committed 4,915 rows for ONE appointment event and runs ~245 rows/s — 87 % of `REFRACTOR_AUDIT`, the cap-read bucket's churn, the trail's 1 h depth. | ★★★ | M–L | 📐 needs designer pass · no-pattern: provenance-scoped publication for a plain lens's keyed target · [measured](../../implementation-artifacts/personal-lens-delta-publication-design.md) §1.2 H4 |
| **[Refractor] A Secure plain lens has no neighbour retraction and no verdict** | Five Secure lenses are refused both neighbour-retraction transports (derivation licence, diff) and the audit, on reasons that name code paths those consumers never take. Fix: audit under a column mask, licence with a single-decrypt re-entry, a business-plane transport gate. | ★★ | M | ✅ ratified (Winston-adjudicated) · [design](../../implementation-artifacts/secure-plain-lens-retraction-and-audit-design.md) |
...
| **CI pipeline speed (continuous)** | Make CI faster without weakening any gate — owned continuously by the **Whetstone**. Twelve parallel jobs; unit sharded 4 ways by measured `go test` time, `internal/natsperm` its own job. | ★★ | M (ongoing) | 🏗️ continuous (Whetstone) · lint-build split into lint-build (build/vet/gofmt) + lint-static (golangci + 16 STRICT lints), parallel (c38af84): 131s/163s vs old serial 171–206s (run 33893636046, full green) · next: unit shards (150–200s) are the pole |
...
- *(older rolled to [archive/lattice-done.md](archive/lattice-done.md); newest `343260a`)*
