...
| **[capability-author] Three admission holes let an authored artifact reach the auth plane** | An authored lens may target `capability-kv`/`weaver-targets` (`LensTarget:true`) and controls its row key, so it can forge `cap.roles.<actor>`; it gets no sensitive-aspect gate (`opMeta`'s fails closed); and `protectedDispatchSets` reads manifests only, leaving bootstrap-seeded meta ops dispatchable. | ★★★ | L | 🏗️ designing · owner: lattice-designer · next: consumer-derived admission model |
