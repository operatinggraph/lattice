...
|---|---|---|---|---|
| **[Processor] The NFR-S6 tail: payload prices the release quantum; membership is a hardcoded list** | One item, two increments: the payload axis is a movable-threshold timing oracle (byte cap first, InputSchema validation refused), and the cap consumes the membership list, whose "dissolve by simplifying" direction is falsified. The receipt+Q fail-closed shape closes both — the design's fork for Andrew. | ★★★ | M | 📐 needs designer pass · no-pattern: submitter-priced work inside the release quantum · [triage §2](../../../docs/reviews/lattice-designer-triage-2026-08-27.md) |
...
| **[Weaver] The sweep enumerates state keys, not declared×projected work** | One root, three symptoms (merges the never-fired-gap + stranded-`issueKeyGapConfig` rows + the contraction-monitor finding): a never-dispatched gap leaves no key so the sweep never visits it (live: 26/28 `clinicSiteBackfill` — blocks the verticals clinic row), a target-scoped config latch can't retire, and the contraction counts' restart-replay claim is false (stable durables resume from the ack floor), reading ~0 after every restart. | ★★★ | M | 📐 needs designer pass · no-pattern: sweep work derived from declared gap columns × projected rows · [triage §5](../../../docs/reviews/lattice-designer-triage-2026-08-27.md) |
| **[Refractor] A varlength-hop lens re-evaluates every actor per CDC event, starving the auth plane** | Derivation statically refuses varlength (10 anchored + 6 plain lenses); a rebuild grinds for days while the sweep stays suppressed and co-tenant lenses lag (live: 15h suppressed, 19 lenses behind). Tractable shape: bounded per-hop expansion under `DefaultActorMaxDepth` + a `*0..` rewrite census. | ★★★ | M–L | 📐 needs designer pass · no-pattern: anchor derivation across a variable-length relationship · [triage §6](../../../docs/reviews/lattice-designer-triage-2026-08-27.md) |
