...
| **[Refractor] A latched `rebuilding` lens status survives a process cycle and never clears** | Found live 2026-08-02: 5 lenses report `rebuilding` with `consumerLag=0` and a fresh heartbeat — 3 latched 2026-08-01 05:20, ~35h and one restart earlier. `watchRebuildCompletion` dies with the process that armed it, and nothing re-arms the transition on the next boot. The shipped un-latch of a stale lens *fault* on clean registration does not cover a stale *rebuilding*. Liveness green, status wrong. | ★★ | S | 📋 ready · consumer: any operator reading lens status after a cycle |
| **[Refractor] A MATCH hot-reload rewrites `plainReprojectLabels` unsynchronized against the consumer** | `useFullEngineBranches` (pipeline.go:401-464) reassigns the map + `plainReprojectAll` while `handle` reads them, no lock. Every interleaving lands broad (fail-closed); pre-dates this fire on the plain corpus, but the Inc 1 gate makes actor-aware pipelines read them too. Consumer: any lens hot-reloaded while consuming. | ★ | S | 📋 ready · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §14.9 |
...
| **[Pkgmgr] `OpMetaSpec` has no vocabulary for a client-mint-and-reveal-secret ceremony** | identity-domain's `[no-op-meta:]`-exempt ops are undiscoverable because their submission is a client-side ceremony, not a fillable form — a package-derived index key, a client-minted secret, a raw-credential actor. | ★★ | L | 🏗️ [design](../../implementation-artifacts/client-ceremony-op-descriptors-design.md) §13.6 · next: Inc 2 — boundTo link + backfill |
...
| **[identity-domain] `ClaimIdentity`'s `consumer_grant_key` is a class-(g) key left underived** | `ddls.go` computes a deterministic `holdsRole` link key from payload + a pinned literal, and commits it as an UNCONDITIONED update precisely because "browser clients cannot compute the deterministic role key a declared read would require" — verbatim the blocker `derive_reads` removes, in the function this fire edited. | ★★ | S | 📋 ready · consumer: ClaimIdentity's unconditioned grant write |
...
- *(older entries rolled to [archive/lattice-done.md](archive/lattice-done.md); newest rolled entry `3c7bf182`)*
