...
| **[Refractor] Anchor derivation refuses a varlength hop its own executor already walks** | Every CDC event on such a lens falls back to a BFS + one full cypher re-execution per actor reached — the cost suppressing the auth plane's sweeps. Inc 1 teaches the derivation the executor's own ranged walk; converts `capabilityServiceAccess` + 4 plain lenses. | ★★★ | M–L | 🏗️ owner: claude/great-lamport-kxa2su · [design](../../implementation-artifacts/varlength-anchor-derivation-design.md) §13 Inc 1 · next: Phase 0 census C3 |
