...
| **[Pkgmgr] `OpMetaSpec` has no vocabulary for a client-mint-and-reveal-secret ceremony** | identity-domain's `[no-op-meta:]`-exempt ops are undiscoverable because their submission is a client-side ceremony, not a fillable form — a package-derived index key, a client-minted secret, a raw-credential actor. | ★★ | L | 🏗️ [design](../../implementation-artifacts/client-ceremony-op-descriptors-design.md) §16.8 · next: Inc 2b-3 — confirm the pane end-to-end |
...
| **[Refractor] A structurally-paused protected lens has no operable resume path + no diagnostic** | Three read models sit frozen at `pauseReason:"structural"`, and no bootstrap-class actor holds the `ctrl.refractor.<lensId>.resume` grant the control plane requires (`internal/controlauth/checker.go`); no CLI wraps `resume`. Measured 2026-08-03: a lens column addition is a live cause, and the pause survives a process cycle. | ★★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/structural-pause-recovery-design.md) · 3 frozen read models |
...
- *(older entries rolled to [archive/lattice-done.md](archive/lattice-done.md); newest rolled entry `6c0a08c7`)*
