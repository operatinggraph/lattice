...
| **[privacy-base] A merge concurrent with erasure step 1 still wedges the subject** | The §6 piiKey gate now refuses any merge submitted after step 1 commits, closing the general window. What remains is the race: a `MergeIdentity` that hydrates its gate reads before step 1's commit and lands after it holds no OCC on `piiKey`. Then the key is destroyed, step 2 refuses `IdentityMerged` permanently, and no re-run repairs it. | ★ | S | 📋 ready · consumer: the identityErasure pattern's step-1 guard · [why](../../implementation-artifacts/erasure-orchestration-design.md) inc 9–10 |
