...
| **[Pkgmgr] An uninstalled package's permission grant cannot be restored by any reinstall** | `uninstall` tombstones the permission vertex + its `grantedBy` link; a later `install`, and `install --force`, both report `install committed` while both keys stay `isDeleted=true`, so the role never regains the verb. | ★★ | S | 🏗️ building · owner: `claude/quirky-cerf-msmesv` · [design](../../implementation-artifacts/reinstall-over-uninstall-occupancy-design.md) · next: occupancy gate on the install submit path |
...
| **[Pkgmgr] Uninstall erases the same secure-lens history with no attestation** | `Uninstall` tombstones a Secure Lens `.spec`, so `registry_probe` stops seeing holder types whose ciphertext still stands in the target store — the destruction oracle attests coverage it lacks. Upgrade/Apply refuse this; uninstall does not. | ★★ | S | 📋 ready · built + review-clean on branch `claude/quirky-cerf-wbehyz`, needs porting onto main's `securecolumnretirement.go` · operator-attested `UninstallOptions`, never the Definition (that would disarm the R3 upgrade gate) |
