...
| **[Weaver] The sweep enumerates state keys, not declared×projected work** | One root, two symptoms (merges the never-fired-gap + stranded-`issueKeyGapConfig` rows): a never-dispatched gap leaves no key so the sweep never visits it (live: 26/28 `clinicSiteBackfill` — blocks the verticals clinic row), and a target-scoped config latch can't retire. | ★★★ | M | 📐 needs designer pass · no-pattern: sweep work derived from declared gap columns × projected rows · [triage §5](../../../docs/reviews/lattice-designer-triage-2026-08-27.md) |
