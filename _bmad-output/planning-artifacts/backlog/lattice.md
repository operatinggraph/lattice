...
| **[Refractor] A plain lens's neighbour event recomputes its whole row set** | A non-anchor endpoint seeds nothing, so the executor prefix-scans every anchor candidate: N+1 bindings per event, N = the anchor population (1,325 events wedged `clinicProviders`). Needs a scan-root terminus on `HopIndex` plus a standing healer; ≤36 of 60 plain lenses addressable. | ★★ | L | ✅ ratified (2026-08-13, detection-licence) · Incs 1+2 buildable now · seq: Inc 3+ behind divergence-audit Fire 2 · [design](../../implementation-artifacts/plain-lens-neighbour-anchor-derivation-design.md) |
