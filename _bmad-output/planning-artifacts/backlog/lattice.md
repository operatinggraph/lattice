...
| **[Processor] Nothing records what a DDL script actually read, so declared-Reads drift is unguardable** | Nothing checks a script's `contextHint` declarations are true, and helper indirection defeats any static extractor. Smallest honest guard is dynamic: record actually-read keys on `ScriptContext`, expose via testutil, e2es assert actual ⊆ declared ∪ sanctioned. | ★★ | S | 🏗️ owner: claude/great-lamport-99b998 · [brief](../../../docs/reviews/verticals-designer-triage-2026-08-27.md) §14 · next: close pass, then T-class fixture fixes |
