...
| **[Pkgmgr] A whole secure column dropped from the manifest still evades the destruction oracle** | `widenSecureColumnsForUpdate` (§29) only widens a narrowed `holderTypes` list; deleting the `SecureColumn` entry entirely produces zero widen, zero counter — `mayHoldHolderType` reads no secure columns as a genuine no. Same §24.6 hazard, cheaper to trigger than the case §29 closed. | ★★ | S–M | 📐 needs designer pass · found 2026-08-16 closing review · [why](../../implementation-artifacts/retention-class-key-custody-design.md) §29 R2 |
| **[Pkgmgr] A secure lens's canonicalName rename sidesteps the §29 widen** | The widen keys off the deterministic (package, canonicalName) key; a rename tombstones the old key and creates a fresh, historyless one, while the old ciphertext stands untouched in the target table. Same hazard as §29.1, reached by rename — Fire 28 closed the analogous retention-class-holder case. | ★★ | S–M | 📐 needs designer pass · found 2026-08-16 · [why](../../implementation-artifacts/retention-class-key-custody-design.md) §29 R3 |
...
| **[Refractor] A behavior-frozen consolidation pass — 105K LOC at ×1.8 in 39 days** | test:prod ≈1:1, `pipeline.go` ≈3.2K, `executor.go` ≈2.1K; fold test scaffolding, split god-files — LOC + CI time down, suite green. | ★★ | M–L | 🏗️ building · [design](../../implementation-artifacts/refractor-pipeline-consolidation-design.md) · next: Inc 1 rebuild.go split |
