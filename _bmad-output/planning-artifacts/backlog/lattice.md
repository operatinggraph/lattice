...
| **[Refractor] Two lenses sharing one IntoKey race per column** | `edgeCatalog` and `edgeCatalogRoles` both project `manifest.op.<id>`; last writer wins per re-derivation, so a multi-hat actor's op rows flap between column shapes (0.14.4 equalized `viaServices` only; `viaRole` still clobbers). Designed: per-source bodies + merged view in the Edge store (completes R2), struct-derivable keyspace guard, 18-tail audit. | ★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/refractor-shared-keyspace-arbitration-design.md) |
| **[Edge] A cold reconnect can report hydrated off a REPLAYED marker while fresh rows are still in flight** | The per-actor SYNC subject retains every historical delta + every prior hydrate burst; a purged mirror replays them all, the boot gate matched an old `hydrationComplete` (log rev 8059 vs the fresh marker 8843), and reads served stale descriptors for minutes. The gate must match the marker at ≥ the hydrate RPC's own returned revision; subject history growth also needs a retention posture. | ★★ | M | 📋 ready |
...
| **[Refractor] Does a lens evaluation need a point-in-time snapshot?** | The read-memo made reads repeatable per key, not across keys — one row can still blend two moments. Necessity proof ran: harmless where consumers re-validate; a **combination-grant** on the auth plane's three conjunctive shapes (`matchEphemeralGrant`, `matchServiceAccess`, RLS grant rows). Designed: no snapshots (`History: 1`); auth-plane footprint validation (verify → re-execute → requeue). | ★★ | M | ✅ ratified · [design](../../implementation-artifacts/refractor-evaluation-consistency-design.md) |
...
| **[Weaver] Convergence directOps copy projected values into irreversible params** | `resolveParam` substitutes `row.<col>` verbatim (strategist.go:583-597); ledger `DebitAccount{amountCents}` lands append-only and the DDL documents it cannot cross-check the copied value — a stale or torn row debits wrong money and the gap self-closes, never healing. Fix: params carry refs; DDLs derive/validate from declared reads; + a provenance gate. | ★★★ | M | 📋 ready · [why](../../implementation-artifacts/refractor-evaluation-consistency-design.md) §3.4 |
...
| **[Processor] `TestLaneSpecs_PerLaneBacklogIsolation` flakes in CI** | `lanes_integration_test.go:81` (`processor-default NumPending = 2, want 6`) — asserts JetStream consumer NumPending immediately after 6 publishes, no settle wait. Reproduced twice today on unrelated commits (runs 30250300831, 30315289962), both green on unforced re-run; smells like a publish/consumer-info race under CI load, not a lane-routing defect. | ★ | XS | 📋 ready · consumer: Whetstone's flake sweep |
