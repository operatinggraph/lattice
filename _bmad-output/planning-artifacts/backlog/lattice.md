...
| **[lease-signing/capability-author] A payload-named subject is read with no ownership guard on 3 `Scope:"any"` ops** | `CreateLeaseServiceInstance`/`CreateLeaseDocInstance`/`CreateAuthoringClaim` take a payload `subjectKey`, checked for shape + liveness only — any `operator`-role holder, not only Loom, can submit with an arbitrary subject. | ★★ | M | 🏗️ building · [design](../../implementation-artifacts/declared-read-scope-authorization-design.md) §12 · Inc 1 primordialActor global
