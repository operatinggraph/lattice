...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[location-domain] 25 live roots carry an abstract class, and 19 guard sites are widened for them** | 19 `vtx.unit.*` + 6 `vtx.building.*` roots minted before the flip still read `class: "location"` (counted live 2026-08-10; aspects are unaffected). Nothing rewrites them, so 7 packages' guards stay widened on the class axis and the Contract #1 tombstone marker cannot retire. | ★★ | S | 📋 ready · consumer: the 19 widened guard sites · one-shot op; class is mutable · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.22 |
