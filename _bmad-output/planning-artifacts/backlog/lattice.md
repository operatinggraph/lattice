...
| **External actor authN — IdP subject binding (Contract #11)** | How a real 3rd-party IdP's JWT `sub` becomes a Lattice identity key: per-source binding (`opaque` derives, dev key `nanoid` passthrough), fail-closed validation, mandatory issuer pin. Realizes claim-flow R3. | ★★★ | S–M | ✅ ratified (2026-07-10) · [design](../../implementation-artifacts/external-actor-authn-binding-design.md) · Contract #11 committed · Steward builds the §9 fire |
| **Multi-credential identity linking + merge credential-awareness** | One human, N IdPs: no path binds a 2nd credential to a claimed U (claim is one-shot); MergeIdentity never repoints credentialindex/bindings and the materializers fold `identity.claimed` only → a merge strands A on the merged-loser. Needs: link-credential flow, provision-time identityindex probe, merge rebind event. | ★★ | M | 📋 needs-design (Designer) · context: [authN design §12.2](../../implementation-artifacts/external-actor-authn-binding-design.md) |
| **Per-identity NATS subscribe-ACL (Edge sync plane)** | Untrusted Edge connections may subscribe ONLY their own per-identity SYNC subject (`subjects.PersonalSync`), and revocation must cut subscribe. #75 v1 explicitly declined subscribe lockdown (§3.2); PL Fork 3 assumed #75 delivers it — un-owned gap, now filed. Needs dynamic per-identity NATS authN (auth-callout vs operator-JWT fork), consuming Contract #11 tokens. | ★★ | M | 📋 needs-design (Designer) · consumer: [Edge EDGE.3](../../implementation-artifacts/edge-lattice-full-design.md) + PL Fork 3 |
