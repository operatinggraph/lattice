...
| **[Processor] Step 6.5 `continue`s past what it cannot adjudicate** | A cache-missing DDL with no live-read fault reads as "not sensitive" and commits plaintext, reachable via `Invalidate`'s warn-only failure (`step8_commit.go`). The empty/mismatched-`class` arm is a distinct gap, filed separately below — not Contract #1 §1.6-sanctioned as first believed. | ★★ | S–M | 🏗️ building · [design](../../implementation-artifacts/ddl-cache-invalidation-fault-signal-design.md) · next: fix round (2 blocking review findings) |
