...
> stack now: **[CI] `edge-browser-store` retry** (★★ XS–S — Whetstone's lane), then the Steward's own
> **[Weaver] fresh-episode/reclaim error-branch coverage** (★ S–M) and **[Bootstrap] `cmd/bootstrap` tests**
> (★ XS–S). The ★★ **[Weaver] drain the polluted windows** is designed (📐 awaiting-Andrew — `resetConfidence` verb).
