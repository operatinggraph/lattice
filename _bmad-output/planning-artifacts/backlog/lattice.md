...
| **[Pkgmgr] `OpMetaSpec` has no vocabulary for a client-mint-and-reveal-secret ceremony** | identity-domain's 5 `[no-op-meta:]`-exempt ops ([standard §8](../../implementation-artifacts/vertical-package-standard.md)) are undiscoverable because their submission is a client-side ceremony, not a fillable form — a package-derived index key, a client-minted secret, a raw-credential actor. Consumer: [verticals.md](verticals.md) 5-ops row. | ★★ | L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/client-ceremony-op-descriptors-design.md) · fork: credential binding as a link |
...
- *(older entries rolled to [archive/lattice-done.md](archive/lattice-done.md); newest rolled entry `3deda37c`)*
