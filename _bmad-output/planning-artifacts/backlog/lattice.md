...
| **[Refractor/lease-signing] `leaseApplicationComplete` cannot keep up — runaway bgcheck re-runs × broad filter** | Production `bgcheckFreshnessWindow` is `5m`: `missing_bgcheck` re-opens every five minutes and Weaver mints a new check instance each time (one identity carries 3,637); each write fans out to a 7-hop lens scanning all N, on the broad filter (unlabeled `forOperation` targets). | ★★★ | S | 🏗️ building · [design](../../implementation-artifacts/bgcheck-runaway-and-broad-filter-design.md) · next: window + labels |
