...
| **[identity-domain] A claim rejection's LATENCY still separates already-claimed from wrong-key** | Structural half closed + pinned; a tombstoned `.claimKey` still returns before the piiKey KVGet and the AEAD decrypt, so the generic refusal stays measurable-through. | ★ | M | 📐 needs designer pass · no-pattern: a constant-time rejection whose wait is off the lane pump worker · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §19.5 |
