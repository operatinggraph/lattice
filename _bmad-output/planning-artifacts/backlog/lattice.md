...
| **[Bootstrap] A re-bootstrap strands the prior epoch's operator grants** | Regenerating `lattice.bootstrap.json` mints a NEW `operator` role; the whole prior epoch — role, grants, identities, edges — stays live, reachable from no current actor. Live: 21 grants on the dead role; `bootstrap verify` passes green. LoftSpace-requested. | ★★ | S–M | 🔭 flag-for-Andrew · detection lands; the reconciliation verb (retire vs re-point) + the prior epoch's live capability lenses need a direction · [design](../../implementation-artifacts/primordial-epoch-stranded-authority-design.md) §7 |
...
| **[rbac] A `grantedBy` link carries no provenance, so a forged grant edge reconciles clean** | Authorization travels the `lnk.permission.*.grantedBy.role.*` edge, not the vertex. The edge carries no `origin` stamp and the permission reconciler walks vertices only — so the cheapest escalation forges no vertex at all and reports clean by design. | ★★ | M | 🏗️ owner: `claude/great-lamport-1h6nm5` · [design](../../implementation-artifacts/grant-edge-provenance-design.md) · next: Inc 1 stamp both channels |
