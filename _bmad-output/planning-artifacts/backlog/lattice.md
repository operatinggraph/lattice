...
| **[Weaver] A declined violating row is Acked once and never revisited** | Never-dispatched gaps invisible forever (26/28 `clinicSiteBackfill` — blocks the verticals clinic row), a config latch that can't retire, false contraction restart-replay claim. Shape: long-floor Nak'd declines standing, replay only as a manual Loupe `ReplayTarget` verb; no new enumerator. | ★★★ | M | 🏗️ owner: claude/great-lamport-i6872w · [design](../../implementation-artifacts/weaver-decline-retry-substrate-native-design.md) §16 brief · next: Inc 2 fix round; Incs 2-4 merge as one |
...
| **Suite reddens under parallel load, in packages the change never touched** | Rotating across unit-1/2 + convergence. Three open: (a) `substrate.Connect`'s 2s no-retry handshake, ~45 sites; (b) a `found=map[...]` starvation in leaseconvergence's 25s lens wait — seen EMPTY and PARTIAL (`ListingLeasedOnApproval`, convergence job), so slow watch, not never-started; (c) a wall-clock DEADLINE read as correctness — `TestRefractor_LeaseSigningConvergence_ProjectsScalarColumns` (20s activation, unit-1). | ★★★ | M | 🏗️ owner: Whetstone · next: root-cause (b) |
