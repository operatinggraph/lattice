...
| **[Bridge] `augur` adapter is never registered, so every live escalation permanently fails** | `cmd/bridge/main.go`'s reference-adapter map wires stripe/backgroundCheck/docGen/notification (+ capabilityAuthor, gated) but omits `augur` — `NewFakeAugur()` (`internal/bridge/fake_augur.go`, built "to mirror the other reference adapters") is called nowhere outside its own file. Live: `BridgeAdapterMissing` for `"augur"`, standing since 06:38 today (config error; redelivery cannot fix it). | ★★ | XS | 📋 ready · consumer: any Weaver `augur.escalate` gap once Fire 1/2a dispatches one |
