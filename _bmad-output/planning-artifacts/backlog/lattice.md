...
| **[Tooling] `verify-claim-ceremony.go` asserts a 5s SLA the platform never promised** | `waitForRoleGrant`'s 5s deadline (verify-claim-ceremony.go:437) reads a real unbounded latency as "never appears" — 4/5 live demo-box runs failed it while every grant was present in Capability KV minutes later. Poll to convergence; report the measured latency. | ★ | XS | 📋 ready |
| **[Processor] A re-claim dies in hydrate, so NFR-S6's generic rejection never renders** | The first claim tombstones `.claimKey`; the re-claim still declares it, so step 4 fails `decrypt …claimKey: read deleted sensitive aspect` → HTTP 500 before the generic 400 `ClaimKeyInvalid` can render, making "already claimed" distinguishable from "wrong key". | ★★ | S | 📋 ready · 3/3 live runs · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §17.7 |
...
| **[Refractor] A staging `WITH`'s carried accumulators are stringified into the grouping key per row** | `projectItems` normalizes every non-aggregating item per binding row, so a generated producer's stage *k* pays `rows_k × Σ|slice_j|` renderings of anchor maps an earlier stage's grouping already determines — redundant by construction. | ★★ | S–M | ✅ ratified 2026-08-06 (Winston, delegated) · [design](../../implementation-artifacts/full-engine-grouping-key-reduction-design.md) · unblocked |
...
| **[Edge] An orphan a purge cannot reap has no server-side backstop** | A revoked credential fails the sign-out reap (the auth callout correctly refuses its connection) and a crashed host never reaps — each strands a durable no client can name again. `InactiveThreshold` is the shape; the shell's 30 min cannot be copied to the Go host. | ★★ | S–M | ✅ ratified 2026-08-06 · [design](../../implementation-artifacts/edge-sync-orphan-expiry-design.md) |
...
| **[Refractor] A structural pause is terminal even where its own probe could settle it** | The tier means "pause until reconciled" but nothing re-checks: `waitWhilePaused` blocks on the operator across restarts, while `VerifyProtectedTable` already adjudicates that condition on a loop for *infra* pauses. A lens self-heals if its table is missing at activation and stays dark forever if it breaks after. | ★★ | M | ✅ ratified 2026-08-06 · [design](../../implementation-artifacts/structural-pause-recovery-design.md) §4.2 (Inc 2) |
...
| **[privacy-base] A pre-narrowing shredded subject earns a clean attestation over live `credentialindex` vertices** | The old shred tombstoned `boundTo` and left each `vtx.credentialindex.<hash>` standing, so the sweep finds no live link to walk, all five residue arms read zero, and the seal attests `violating=false` over N live `sha256(sign-in id) → erased person` rows. | ★★ | S–M | 📋 ready · consumer: the attestation over any pre-narrowing shred · [why](../../implementation-artifacts/erasure-orchestration-design.md) inc 9 |
| **[privacy-base] A merge landing between erasure steps 1 and 2 burns the key and wedges the subject** | Step 1's `absent(subject.mergedInto…)` guard is evaluated before step 1 and cannot see a merge that commits after it. The key is destroyed, step 2 refuses `IdentityMerged` permanently, no marker is written, both sweeps refuse forever — and no re-run repairs it. | ★★ | S–M | 📋 ready · consumer: the identityErasure pattern's step-1 guard · [why](../../implementation-artifacts/erasure-orchestration-design.md) inc 9 |
...
| **Package drive tests redden under full-suite load** | Six-plus `packages/*` tests, each green alone, redden together: `TestSettle_ConsumerSelfScope_Allowed`, `TestRecordPII_OperatorRoleOnSecondPage`, `TestCompleteCredentialLink_GenericError_NoEnumeration`, `TestUnbindIdentityCredentials_WideSubject_ConvergesPastOnePage`, `TestClinic_SlotGridAndTooLong`, `TestLeaseServiceReply_ReadFree_CommitsWithoutHydration`. Several blow the 250ms Starlark wall during package INSTALL. Not the handshake signature — root-cause, never loosen. | ★★ | S–M | 📋 ready · owner: Whetstone |
