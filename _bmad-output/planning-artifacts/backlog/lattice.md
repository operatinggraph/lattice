...
| **[Refractor] Sibling OPTIONAL branches multiply instead of folding** | `applyMatch` builds the full `[]binding` cross product; the 1M-row cap then permits ~730 MB. 24 lenses hold 2+ sibling branch groups in one stage (census-derived, not the design's eyeballed 14); producer staging covered only generated ones. | ★★ | L | 🏗️ building · [design](../../implementation-artifacts/full-engine-independent-branch-decomposition-design.md) · next: Inc 1 close pass |
