...
| **[Refractor] A `WITH` refuses per-anchor closure wholesale, so three plain lenses lose narrowing AND retraction** | `anchorProjectionShape` refuses a `WITH` before reading a key column: `leaseApplicationsRead` rescans the corpus per event (~64k pending, ~1 msg/min), it and `renewalsRead` retract nothing at all, `clinicPatientsRead` pays DiffRetraction. | ★★★ | M | 🏗️ building · owner: steward-lattice-with-alias · [design](../../implementation-artifacts/with-alias-anchor-closure-design.md) · next: Inc 1 predicate + census |
