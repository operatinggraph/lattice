...
| **[Weaver] The `data:` latch is read-driven, so entries strand and `planGap`'s log cannot be damped** | `issueKeyDataEntity` has five writers, cleared on every parsing read (`boolColumn`/`intColumn`). An unread column strands a `RowDataError`; the same coupling leaves `planGap`'s per-pass record (~7,680/parked gap) undampable — damping erases the fact. Damping must key on a CLOCK: `errConfig`/`errTransient` survive a row write. | ★★ | M | 🏗️ owner: `claude/great-lamport-c6lifp` · [design](../../implementation-artifacts/weaver-data-latch-fact-pairing-design.md) · next: Inc 1 fact-scoped template key |
...
- *(older rolled to [archive/lattice-done.md](archive/lattice-done.md); newest `814e4c8`)*
