...
| **[Loupe] Surface component `status`/`issues` on health + system map** | Loupe's health dashboard (`cmd/loupe/health.go`, `kindComponent` path) and system map (`cmd/loupe/systemmap.go`, `beat`) derive a component node's status from **heartbeat freshness only** — they ignore the Contract #5 §5.4 `status` and §5.5 `issues[]` fields a component emits. So the new Refractor **Capability-Lens liveness alert** (and any future component anomaly: paused lens, projection failure) is surfaced by the Lamplighter but **not** on the Loupe component cards. Read `doc["status"]` (degrade green→yellow/red on `degraded`/`unhealthy`) + map `doc["issues"]` (objects `{code,severity,message,since}`) into the card/node issue list. Pure Loupe backend + minor FE. Refs: `cmd/loupe/health.go:132-154`, `cmd/loupe/systemmap.go:136-157`. | ★★ | S | 📋 ready |
