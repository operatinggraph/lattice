...
| **[Pkgmgr] An op-meta tombstone orphans the open tasks that reference it** | A package move tombstoned `RecordIdentityPII`'s op-meta while an open task still linked it `forOperation` — the task becomes an undispatchable zombie (lens projects null, descriptor path dead). No uninstall/upgrade guard covers open referents; needs a designer call: block, migrate links, or auto-cancel. Live repro healed via operator CancelTask. | ★★ | M | 📋 ready |
