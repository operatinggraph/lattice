...
| **[Refractor] `edgeManifestReadGrants` builds a >1M-row cross product per event** | The binding cap named the lens behind the 8.4 GB profile: rule `RKF4MBBKaEEPktjxRKF4` passes 1,000,001 bindings on one `lnk.session.*.atLocation.*` event, so it is now refused + redelivered where it used to consume the memory silently — its read-grants stop refreshing meanwhile. Likely an under-anchored walk whose OPTIONAL branches cross-product. | ★★★ | M | 📋 ready · live since `5527e0e2` |
