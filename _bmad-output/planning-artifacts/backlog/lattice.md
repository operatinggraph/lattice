...
| **[Pkgmgr] `OpMetaSpec` has no vocabulary for a client-mint-and-reveal-secret ceremony** | identity-domain's 5 `[no-op-meta:]`-exempt ops are undiscoverable because their submission is a client-side ceremony, not a fillable form — a package-derived index key, a client-minted secret, a raw-credential actor. | ★★ | L | 🏗️ [design](../../implementation-artifacts/client-ceremony-op-descriptors-design.md) §12.8 · next: Inc 3 — OpCeremonySpec + G1 |
...
- *(older entries rolled to [archive/lattice-done.md](archive/lattice-done.md); newest rolled entry `4e3fd70c`)*
