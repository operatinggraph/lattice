...
| **Package drive tests redden under full-suite load** | A rotating set of `packages/*` tests, each green alone, reddens together across `identity-domain`, `clinic-domain` and `lease-signing` — the membership varies run to run, so it is the load and not any one test. Several blow the 250ms Starlark wall during package INSTALL. Not the handshake signature — root-cause, never loosen. | ★★ | S–M | 📋 ready · owner: Whetstone |
