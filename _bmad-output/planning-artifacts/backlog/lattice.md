...
| **[Weaver] A userTask gap's legitimate `inflight_<g>` column raises a standing `error`** | lease-signing projects `inflight_onboarding`/`inflight_signature` deliberately, so `gapSuppressed` can pace two human gaps (`lenses.go:495-503`); `staleMark` calls that an authoring bug and alerts `error` on every reclaim. | ★★ | S | 📋 ready · steward-owned · next run · consumer: Weaver's health status |
...
| **Async-convergence's retry + timeout legs run in no CI gate, and fail at head** | The three `TestAsyncConvergence_*` sit behind `-tags leaseshortwindow`, which `go test ./...` never sets and `make test-lease-convergence`'s `-run` never selects. Two of the three fail at clean `main` — `a failed (timed-out) call must trigger a fresh retry`. | ★★★ | S–M | 📐 awaiting-Andrew (§10.3 edit staged UNCOMMITTED) · built, green, reviewed in worktree · [design](../../implementation-artifacts/async-reply-design.md) · on ratify: merge + contract, one commit |
