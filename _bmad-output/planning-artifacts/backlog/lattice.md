...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[identity-hygiene] Dedup over encrypted PII (duplicateCandidates)** | Write-time flagging on the identityindex convention: create-collision emits a durable `duplicateOf` link; PII-free structural lens; decrypt-free merge repoint + in-commit shred erase via `indexes` links; fixes the live duplicate-create RevisionConflict + merge EdgeNotALink bugs. | ★★ | M | ✅ ratified 2026-07-10 (unkeyed hashes; Fire 4 on-demand) · [design](../../implementation-artifacts/dedup-over-encrypted-pii-design.md) |
