...
| **[Refractor] A lens consumer's ack floor freezes, so its rebuild can never drain** | Measured live 2026-08-03 on `clinicProviders`: ack floor pinned at consumer seq 3,930 while delivery reached 27,196 — 678 ack-pending, unmoved over 20m, one processed per 5m AckWait cycle, no error or pause logged. The rebuild-wedge signal is auth-plane-only (`CapabilityLensProvider`), so a plain lens's stuck rebuild publishes nothing. | ★★ | S–M | 🏗️ building · [design](../../implementation-artifacts/lens-consumer-ack-window-design.md) · next: Inc 1 substrate prefetch + heartbeat |
