...
| **[Loom] The deadline probe reads absence as rejection past its evidence's lifetime** | The `arm + window < TrackerTTL` invariant covers every replayed marker, not one the server mints late (a >23 h outage or clock jump expires the deadline key and the tracker in one pass), which fails a healthy parked instance with a misleading reason. | ★ | S–M | 🏗️ owner: claude/exciting-clarke-ulleji · [triage §3](../../../docs/reviews/lattice-designer-triage-2026-09-10.md) · next: the stale-evidence guard |
