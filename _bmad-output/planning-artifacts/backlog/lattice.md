...
| **[Refractor] A plain lens's neighbour event recomputes its whole row set** | A non-anchor endpoint seeds nothing, so the executor prefix-scans every anchor candidate: N+1 bindings per event, N = the anchor population (1,325 events wedged `clinicProviders`). Needs two pieces — a scan-root terminus on `HopIndex` that leaves `Anchor`/`DeclaresActorAnchor` alone, and a standing healer a plain lens can actually have. | ★★ | L | 📋 needs designer pass — seedable ⇒ un-sweepable by construction · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §22.1 |
...
| **[Refractor] A personal row goes stale when its own D1 grant flips** | `personalEnvelopeFn` gates each row on `cap-read.*`, which lives in Capability KV, not Core KV — so no CDC event reaches the personal pipeline when a grant lands or retracts, and personal lenses get no sweep plan. A revoked grant leaving a row live is the over-grant direction. | ★★ | M | 📋 needs designer pass · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §19.6 |
| **[Refractor] A staging `WITH`'s carried accumulators are stringified into the grouping key per row** | `projectItems` renormalizes every non-aggregating item per binding row, so a generated producer's stage *k* re-renders anchor maps an earlier stage's grouping already fixed. | ★★ | S–M | 🏗️ building · [design](../../implementation-artifacts/full-engine-grouping-key-reduction-design.md) · next: Inc 1 grouping.go + projectItems |
...
| **[Processor] A class-(e) enumeration has no budget of its own — the 250ms wall binds first** | `kv.Links` costs one KVGet per key and re-lists per page, so the wall binds far below any declared link budget: erasure dies `ScriptTimeout`, and ~19 hops sink self-pay live. | ★★★ | M–L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/script-live-read-round-trip-collapse-design.md) · 2 contract edits staged uncommitted |
...
| **Typed relation signatures — `containedIn: location→location`** | Declare a relation's endpoint types against the taxonomy, enforced at step 6 fail-closed; a signed variable-length hop contributes its endpoint expansion rather than clearing exhaustiveness. Converts the `authz_anchors` grant lenses `landlordUnitsRead` + `applicantRosterRead`; NOT `capabilityServiceAccess` (bare `exLoc` is ratified security). 14 executable hops, not 25. Retires 5 `LOCATION_TYPES` copies. | ★★ | L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/typed-relation-signatures-design.md) |
...
| **Suite reddens under parallel load, in packages the change never touched** | Rotating membership across unit-1, unit-2 and the convergence job. TWO mechanisms: (a) a test dialling production `substrate.Connect` inherits nats.go's 2s no-retry handshake, bypassing `natsfixture` — processor tree converted, ~45 sites remain; (b) a distinct starvation signature (zero lenses activated in 25s, `found=map[]`) in `internal/refractor`+`internal/leaseconvergence` that (a) does not explain. | ★★★ | M | 🏗️ owner: Whetstone · next: root-cause (b) |
...
| **CI pipeline speed (continuous)** | Make CI faster without weakening any gate — owned continuously by the **Whetstone**. Matrix split done (serial → 4 parallel jobs); convergence + unit parallelized; unit itself now sharded across 3 runners (was 2). | ★★ | M (ongoing) | 🏗️ continuous (Whetstone) · unit pole 210s→179s, overall wall-clock 227s→195s (run 31447721981) · next: lint-build (152s) or convergence's lease-convergence step (158s) are the new poles — propose paid larger runners to Andrew if squeezing those further matters |
