...
| **[Edge] A second device's hydrate releases this device's first-paint gate** | `personal.hydrate` is per-**identity** and both devices read `lattice.sync.user.<id>`, so B's burst lands above A's armed `SyncStartSeq`, names the same rule IDs and satisfies A's gate. The positional freshness test cannot help — B's burst genuinely is newer. Needs a per-hydrate correlation id on the marker. | ★ | S–M | 📋 ready · consumer: one identity signed in on two devices · wire change |
...
| **[Edge] A cold sign-in replays the actor's retained history, not their world** | Measured live 2026-07-31: 2,049 frames for a 14-key world (146×), `ready` at 33s. One subject `lattice.sync.user.<id>` carries every key's every revision, consumed `DeliverAll` (`substrate/consumer.go:142`). | ★★ | M | 🏗️ Fires 1+2 shipped, live · [design](../../implementation-artifacts/edge-cold-signin-delivery-position-design.md) · next: Fire 3 browser parity; Fire 4 refuted, needs designer |
