...
| **[Bootstrap] A re-bootstrap strands the prior epoch's operator grants** | Regenerating `lattice.bootstrap.json` mints a NEW `operator` role; the prior one and its `grantedBy` permissions stay live with no holder. Live: 21 grants reachable only from the dead role; `bootstrap verify` passes green. LoftSpace-requested. | ★★ | S–M | 🏗️ owner: claude/great-lamport-018uaf · [design](../../implementation-artifacts/primordial-epoch-stranded-authority-design.md) · next: Fire 1 detector + verify gate |
