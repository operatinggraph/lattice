...
| **[Tooling] `internal/` hardcodes package-owned operation names, undeclared** | Bare op-name literals across 15 non-test engine files; `nfrS6Operations` and the Gateway's `rawCredentialCarveOut` are byte-identical sets kept independently with no gate — a drift silently un-covers an enumeration oracle. | ★★ | S | 🏗️ owner: claude/great-lamport-47sone · [design](../../implementation-artifacts/package-op-name-declaration-gate-design.md) · next: gate + 15-file annotate + carve-out containment |
...
| **[Weaver] A playbook param can only be a string literal** | `GapActionSpec.Params` is `map[string]string` and `resolveParam` passes a non-templated value through verbatim ([strategist.go:615](../../../internal/weaver/strategist.go:615)), so a numeric or boolean literal reaches the op as a string; a typed script reader drops it for its own default and the op's `"type":"number"` InputSchema does not reject it — fail-silent. | ★★ | S | 📋 ready · sibling of the `GapActionSpec` OptionalReads row below — consolidate into one fire |
