...
| **[capability-author] `SubmitCapabilityProposal`** | Proposal *creation* is bridge-locked to the AI path (`RecordCapabilityProposal` needs a live authoring claim + model provenance), so a human-authored artifact has no way into the review queue. An operator-gated creation op on the same DDL, `source: "operator"`, no claim indirection. | ★★★ | S–M | ✅ Andrew-ratified · consumer: Loupe F25.3b · [design §6.4](../../implementation-artifacts/weaver-target-studio-design.md) |
