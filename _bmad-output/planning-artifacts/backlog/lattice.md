...
| **[Bridge] `augur` adapter is never registered, so every live escalation permanently fails** | `cmd/bridge/main.go`'s reference-adapter map wires stripe/backgroundCheck/docGen/notification (+ capabilityAuthor, gated) but omits `augur` — `NewFakeAugur()` (`internal/bridge/fake_augur.go`) is called nowhere outside its own file. Live: `BridgeAdapterMissing` for `"augur"` — a config error redelivery cannot fix. | ★★ | XS | 🏗️ owner: claude/great-lamport-4ty2l4 · next: register + composition-root coverage gate |
