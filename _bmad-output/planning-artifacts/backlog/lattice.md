...
| **[Weaver] A declined violating row is Acked once and never revisited** | Never-dispatched gaps invisible forever (26/28 `clinicSiteBackfill` — blocks the verticals clinic row), a config latch that can't retire, false contraction restart-replay claim. Shape: long-floor Nak'd declines standing, replay only as a manual Loupe `ReplayTarget` verb; no new enumerator. | ★★★ | M | 🏗️ owner: claude/great-lamport-dq3i70 · [design](../../implementation-artifacts/weaver-decline-retry-substrate-native-design.md) §16 brief · next: Inc 1 substrate Decision |
| **[Weaver] A lens may project a `missing_*` column no playbook names, and nothing says whether that is a bug** | `lease-signing` does it deliberately (`missing_decision`/`missing_manager`); a genuinely-missing entry is indistinguishable, so `GapWithoutPlaybook` can't Nak the second without parking the first forever. Shape: gate lens `missing_*` ⊆ declared gaps, convert the two to `surface`, then row 8 takes the long floor. | ★★ | S–M | 📋 ready · [why](../../implementation-artifacts/weaver-decline-retry-substrate-native-design.md) §3.2 row 8 |
| **[Refractor] Anchor derivation refuses a varlength hop its own executor already walks** | Every CDC event on such a lens falls back to a BFS + one full cypher re-execution per actor reached — the cost suppressing the auth plane's sweeps. Inc 1 teaches the derivation the executor's ranged walk (shared clamp, no new state/budget); converts `capabilityServiceAccess` + 4 plain lenses. Phase 0 runs census C3 and stops if its expectation fails. | ★★★ | M–L | ✅ RATIFIED (Andrew 2026-08-27) · [design](../../implementation-artifacts/varlength-anchor-derivation-design.md) §13 Inc 1 |
| **[Refractor] The `WITH`-scope refusal masks the two `cap-read.edgeManifest*` producers** | The generated producer re-binds a name a `WITH` dropped, so derivation refuses it — the second blocker behind the varlength one, and the lenses the live symptom actually names. Inc 2 narrows the refusal to a structural-identity whitelist. | ★★★ | S–M | 🗄️ shelved (revive: Inc 1 shipped + observed live, C4 re-derived, no soundness regression) · [design](../../implementation-artifacts/varlength-anchor-derivation-design.md) §13 Inc 2 |
