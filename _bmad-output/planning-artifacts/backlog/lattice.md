...
| **[Refractor] `clinicProviders`' 600 stranded ack-pending messages need a durable reset** | A filter update does not retract the server's pending map, and JetStream will not redeliver a message the current filter excludes — so 600 sit ack-pending forever, floor pinned at stream seq 4,287. `lattice lens rebuild` is the drain. | ★★ | XS | 📋 ready · consumer: the frozen `clinic-providers` read model · [why](../../implementation-artifacts/lens-trigger-relation-narrowing-design.md) §6.1 |
...
- 2026-08-03 · `4336978b` · [Refractor] a lens subscribes to the relations it traverses, not to every link touching a referenced type — the 674 that froze `clinicProviders`' ack floor were `providedTo` links it can never bind
