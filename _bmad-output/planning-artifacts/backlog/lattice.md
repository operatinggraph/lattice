...
| **[Refractor] D2 Phase 2 — reverse anchor enumeration for neighbor events** | A referenced-non-anchor-type event still triggers a full per-lens recompute; the closing-restart drain measured one heavy convergence lens redelivery-crawling on exactly this class (design §7 — the Phase-2 trigger, now demand-justified). Mechanism sketched in §D2: adjacency reverse walk from the event label to the anchor label, per-anchor seeded recomputes, conservative fallback for non-linear patterns. | ★★ | M | 📋 ready · [design §D2/§7](../../implementation-artifacts/refractor-footprint-reduction-design.md) |
...
- 2026-08-01 · `714cefd6` · [Refractor] a lens tombstone now stops its pipeline and deletes its durable; wires the previously-dead operator delete RPC; Unregister clears the Reprojector/Hydrator registries
...
- 2026-08-01 · `0c3fc635` · [Refractor] deleted the per-pipeline adjacency watch — handler could never resolve (bare-NanoID Get), both link arms pre-apply; −95 standing consumers [design](../../implementation-artifacts/refractor-footprint-reduction-design.md)
