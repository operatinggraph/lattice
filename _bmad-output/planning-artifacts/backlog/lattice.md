...
| **[Contract #10] §10.3's `inflight_<g>`⇒`maxretries_<g>` MUST reads unqualified** | Its justification (the fresh-`claimId` path has no collapse to pace it) holds only for the external class, so read literally it condemns lease-signing's two human gaps, whose cap was deliberately removed as create-once-forever. The engine has always read it scoped; the text has not said so. | ★ | XS | 🔭 flag-for-Andrew · proposal branch `claude/contract-10-inflight-suppression-scope` (branch-vs-main diff IS the proposal) · ratify = merge |
| **[Contract #2] §2.5 leaves a submitter's declared-read MEMBERSHIP open** | The descriptor's disposition is a floor and nothing bounds the count, so on an operation whose rejections are collapsed and quantized the submitter prices the work inside the window. Behaviour ships (`c69aa4a4`); the text does not. | ★★ | XS | 🔭 flag-for-Andrew · proposal branch `claude/contract-2-5-nfr-s6-closed-declared-set` (branch-vs-main diff IS the proposal) · ratify = merge |
| **[Contract #2] §2.5's declared-read cost model is a GET per key, not a batch** | Three statements describe hydration as one sequential Core KV GET per declared key and the ceiling's justification counts round trips; step 4 issues one batched read over the union. Text only, no behaviour proposed. | ★ | XS | 🔭 flag-for-Andrew · proposal branch `claude/contract-2-5-declared-read-cost-model` (branch-vs-main diff IS the proposal) · ratify = merge |
| **[Contract #10] §10.3's reserved-key paragraph says the sweep skips all three** | The same table's `__effect` row says those keys are "GC'd by the sweep's orphan legs", so the sentence is already self-contradictory in main; the count leg closing §10.8's never-a-silent-park promise makes it false for `__count` too. Text only, no behaviour proposed. | ★★ | XS | 🔭 flag-for-Andrew · proposal branch `claude/contract-10-weaver-state-sweep-enumeration` (branch-vs-main diff IS the proposal) · ratify = merge |
...
| **[Bootstrap] A re-bootstrap strands the prior epoch's operator grants** | Regenerating `lattice.bootstrap.json` mints a NEW `operator` role; the prior epoch stays live but unreachable. Live: 40 permissions no service actor can reach — all 3 verticals' ledger `CreateAccount`/`DebitAccount`, `AttachObject`, the shred plane. LoftSpace- + Café-requested. | ★★★ | S–M | 🔭 flag-for-Andrew · reconciliation verb (retire vs re-point) + the epoch's live capability lenses need a direction · [design](../../implementation-artifacts/primordial-epoch-stranded-authority-design.md) |
