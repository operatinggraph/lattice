...
> 🎯 **Build-ready now.** Top picks needing no new design: the **three Refractor hot-reload rows**
> (★★ S–M each — one coupled fire; the grant-lens INTO strand row belongs with them), the
...
| **[appsession] The origin gate is Manager-bound, and Loupe keeps a fork of it** | The gate hangs off `*Manager`, so the one console that does not use the kit's session surface kept a ~110-line copy — which had already drifted (it accepted an empty host label the kit rejects). The copy also admitted NESTED navigations: `Sec-Fetch-Mode: navigate` is sent for an iframe too, so a co-hosted page could frame any signed-in app. Extract a `Manager`-independent gate, fix the nested-navigation hole once, delete the fork. | ★★ | S–M | ✅ SHIPPED |
...
| **[Refractor] An INTO edit can strand a grant lens's live grants** | `HotReloadInto`'s refusal is armed only from inside itself for this family (`InstallActorAggregate` never runs for a grant lens), and the target/bucket pin misses a `grantTable: true → false` edit — so the first such swap is accepted and every row the lens wrote is left unretractable. Over-grant; pre-existing. | ★★ | S–M | 📋 ready · consumer: an operator editing a grant lens's INTO · pair with the 3 hot-reload rows · [why](../../implementation-artifacts/lens-projection-liveness-design.md) §13.4 |
...
| **[appsession] The production IdP posture cannot open a session** | `setCookie` runs only under a non-nil `Signer`, so with `_JWT_PUBLIC_KEY`/`_ISSUER` set nothing can issue the cookie — the verify-only posture is unreachable (401 everywhere), and `/api/session/refresh` 404s so every FE write path dies with it. | ★★ | L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/appsession-oidc-production-signin-design.md) · no contract change |
