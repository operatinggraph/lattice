...
| **[Weaver] The sweep enumerates state keys, not declared×projected work** | One root, three symptoms: a never-dispatched gap leaves no key so the sweep never visits it (live: 26/28 `clinicSiteBackfill` — blocks the verticals clinic row), a target-scoped config latch can't retire, and the contraction counts' restart-replay claim is false (stable durables resume from the ack floor; ~0 after every restart). | ★★★ | M | 📐 needs designer pass · no-pattern: sweep work derived from declared gap columns × projected rows · [triage §5](../../../docs/reviews/lattice-designer-triage-2026-08-27.md) |
