...
| **[Processor] A descriptor's read disposition is not enforced against the envelope** | Contract #2 §2.5 now requires the merged disposition to be the weaker of descriptor and envelope, so a submitter cannot harden a key the descriptor declared absence-tolerant. **Ratified and committed; the enforcement is unbuilt, so the clause is fail-open.** Closes the `HydrationMiss` existence oracle at its only real enforcement point. | ★★★ | S | 🎯 top-priority · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §19.5 |
