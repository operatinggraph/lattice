...
| **[Refractor] A sweep write after a rule swap's truncate locks the rebuild out of the key** | `Reproject` takes a rule snapshot but, unlike `writeResults`, never checks `supersededRule` — and it stamps the consumer head, so one old-rule sweep write landing after the reload's truncate recreates the key at watermark H and the ≤-guard rejects the whole replay beneath it: a revoking MATCH edit silently defeated, reported healed. | ★★ | S | 📐 awaiting-Andrew · [design](../../implementation-artifacts/sweep-rule-snapshot-granularity-design.md) |
...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[Vault] Sensitive aspects are identity-anchored, so retained records have no home** | Step 6 rejects a sensitive aspect on any non-identity parent, so three retained-class records sit plaintext in Core KV: clinic `.encounter`, lease-signing's income `.profile`, the background-check `outcome`. Custody belongs to a key holder carrying a retention policy — identity is the erase-on-request kind. | ★★★ | L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/retention-class-key-custody-design.md) |
