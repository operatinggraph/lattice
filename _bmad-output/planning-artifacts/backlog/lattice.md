...
| **[identity-domain] A claim rejection's LATENCY still separates already-claimed from wrong-key** | A tombstoned `.claimKey` skips the piiKey KVGet and the AEAD decrypt, so the rejection causes may differ in time even though the wire shape does not. | ★ | S | 🏗️ owner: claude/great-lamport-ukjho5 · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §19.5 · next: concurrent re-measure, then floor-or-close |
