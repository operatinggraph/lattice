...
| **[location-domain] The "any location" write guards read a package-local list, not the taxonomy** | Each hardcodes `LOCATION_TYPES`, so a leaf another package declares is invisible to them. The taxonomy-read form needs Contract #2 §2.5 declarations at every dispatcher plus a fail-closed answer. | ★★ | M | 📋 ready · needs design · consumers: `WireContainedIn`, service-location's 4 wiring ops, `CreateMenuItem` · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §9.3 |
| **[Processor] `entityType` ⟷ `entityKey` type-segment pairing is unenforced** | Nothing checks a document's class against its key's type segment. Cannot be switched on as-is: 69 live location vertices carry `class:"location"` on `vtx.unit.*`/`vtx.building.*` keys and would violate it. | ★★ | M | 📋 ready · consumer: every `edge-manifest` walk tail · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §14 |
| **[Refractor] No per-row `typeOf(x.key)` engine function** | `AnchorWalk.AnchorType`'s audit literal is what forbids an abstract Path-B anchor today. | ★ | S–M | 📋 ready · consumer: `AnchorWalk.AnchorType` · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §14 |
| **[Pkgmgr] `subtypeOf`-driven DDL inheritance** | location-domain's three leaves share one shape through a Go factory; inheritance would remove the duplication, but §3.4 warns it reintroduces multiple-parent ambiguity — forbid multiple parents or set precedence first. | ★ | M | 📋 ready · needs design · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §9.1 |
| **[Processor] An abstract type has no place to hang a category-level command** | One operationType on N sibling leaf classes makes `buildByCommand` mark it ambiguous (`ddl_cache.go:526-583`), so `ClassForCommand` drops it and every submitter must name a concrete class. | ★★ | M | 📋 ready · needs design · consumers: the 30 location submitters · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.16 |
| **[Processor] A withdrawn `Sensitive` or `canonicalName` declaration is still read as live** | `ddl_cache`'s `sensitive` and `canonicalName` readers skip `isDeleted`, so an upgrade that stops emitting either keeps the old value forever. Sibling of the `script`/`permittedCommands` readers fixed alongside them; the custody reader beside them filters. | ★★ | S–M | 📋 ready · needs design (honoring a withdrawal stops encrypting a class — a decision about existing ciphertext, not a filter) · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.17 |
| **[Refractor] An operator corpus-rebuild bypasses B0's coalescing scheduler** | `control.Service.rebuildRule` (`internal/refractor/control/service.go:930-943`) spawns one uncoordinated goroutine per request, reproducing the consumer delete-recreate burst that has OOM-killed `lattice-nats`. Routing it needs the control service to reach the reloader. | ★★ | S–M | 📋 ready · consumer: an operator corpus-rebuild · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.15 |
| **[Refractor] An auth lens with a containment walk cannot be narrowed at all** | `ReferencedLabels` clears exhaustiveness for any variable-length hop, so `capabilityServiceAccess` takes the broad filter whatever it labels — the taxonomy's stated payoff measured false. Narrowing it means reconciling `*0..` walks with the exhaustiveness rule. | ★★ | M | 📋 ready · needs design · consumer: `capabilityServiceAccess`'s footprint · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §2.1 |
...
- 2026-08-10 · `7e60c4db` · [taxonomy] `location` becomes an abstract type over `unit`/`building`/`property` — Fire B complete, first `:location*` consumer, 19 guards migrated, and the upgrade-path tombstone five readers took as live
