...
| **[Tooling] The history-comment gate misses "this fire"-shaped narration** | CLAUDE.md's most-violated rule, unscreened for narration anchored on the authoring change: 89 live sites / 71 files. Gate-flip and sweep must land together, and the sweep first needs `lint-package-version`'s comment-only exemption extended to in-package files. | ★ | M | 🏗️ owner: `claude/great-lamport-2hvzj6` · [design](../../implementation-artifacts/history-comment-gate-this-fire-shape-design.md) · next: Inc 1 comment-only exemption |
