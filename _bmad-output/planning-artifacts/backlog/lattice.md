...
| **[lease-signing/Loom] A superseded background check keeps its instance — 12k accumulated on seven identities** | A check superseded by its successor stays a live instance; `leaseApplicationComplete` scans all N per event (3,637 on one identity, ~30 s), so its rebuild drains at ~2 msg/min. Fire 2 (Andrew-authorized): a sanctioned supersession tombstone op + a one-off purge. | ★★★ | M | 🏗️ building · [design](../../implementation-artifacts/bgcheck-runaway-and-broad-filter-design.md) §6 · next: op + purge |
