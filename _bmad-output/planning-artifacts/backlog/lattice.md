...
| **[Processor] Step 6.5 `continue`s past what it cannot adjudicate** | A cache-missing DDL with no live-read fault reads as "not sensitive" and commits plaintext, reachable via `Invalidate`'s warn-only failure (`step8_commit.go`). The empty-`class` arm is Contract #1 §1.6 permissive-by-design (confirmed, not a bug) — row narrowed 2026-08-14. | ★★ | S–M | 🏗️ building · [design](../../implementation-artifacts/ddl-cache-invalidation-fault-signal-design.md) · next: degraded latch + step 6.5 gate + Health signal |
