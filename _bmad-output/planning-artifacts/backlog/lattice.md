...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[Refractor/Weaver] Expiry is a fact on the CHECK, not a clock in the lens — sweep the `$now` lenses** | 16 lenses read `$now`: stored rows are claims about a past instant, the sweep cannot tell a broken projection from a moving clock, and the plain derivation keeps the Postgres pair's corpus rescan. Fix: the timer's marker records the lapse per target and the lens reads it. | ★★★ | L | 🏗️ building · owner: steward-lattice-expiry-fact · [design](../../implementation-artifacts/expiry-as-a-recorded-fact-design.md) · next: Inc 0 premises + 4-deep pin |
...
| **[Refractor] The Postgres pair projects N rows per anchor, so the per-anchor plain derivation stays refused** | `leaseApplicationsRead` / `landlordLeaseApplicationsRead` anchor on the identity / landlord and key rows by application; `ProjectsOneRowPerAnchor` is false by shape, so with the `$now` refusal gone the licence refuses at `anchor_derivation_plain.go:341` and every neighbour event still rescans the corpus (~64k pending each, ~1 msg/min). | ★★★ | M–L | 📐 needs designer pass · no-pattern: per-anchor plain derivation for a lens projecting N rows per anchor · [why](../../implementation-artifacts/expiry-as-a-recorded-fact-design.md) §13 |
