...
> grant keys** fix (★★ L) is now `🏗️ building` — Fire 1 increments 1–7 (descriptor field, driver
> perEntry emission, core prefix-diff retraction, actor-disappearance retraction, shred-site prefix
> enumeration, retry-path refusal, sweep coverage/orphan deltas) shipped; next is the per-entry
> envelope wiring + §4.5 truncate. `appsession`'s OIDC design stays
