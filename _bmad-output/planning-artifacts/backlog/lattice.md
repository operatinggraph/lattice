...
| **[Refractor] The taxonomy barrier arms then disarms across a connection loss** | `TestCoreKVSource_TaxonomyBarrier_VerdictStraddlingAConnectionLossIsDiscarded` reports `[true false]` under `-race` (5/8 at clean main, 2/8 with C2.6). A caught-up verdict computed before `proxy.cut()` still arms afterwards — the fail-open window `d470c9ac` exists to close. CI does not run `-race`. | ★★ | S–M | 🏗️ building · tighten, never loosen · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.21 |
...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **Dynamic type taxonomy — an abstract type a lens can label** | `subtypeOf` links between type metas, resolved to a leaf-label set at activation, so a leaf any package declares is picked up by lenses writing `:abstract*`. First consumer `capabilityServiceAccess`. | ★★★ | L | 🏗️ building · [design](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.21 · 🔭 Contract #1 edit uncommitted (tombstone exemption) · next: C4/C5 designer pass |
