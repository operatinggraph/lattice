...
| **[Refractor/edge-manifest] `edgeManifestReadGrants` strands `op`, bound over unrelated chains** | Three `edgeCatalog` walks anchor `op:meta` over different chains and two land in the base producer, so a stage boundary strands a name no engine narrowing may re-admit. The shared `edgeCatalogTail` binds `op`/`role` by name, so per-walk scoping is a mechanism, not a rename. | ★★ | M | 📐 needs designer pass · no-pattern: per-walk anchor-variable scoping in `generateProducerSpec` · [why](../../implementation-artifacts/varlength-anchor-derivation-design.md) §5 |
...
| **[lease-signing] Supersede a background check automatically when its successor completes** | A completed check with a strictly-later completed sibling on the same applicant retires itself: a lens projects the pair, Weaver's `directOp` submits the shipped `TombstoneSupersededLeaseServiceInstance`; the op derives six of its seven reads and admits Weaver. History stays at rest as a body-preserving tombstone. | ★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/bgcheck-supersession-convergence-rule-design.md) |
| **[Loom] The deadline probe's evidence is shorter-lived than the wait it backstops** | The probe judges rejected-or-lost from the 24 h tracker's absence while the wait is unbounded, so a `deadline.<id>` marker delivered past 24 h fails a healthy parked instance. | ★★ | M | 📐 needs designer pass · no-pattern: an armed/disarmed deadline fact on the instance record · held out of reach by `MaxDeadlineArm`+marker window (≤2 h), not closed · [why](../../implementation-artifacts/loom-state-tombstone-sweep-design.md) §11.2 |
...
| **[Weaver] Mechanize the retire-above-guard ordering — no dispatch reachable from an ungated arm** | The dossier's retire/guard class has now been minted four times, latest at three seams at once (leg release vs the `inflight_<g>` gate). A gate asserting every `advanceReleasedLeg`/`fireEpisode` is reachable only past a `gapSuppressed*` consult would catch the shape statically. | ★★ | S–M | 📋 ready · [dossier](../../../docs/components/weaver.md) entry 9 |
