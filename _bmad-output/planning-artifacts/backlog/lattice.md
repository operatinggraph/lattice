...
| **[Refractor] A lens consumer's ack floor freezes, so its rebuild can never drain** | The 674 wedging `clinicProviders` are `lnk.service.*.providedTo.identity.*` events — a relation the lens never traverses. `CoreKVNarrowedFilters` wildcards the relation segment, so a lens subscribes to every link touching a referenced type. The pump hardening and the health signal shipped; this is the cause. | ★★ | S–M | 🏗️ building · [design](../../implementation-artifacts/lens-trigger-relation-narrowing-design.md) · next: Inc 1 `ReferencedRelations` |
