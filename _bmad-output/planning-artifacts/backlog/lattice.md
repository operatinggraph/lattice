...
| **Hard-delete mutation verb (true link/aspect keyspace reclaim)** | The Starlark mutation vocab is `create`/`update`/`tombstone` (all soft PUTs); a soft-tombstoned key persists and is still enumerated by `kv.Links`. A 4th hard-delete/purge verb (substrate purge + step-8 commit + Contract #2) would let terminal `hasBooking` links (and other dead keys) leave the keyspace, bounding `kv.Links` **LIST** cost (not just the GET fan-out eager-tombstone already bounds). Surfaced by kv.Links Fire 2. | ★ now / ★★ at scale | M | 📐 needs design · contract-touching (new mutation verb) → Andrew-gated; not a live-correctness gap (guard's live-status skip is the backstop) |
