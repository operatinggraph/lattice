...
| **[Refractor] `CapabilityRepairBlocked` leaves an auth-plane row permanently wrong** | The ordering guard declines a divergent capability projection's repair (stored watermark ≥ reconciliation token) and the write reports success having changed nothing — wrong until an unrelated CDC event reprojects it. Live: 17 rows unrepairable, 250+ consecutive sweep passes (Contract #6 §6.2). | ★★★ | M–L | 📐 needs designer pass · no-pattern: a repair path for a watermark-blocked capability divergence · `internal/refractor/health/lattice_heartbeater.go` |
...
| **[identity-domain] A claim rejection's LATENCY still separates already-claimed from wrong-key** | A tombstoned `.claimKey` returns before the piiKey KVGet and the AEAD decrypt, so already-claimed is measurably faster than wrong-key; the wire shape is not the leak. | ★ | M | 📋 ready · deferred reply off the lane pump worker, precedent `AuthTraceEmitter.Emit` · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §19.5 |
