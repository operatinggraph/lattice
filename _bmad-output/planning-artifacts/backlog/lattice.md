...
| **[Refractor] D2 Phase 2 — reverse anchor enumeration for neighbor events** | A referenced-non-anchor-type event still triggers a full per-lens recompute; the closing-restart drain measured one convergence lens redelivery-crawling on exactly this class — the §D2 Phase-2 trigger, now demand-justified. | ★★ | M | 📋 ready · [design §D2/§7](../../implementation-artifacts/refractor-footprint-reduction-design.md) |
| **[Refractor] Orphan-durable janitor at boot** | The tombstone-event reap needs a live pipeline, so a lens tombstoned while Refractor is down (or pre-reap history) strands its `refractor-<ruleID>` durable forever — three found live 2026-08-01, each holding ~11k phantom pending that reads as lens lag in Loupe. Boot pass after lens-source catch-up: delete lens durables whose meta is absent/tombstoned. | ★★ | S | 📋 ready |
| **[Refractor] A staging `WITH`'s carried accumulators are stringified into the grouping key per row** | `projectItems` normalizes every non-aggregating item per binding row, so a generated producer's stage *k* pays `rows_k × Σ|slice_j|` full renderings of already-collected anchor maps — measured 3.3 s / 7.2 GB alloc at 5,009 anchors while peak rows sat at 0.3% of the cap. The accumulators are constant across a stage's rows, so the key fragment is recomputable once. | ★★ | S–M | 📋 ready · consumer: an actor whose grant set passes ~2k anchors |
...
| **[Refractor] The executor still materializes the whole binding set** | `applyMatch`/`matchPath` build the full `[]binding` cross product — ~75 MB per 102k rows, so the 1M-row cap permits ~730 MB. The one lens that legitimately hit the cap is closed by producer staging, so this now waits on a lens whose SINGLE walk fan-out exceeds it. Streaming the set (lazy expansion, or push-down into the fold) removes the ceiling rather than bounding it. | ★★ | L | 📋 designer · no live consumer · [why](../../implementation-artifacts/read-grant-single-source-walk-design.md) §12 |
...
| **[Edge] 74 orphaned SYNC durables have no host that can reach them** | Measured 2026-08-01 on the dev stack: 74 durables for 13 identities, all but three pinned at the 10k per-subject cap (~740k phantom pending, reads as lens lag in Loupe). New ones stopped; each orphan is named by a device id no surviving mirror holds, so only an operator sweep can clear them. | ★★ | XS | 📋 ready · delete all `edge-sync-*` on SYNC (refractor NKey has the grant); safe — verified no mirror names any of them |
| **[Edge] An orphan a purge cannot reap has no server-side backstop** | A revoked credential fails the sign-out reap (the auth callout refuses its connection, correctly) and a crashed host never reaps at all — each strands a durable no client can name again. `InactiveThreshold` is the shape; the browser shell's 30 min cannot be copied to the Go host. | ★★ | S | 📋 designer · seq behind [cold-signin](../../implementation-artifacts/edge-cold-signin-delivery-position-design.md) — its fork sets the value |
...
- *(older entries rolled to [archive/lattice-done.md](archive/lattice-done.md); newest rolled entry `4186479a`)*
