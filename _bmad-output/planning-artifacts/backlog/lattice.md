...
| **[Weaver/Pkgmgr] The `inflight_<g>`⇒`maxretries_<g>` MUST is unenforced; privacy-base ships the uncapped-external case** | §10.3 requires an external gap declaring `inflight_<g>` to declare `maxretries_<g>`; nothing checks it, and privacy-base's 3 residue gaps (directOp, const-false marker, no cap) are exactly that — reclaim paced-but-unbounded. | ★★ | M | 📋 ready · install-time gap-spec validation (precedent: pkgmgr target validation) · prereq: privacy-base's declaration decided · [why](../../implementation-artifacts/inflight-suppression-vs-reclaim-contract-design.md) §7 |
