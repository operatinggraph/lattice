...
| **[Refractor] A Personal lens write hits `natssubject upsert: key field "ns" absent from keys map`** | Live, 500+/5min across 3+ ruleIds incl. edgeCatalog, `stage=write adapter=nats_subject`. `ns` is a RETURN literal, never legitimately absent — ungrounded. Logged `entityId` is the triggering CDC key (`pipeline.go:1301`), not the failing row. | ★★ | M | 📋 ready · found live 2026-07-30 post-cycle-refractor, reproduce fresh before grounding |
