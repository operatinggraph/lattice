...
| **[Weaver] A declined violating row is Acked once and never revisited** | Never-dispatched gaps invisible forever (26/28 `clinicSiteBackfill` — blocks the verticals clinic row), a config latch that can't retire, false contraction restart-replay claim. Shape: long-floor Nak'd declines standing, replay only as a manual Loupe `ReplayTarget` verb; no new enumerator. | ★★★ | M | 🏗️ owner: claude/great-lamport-i6872w · [design](../../implementation-artifacts/weaver-decline-retry-substrate-native-design.md) §16 brief · next: close-pass fixes, then merge Incs 2-4 |
