...
| **[Loom] An instance parked on an inconclusive deadline verdict has no operator verb** | The shipped horizon guard alerts and records rather than assert a terminal it cannot support, so a genuinely-rejected step holds with no engine verdict and nothing to resolve it. Widening `redrive` was built and withdrawn on two proved defects. | ★★ | M | 📐 needs designer pass · no-pattern: an operator resolution for a running-but-unadjudicable instance · [catalogue](../../../docs/reviews/lattice-designer-triage-2026-09-10.md) §3.8 |
...
| **[Processor/Starlark] `time.weekday` / `time.seconds_of_day` have no zone** | Both builtins evaluate in UTC only ([modules.go:107](../../../internal/starlarksandbox/modules.go:107)); a script cannot ask what local weekday or time an instant is at a site, so every wall-clock rule (clinic hours, time-off) is authored in UTC and drifts at DST. An optional IANA-zone argument, Go tzdata embedded. Requested by Clinic (hours + time-off in the site's clock). | ★★ | XS–S | 📋 ready |
...
- *(older rolled to [archive/lattice-done.md](archive/lattice-done.md); newest `89b61556`)*
