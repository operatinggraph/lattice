...
|---|---|---|---|---|
| **[Processor] NFR-S6 membership is a hardcoded op-name list in engine code** | `nfrS6Operations` pins `ClaimIdentity`/`CompleteCredentialLink` by name in `claim_reply_floor.go` — engine code carrying package knowledge; a third ceremony op can't join the anti-enumeration class without a Processor edit. Andrew: de-hardcode by SIMPLIFYING — net reduction in lines, no new machinery (2026-08-25; the closed-set contract clause was rejected as implementation detail the same day). | ★ | S | 📐 needs designer pass · no-pattern: engine-external anti-enumeration membership |
| **[Processor] A submitter prices an NFR-S6 op's release quantum by PAYLOAD size** | The declared-read axis is closed; the payload axis is not. `env.Payload` is decoded by the admitted-set compile, the `derive_reads` pre-pass and the step-5 runner, with no bound below the Gateway's 1 MiB cap and no server-side `InputSchema` — the Processor loads no such aspect and carries no validator. | ★★ | M | 📐 needs designer pass · no-pattern: server-side payload admission against a descriptor · [why](../../implementation-artifacts/auth-plane-projection-latency-design.md) §20.8 |
...
| **[Weaver] `issueKeyGapConfig` strands on the orphan-column route** | Retirable only by lane 1's candidate walk, which needs the row to still project the column; a quiet row plus a playbook that drops the gap leaves it standing for the process's lifetime. The fact is target-scoped, so no sweep leg can tell "the column left the target" from "one entity's gap ended". | ★ | M | 📐 needs designer pass · no-pattern: a per-target observed-column set · [why](../../implementation-artifacts/weaver-data-latch-fact-pairing-design.md) §6 |
| **[Weaver] A directOp gap that never fires once gets no sweep coverage, ever** | Live: only 2 of 28 `clinicSiteBackfill`-violating entities ever got a `weaver-state` dispatch-count; `sweepCount`'s fresh-dispatch arm only re-arms a count reset to 0, never an absent one. Root cause of the verticals-lane "29 appointments never get a site" row — a `violating`-predicate fix there would mask, not fix, it. | ★★★ | M | 📐 needs designer pass · no-pattern: sweep coverage for a gap with no dispatch-count key · [verticals.md](verticals.md) clinicSiteBackfill |
...
| **[Refractor] Capability sweep reports real event loss while a lens rebuild looks wedged** | `CapabilityCoverageDivergence` hit error (recurring heals — design's own "events lost, not repaired" bar) while several lenses sat `rebuilding` at huge lag, zero measured drain, Refractor near-saturated CPU; the sweep itself unverified for hours (rebuild in flight). No seed/backfill running. Hypothesis: a wedged rebuild starves the sweep. | ★★★ | M | 📋 ready · investigate the wedged rebuild · [why](../../implementation-artifacts/capability-projection-reconciliation-design.md) |
| **[Loom] Heartbeat's running-instance scan silently breaks down past the KVGetMulti 1,024-subject fast-path cap** | Live: `loom-state` holds 9,260+ instance keys, so every heartbeat tick's `KVGetMulti` falls to the slow drain fallback and times out (`internal/loom/health.go:66`). Caught and WARN-logged only — `runningInstances` drops silently, heartbeat still reports `status=healthy`. | ★★ | S–M | 📐 needs designer pass · no-pattern: bounded scan past the fast-path cap, or surface as a health issue not a log-only WARN |
| **[Processor] descriptor-floor Reads-template can't express a link-key / bare-type-segment read** | `expandDescriptorTemplate` resolves only bare `:id` or a full `vtx.<type>.<id>`, not just `<type>` mid-template — `DetachObject`'s declared read needs it. Distinct from verticals' `SetAppointmentStatus` row (triage 2026-08-27: 25-op census, zero class-D — only objects-base's D7 pair needs this). | ★ | S | 📐 needs designer pass · no-pattern: type-agnostic link-key Reads-template segment · [why](../../implementation-artifacts/staff-descriptor-rendering-design.md) §22 |
...
| **[Weaver/Loom/Refractor] `control-operator` role has zero holders — its 22 `ctrl.*` grants are unreachable except via `consoleOperator`'s parallel copy** | `control-authz` grants `ctrl.*` to `control-operator`; a live census of every `holdsRole` edge finds none into it. `console-operator` duplicates the same set onto `consoleOperator` "verbatim, not imported" — Loupe works only via that duplicate. Undecided: bootstrap-assign a real holder, or retire as dead surface. | ★ | XS | 📐 needs designer pass · no-pattern: intended holder of control-operator |
...
| **[Weaver] `GapActionSpec` has no `OptionalReads`, so a lazy-account-open gap can't declare its read-before-create guard check** | `pkgmgr.GapActionSpec` (definition.go:423) has no `OptionalReads`, unlike DDL-level `ContextHint`; a racing `ClinicCreateAccount`/`WellnessCreateAccount` `missing_account` dispatch can't declare its guard-aspect read, so a clean `AccountAlreadyExists` path is unreachable — settles via a raw substrate conflict instead (retry cost, not a correctness bug). | ★ | S | 📋 ready · found building [verticals.md](verticals.md) wellness missing_account |
