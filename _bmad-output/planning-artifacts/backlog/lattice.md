...
| **[Bootstrap/Loom] The 1 s marker TTL is the delivery window of every TTL-expiry signal** | `LimitMarkerTTL=1s` on all six `PerKeyTTL` buckets means a `deadline.*` expiry — Loom's only rejected/lost-step signal — exists for one second; a consumer down across it never sees it and no startup scan notices. Decide the value per bucket. | ★★★ | S | 🏗️ owner: claude/bold-newton-p45ugl · [design](../../implementation-artifacts/platform-bucket-marker-ttl-design.md) · next: Inc 1 registry field |
| **[Loom] The deadline probe's evidence is shorter-lived than the wait it backstops** | The probe judges rejected-or-lost from two absences — the 24 h tracker and the outbox record — while the wait is unbounded, so a `deadline.<id>` marker delivered past 24 h fails a healthy parked instance. | ★★★ | M | 📐 needs designer pass · no-pattern: an armed/disarmed deadline fact on the instance record, readable across redrive/replay · [why](../../implementation-artifacts/loom-state-tombstone-sweep-design.md) §11.2 |
