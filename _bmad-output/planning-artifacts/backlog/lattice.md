...
| **[Processor] A submitter prices the work inside an NFR-S6 op's release quantum** | Declared reads (cap 1000) resolve inside the quantized window and the Gateway copies `contextHint` verbatim, so padding can shift a rejection across a boundary. Quantization bounds this to a boundary-crossing probability; full closure needs a CLOSED declared-read set. | ★★ | M | 🏗️ owner: `claude/great-lamport-oevqfj` · [brief](../../implementation-artifacts/auth-plane-projection-latency-design.md) §20 · next: closure at step-4 head |
