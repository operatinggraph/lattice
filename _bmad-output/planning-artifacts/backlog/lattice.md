...
| **[Weaver] The `data:` latch is read-driven, so entries strand and raises cannot be damped** | `issueKeyDataEntity` has five writers and its clear runs on every parsing read (`boolColumn`/`intColumn`). A column no longer read strands a standing `RowDataError`; the same coupling means any raise on that key, if damped to cut log volume, goes absent for the whole window. | ★★ | M | 📐 needs designer pass · no-pattern: per-delivery re-derivation of a row's data-issue family · from the `data:` split (`173de5cc`) |
