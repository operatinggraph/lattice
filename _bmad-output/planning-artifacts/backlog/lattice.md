...
| **[Refractor] A plain lens's neighbour event recomputes its whole row set** | A non-anchor endpoint seeds nothing, so the executor prefix-scans every anchor candidate: N+1 bindings per event, N = the anchor population (1,325 events wedged `clinicProviders`). Needs a scan-root terminus on `HopIndex` plus a standing healer; 33 of 60 plain lenses addressable. | ★★ | L | 🏗️ building · [design](../../implementation-artifacts/plain-lens-neighbour-anchor-derivation-design.md) · Incs 1+2 shipped (`0d69d5f3`, shadow) · next: Inc 3 licence, seq: divergence-audit Fire 2 |
...
| **[Refractor] Lens health is liveness-only — a frozen row renders green** | 12 orphanedTaskGrants rows sat 12 days stale behind a green lens card: status/lag/error health cannot see per-row wrongness. | ★★ | M | 🏗️ building · [design](../../implementation-artifacts/lens-projection-divergence-audit-design.md) · next: Fire 2 plain-lens Auditor, unblocks plain-lens-neighbour Inc 3 |
