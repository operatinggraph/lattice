...
| **Natural-language Weaver targets (NL-1 model-runner + adapter, NL-2 Loupe Describe/edit)** | A NATS queue-group `cmd/model-runner` holds the vendor credential; a validator-injected `capabilityAuthor` requestor rides the shipped async path; bridge stays secret-free. Fork: first vendor credential + paid API calls. Propose-only. | ★★★ | L | 🏗️ building · [design](../../implementation-artifacts/natural-language-weaver-targets-design.md) · next: fix-round then merge |
