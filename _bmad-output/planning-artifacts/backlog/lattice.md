...
| **[Core] `capability-kv` has ≥8 readers, two re-implementing the read inline** | `aiagent/traversal.go:170` and `gateway/rolesanchors` read the authorization surface outside `capabilitykv.ReadAndMerge`; UNVERIFIED whether they apply the reserved-op/provenance/lane gates the Processor does. Absorbs `containsSensitiveRefLiteral` being an advisory substring scan. | ★★ | S–M | 📋 ready · [why](../../implementation-artifacts/authored-artifact-admission-model-design.md) §14 |
