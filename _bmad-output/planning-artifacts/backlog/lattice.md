...
| **[Weaver] A userTask gap's legitimate `inflight_<g>` suppression column raises a standing `error`** | lease-signing deliberately projects `inflight_onboarding`/`inflight_signature` so `gapSuppressed` can pace two human gaps (`lenses.go:495-503`). `staleMark` calls that an authoring bug and alerts `error` on every reclaim — Weaver reports unhealthy. The column is valid for suppression; only trusting it for stale-reclaim must be refused. | ★★ | S | 📋 ready · steward-owned · next run · consumer: Weaver's own health status |
...
| **Async-convergence's retry + timeout legs run in no CI gate, and fail at head** | The three `TestAsyncConvergence_*` sit behind `-tags leaseshortwindow`, which `go test ./...` never sets and `make test-lease-convergence`'s `-run` never selects. Unrun in CI since filed; `_Timeout_FailedThenOneRetry` + `_BoundedRetry_Exhausted` fail at clean `main` — `a failed (timed-out) call must trigger a fresh retry`. | ★★★ | S–M | 📐 awaiting-Andrew · fix built+green+3-layer-reviewed in worktree; §10.3 edit staged UNCOMMITTED · [design](../../implementation-artifacts/async-reply-design.md) · on ratify: merge worktree + contract in one commit |
