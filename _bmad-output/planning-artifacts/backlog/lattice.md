...
| **[Refractor/edge-manifest] `edgeManifestReadGrants` strands `op`, bound over unrelated chains** | Two `edgeCatalog` walks bind `op:meta` over different chains in one domain, so the base producer's stage boundary strands the name and the engine refuses its hop index (≈4 s/event on the BFS). The own-task walk is a distinct authorization basis: give it its own `ReadGrantDomain` (the `domainProvider` precedent) and the base producer indexes — spike-proven. | ★★ | S | 📋 ready · [triage §2](../../../docs/reviews/lattice-designer-triage-2026-09-10.md) |
...
| **[Pkgmgr] An AI-authored `weaverTarget` artifact has no static holder of `missing_*` ⊆ gaps** | `validateWeaverTargetArtifact` defers LensRef resolution by design and the artifact Definition carries no lens, so the new `lint-gap-column-declaration` gate cannot reach this path. Enforcing it needs validation to resolve an already-installed lens's columns — live state, where the artifact checks are pure today. | ★★ | M | ✅ Andrew-ratified 2026-09-11 (Inc 1+2; Inc 3 seq: §11 trigger) · [design](../../implementation-artifacts/weaver-target-gap-declaration-holder-design.md) |
...
| **[lease-signing] Supersede a background check automatically when its successor completes** | A completed check with a strictly-later completed sibling on the same applicant retires itself: a lens projects the pair, Weaver's `directOp` submits the shipped `TombstoneSupersededLeaseServiceInstance`; the op derives six of its seven reads and admits Weaver. History stays at rest as a body-preserving tombstone. | ★★ | M | ✅ Andrew-ratified 2026-09-11 · [design](../../implementation-artifacts/bgcheck-supersession-convergence-rule-design.md) |
| **[Loom] The deadline probe reads absence as rejection past its evidence's lifetime** | The `arm + window < TrackerTTL` invariant covers every replayed marker, not one the server mints late (a >23 h outage or clock jump expires the deadline key and the tracker in one pass), which fails a healthy parked instance with a misleading reason. Close: the fail verdict reads the step's token-pointer epoch and, past `opstatus.TrackerTTL`, alerts + notes the record instead of failing. | ★ | S–M | 📋 ready · [triage §3](../../../docs/reviews/lattice-designer-triage-2026-09-10.md) |
