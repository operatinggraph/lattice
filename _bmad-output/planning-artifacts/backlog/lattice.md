...
| **[Refractor] Two Postgres-target lenses made zero net progress for 3 days while KV-target siblings drain** | 24 lens consumers sit at one in-flight message each, 20–190 s per message: the relation-blind actor walk crosses the `service` type-descriptor hub, and the plain Postgres lenses re-scan their corpus for want of a `RowReader`. Same fire reviews the periodic sweep / lag-poller / rebuild load. | ★★★ | M | 🏗️ building · [design](../../implementation-artifacts/refractor-hub-walk-and-periodic-load-design.md) · next: Inc 1 pattern-scoped actor walk |
