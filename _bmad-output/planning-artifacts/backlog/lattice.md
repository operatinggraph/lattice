...
| **[Refractor] Cold bring-up replay debt reads as hours of lens lag** | After the demo box's fresh-world bring-up (2026-07-26), all 79 lenses + capability reported `consumerLag` ≈ activation-to-head distance (~2000–2500) while every read model was verifiably complete (`read_lease_applications` held both seeded rows at reported lag 2485; manifests/worklists/authz all live). The measured consumer drains ~50 msg/min, roughly one rule at a time, so the public demo Loupe shows YELLOW "80 degraded" for hours after each nightly reset — a false staleness alarm. Ground which consumer the gauge reads vs the path that projected; credit the live path or drain the replay fast. | ★★ | M | 📋 ready · evidence: demo-box health API + Postgres, this date |
...
| **[CI] lease-convergence 15s drain deadlines are under-budgeted** | The 3 `drainUntilConverged(…, 15s)` sites are outliers against the suite's own 30s (and one 60s) convention for the same all-engines drain; under parallel CI load `SensitiveParamEgress_PlaintextAtAdapter` timed out at 16.55s (12.76s locally). Root-cause the tail before rebudgeting — do not just raise it. | ★★ | S | 📋 ready · run [30195021743](https://github.com/operatinggraph/lattice/actions/runs/30195021743) · green on re-run |
