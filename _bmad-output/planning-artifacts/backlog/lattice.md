...
| **Dynamic type taxonomy — an abstract type a lens can label** | `subtypeOf` links between type meta vertices, resolved to a leaf-label set at activation, so a leaf declared by any package is picked up by lenses labelling the abstract type with no lens edit. Recovers the polymorphism the label-binding fire removes; first consumer is `capabilityServiceAccess`, unnarrowable today. | ★★ | M–L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/dynamic-type-taxonomy-design.md) · fork: write-side guards |
