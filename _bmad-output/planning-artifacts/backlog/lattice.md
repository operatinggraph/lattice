...
| **[privacy-base] A shred erases the credential→person link but leaves the index vertex naming both** | `ShredIdentityKey` tombstones every `boundTo` link because it names in plaintext which credential belonged to whom; `vtx.credentialindex.<hash>` carries the same `{actorKey, identityKey}` and is in no erasure set. The correlation survives, decrypt-free. | ★★ | S–M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/credential-binding-plane-lifecycle-design.md) Inc 1 · fork: what a shred erases |
| **[identity-domain] A provisioned raw actor has no `credentialindex`, so its sign-in method is unlistable** | `ProvisionConsumerIdentity` writes the identity vertex and `.idpBinding` only — no index vertex, no `boundTo`, no `credentialBinding`. So a Scenario-B person's only credential is invisible to `identityCredentialBindingsRead`, and no reconcile reaches it. An incomplete list is harder to notice than an empty one. | ★★ | M | 📐 awaiting-Andrew · folded as Inc 3 of [design](../../implementation-artifacts/credential-binding-plane-lifecycle-design.md) · self-binding refused |
...
| **[Refractor] A sweep pass spans a rule swap, so actors in one pass reproject under two rules** | Each `Reproject` takes its own rule snapshot (`sweep.go:391`), so a hot-reload mid-pass leaves earlier actors converged under the old rule and later ones under the new. Each actor's own row stays coherent and the next pass re-runs, so this is a granularity question, not a torn read. | ★ | S | 📋 needs design · consumer: a MATCH edit landing during a sweep of a large actor set |
| **[Refractor] A structural pause outlives its own cause with nothing to re-probe or announce it** | The column-convergence path repairs the table at the next activation, but a consumer paused before that stays paused — sticky by design, with no re-probe and no alert. Live: `leaseApplicationsRead` dead ~23 h after its cause was gone, 10,317 events stranded. | ★★ | S–M | 📋 ready · consumer: LoftSpace applicant view, verticals.md |
| **[Weaver] A gap's retry-budget exhaustion has no lens-readable signal** | The per-gap dispatch-count lives only in `weaver-state`, excluded from every lens by design (`lease-signing/lenses.go:489`). No read model distinguishes "retrying" from "exhausted". | ★ | S–M | 📋 needs design · consumer: loftspace-app stepper, verticals.md |
...
| **[cap-read] §6.14 authz anchors are untyped bare NanoIDs** | The primordial self-grant gives an identity its own bare NanoID, and `authz_anchors` carries building/unit/lease NanoIDs in that same flat namespace — so an identity whose NanoID equals a non-identity anchor reads that anchor's rows. Live: a JIT-provisioned `vtx.identity.A9jn…` with no `manages` link read all 11 lease applications of `vtx.building.A9jn…`. | ★★ | M | 📋 needs design · consumer: any posture where the IdP subject shapes the provisioned NanoID |
