...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **gateway-revocation-kill-switch-activation** | Kill-switch dormant — nothing populates a revocation set, no admin surface revokes, a failed bucket-open downgrades to verification-only (default-open, Health-KV-visible). Andrew-steered mechanism (`RevokeActor` op → outboxed event → Gateway-owned local KV, fail-closed) in the design doc. | ★★ | M | ✅ ratified (2026-07-03) · [design](../../implementation-artifacts/gateway-token-revocation-activation-design.md) · no contract change; 2 fires; unblocks loupe F11 |
