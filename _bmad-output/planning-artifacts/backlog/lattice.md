...
| **[Processor] A script `update`/`tombstone` wipes the rest of the document** | Step 8 writes the whole script-supplied document and re-stamps only `lastModified*`; `VertexDoc` never exposes `createdAt`/`createdBy`/`createdByOp`, so no script CAN preserve them (Contract #1 §1.3: immutable). A tombstone is worse — `{isDeleted, data}` leaves a link with no `class`/`sourceVertex`/`targetVertex`, unreadable as a link. Hit live by the showcase recovery; also why a revived package entity cannot recover its creation provenance. | ★★★ | S–M | 📋 ready · found fixing Wire revive |
