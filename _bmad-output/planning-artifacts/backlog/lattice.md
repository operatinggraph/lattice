...
| **[Edge] 61 orphaned SYNC durables have no host that can reach them** | Measured 2026-08-01: 74 durables for 13 identities, all but three pinned at the 10k per-subject cap (~610k phantom pending, reads as lens lag in Loupe). New ones stopped, but each orphan is named by a device id its host no longer holds, so nothing but an operator sweep can delete them. | ★★ | XS | 📋 ready · one-off admin delete of every `edge-sync-*` with no live holder |
