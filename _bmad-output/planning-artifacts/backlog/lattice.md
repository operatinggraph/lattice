...
| **`internal/bootstrap` primordial-ID globals race** | `populate()` (nanoid.go) writes ~64 package-level globals per call; `SetupPackageTestEnv` calls it per-test, so `t.Parallel()` races (confirmed `-race`). Blocks parallelizing lease-signing/clinic-domain/identity-domain tests. | ★★ | M | 📐 awaiting-Andrew · [design](../../implementation-artifacts/bootstrap-primordial-globals-race-design.md) · test-scoped fix recommended, universal refactor rejected w/ revive condition |
