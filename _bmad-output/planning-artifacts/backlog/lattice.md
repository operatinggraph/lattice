...
| **[lease-signing/Loom] A superseded background check keeps its instance — 12k accumulated on seven identities** | A check superseded by its successor stays a live instance; `leaseApplicationComplete` scans all N per event (3,637 on one identity, ~30 s), so its rebuild drains at ~2 msg/min. Rule: the reply op tombstones the prior instance; the one-off purge needs an op the service DDL lacks. | ★★★ | M | 📐 needs designer pass · no-pattern: supersession of a service instance by its successor · [design](../../implementation-artifacts/bgcheck-runaway-and-broad-filter-design.md) §2 |
...
| **[Refractor] `edgeInstances` is licensed and derives, yet pays ~15 s per pattern-bound event** | The derivation names one actor per service/`providedTo`/`.outcome` event; the whole-actor recompute + keyset frame then costs 10–22 s, so it drains at 0.13 msg/s (96 k backlog) while `edgeCatalog` drains the same events at 25 msg/s. | ★★ | M | 📐 needs designer pass · no-pattern: sub-actor incremental reprojection for a personal lens (the keyset frame is whole-actor by ratified design) · [measured](../../implementation-artifacts/personal-lens-derivation-licence-design.md) §15.8 |
