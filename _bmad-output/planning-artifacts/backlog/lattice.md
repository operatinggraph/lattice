...
| **[Refractor] Personal Lens rows never retract** | No stored key-state to diff against (`NatsSubjectAdapter`), so a stale multi-row anchor (e.g. a completed task) never gets a `Delete`, live or via cold `Hydrate`. Needs a design call on where reconciliation state lives. | ★★ | M | 🏗️ designing (Designer fire 2026-07-22) |
