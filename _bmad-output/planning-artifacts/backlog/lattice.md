...
| **[privacy-base] A shred erases the credential→person link but leaves the index vertex naming both** | `ShredIdentityKey` tombstones every `boundTo` link because it names in plaintext which credential belonged to whom; `vtx.credentialindex.<hash>` carries the same `{actorKey, identityKey}` and is in no erasure set. The correlation survives, decrypt-free. | ★★ | S–M | 📋 needs design — erasure becomes a Loom pattern (Andrew 2026-08-06) · [why](../../implementation-artifacts/credential-binding-plane-lifecycle-design.md) |
...
| **Dynamic type taxonomy — an abstract type a lens can label** | `specializes` links between type meta vertices, resolved to a leaf-label set at activation, so a leaf declared by any package is picked up by lenses labelling the abstract type with no lens edit. Recovers the polymorphism the label-binding fire removes; first consumer is `capabilityServiceAccess`, unnarrowable today. | ★★ | M–L | 📐 awaiting-Andrew · [design](../../implementation-artifacts/dynamic-type-taxonomy-design.md) · fork: write-side guards |
