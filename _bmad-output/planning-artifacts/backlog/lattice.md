...
| **Natural-language Weaver targets — model-runner service + real capability-author adapter (NL-1)** | The shipped authoring pipeline gains its brain: a NATS queue-group `cmd/model-runner` (sole vendor-credential holder; the parked vendor-adapters direction) + a thin validator-injected `capabilityAuthor` requestor on the shipped async path; bridge stays secret-free. Fork: first vendor credential + paid API calls. Propose-only. | ★★★ | L | 🏗️ building · [design](../../implementation-artifacts/natural-language-weaver-targets-design.md) · next: NL-1 runner + adapter |
