...
| **[Weaver] `GapWithoutPlaybook` long-Naks a column no playbook will EVER name** | A column a package projects deliberately with no gaps entry holds a pending slot and re-runs the preamble every long floor forever — §4.2's unregistered-target argument, never applied to row 8. Fix: declare orphan columns `surface` + gate lens `missing_*` ⊆ gaps. | ★★★ | M | 🏗️ owner: claude/great-lamport-f62lcy · [why](../../implementation-artifacts/weaver-decline-retry-substrate-native-design.md) §3.2 · next: surface-gap declaration + ⊆ gate |
