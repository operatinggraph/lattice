...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[rbac] A second grant channel mints capability grants outside the package plane** | `CreatePermission`/`UpdatePermission`/`GrantPermission` are `scope:any` to operator with no allow-list, so a grant no manifest records and no uninstall retracts is indistinguishable at step 3 from a package one — reaching `ShredRetentionClassKey`. Fix is provenance, not a deny-list. | ★★ | M | ✅ ratified Branch A (2026-08-13) · low priority · un-tombstone prerequisite CLEARED 2026-08-14 · [design](../../implementation-artifacts/grant-provenance-runtime-permission-minting-design.md) |
