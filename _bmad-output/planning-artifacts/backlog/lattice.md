...
| **[Edge] The first-paint gate has no identity for the hydrate cycle it gates** | Two defects: `hydrationComplete` carries no `Lens`, so the gate releases on the first marker and can paint a partial world; and `personal.hydrate` is per-**identity**, so a second device's burst satisfies this device's gate. The client-side fix was built and refuted. | ★★ | M | 🏗️ designing (Designer) · no-pattern: per-hydrate correlation-id + gate-keying · [why](../../implementation-artifacts/edge-cold-signin-delivery-position-design.md) §6 Fire 4 |
