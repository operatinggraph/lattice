...
| **[Pkgmgr] `OpMetaSpec` has no vocabulary for a client-mint-and-reveal-secret ceremony** | identity-domain's `[no-op-meta:]`-exempt ops are undiscoverable because their submission is a client-side ceremony, not a fillable form — a package-derived index key, a client-minted secret, a raw-credential actor. | ★★ | L | 🏗️ [design](../../implementation-artifacts/client-ceremony-op-descriptors-design.md) §14.4 · next: Inc 2b — backfill + pane section + descriptor |
...
| **[identity-domain] A credential whose identity vertex was never provisioned projects no binding row** | `identityCredentialBindingsRead` anchors on `(c:identity)`; `seedNodes` lists the `vtx.identity.` prefix and `readNode` returns nil for a missing key, so the row silently never appears while `identityCredentialsRead` still lists the credential from the decrypted array. Reachable via `lattice identity claim --actor`, which skips the Gateway's provisioning pre-flight. | ★★ | S–M | 📋 needs design · fork: provision in the CLI vs re-anchor the lens · binding in Inc 2b (un-listable ⇒ un-unlinkable) |
