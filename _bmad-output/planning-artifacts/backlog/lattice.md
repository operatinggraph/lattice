...
| **[Substrate] A missing or malformed `CredsFile` retries like a transient stall** | `permanentConnectError` recognizes the NKey/Token auth sentinels only; `CredsFile`'s validation is a callback nats.go invokes lazily inside the dial, so a bad path or malformed creds is retried `connectAttempts` times before erroring, unlike `NKeySeedFile`'s eager pre-flight. | ★ | XS | 📋 ready · consumer: a misconfigured `CredsFile` path · mirror `NKeySeedFile`'s pre-flight in `conn.go` |
...
| **[Refractor] One taxonomy event fans out into a durable delete-recreate per affected lens** | `supervisor.Reset` deletes and recreates the lens's `KV_core-kv` durable, applied per entry whose set moved — measured 118 consumers, 111 one-per-lens. Has OOM-killed `lattice-nats`. Also owns rebuild ORDERING: a shrink installs the narrowed filter before the replay, so nothing retracts the dropped subtype's rows. | ★★★ | M | 📋 ready · **gates Fire B** · coalesced scheduler + shrink ordering · [why](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.8 |
...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **Dynamic type taxonomy — an abstract type a lens can label** | `subtypeOf` links between type metas, resolved to a leaf-label set at activation, so a leaf any package declares is picked up by lenses writing `:abstract*`. Recovers the polymorphism the label-binding fire removes; first consumer `capabilityServiceAccess`. | ★★★ | L | 🚧 Fire A shipped · [design](../../implementation-artifacts/dynamic-type-taxonomy-design.md) §17.12 · Fire B blocked-on the rebuild fan-out row |
