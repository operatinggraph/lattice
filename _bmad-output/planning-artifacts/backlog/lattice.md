...
| Item | What it is | Imp | Size | State |
|---|---|---|---|---|
| **[Refractor/Weaver] Expiry is a fact on the CHECK, not a clock in the lens — sweep the `$now` lenses** | 16 lenses read `$now`: stored rows are claims about a past instant, the sweep cannot tell a broken projection from a moving clock, and the plain derivation keeps the Postgres pair's corpus rescan. Fix: the fired timer's marker records the lapse per target and the lens reads it; the check instance becomes its own anchor. | ★★★ | L | 🏗️ building · owner: steward-lattice-expiry-fact · [design](../../implementation-artifacts/expiry-as-a-recorded-fact-design.md) · next: Inc 0 premises + 4-deep pin |
