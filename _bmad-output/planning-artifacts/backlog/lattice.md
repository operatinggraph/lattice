...
| **[platform] A payload-named subject reaches an external adapter with no engine guard on 7 `Scope:"any"` ops** | Seven engine-dispatched ops forward a payload-named key off-platform in an `external.<adapter>` event after shape + liveness checks only — any `operator`-role holder, not just the dispatching engine, can name an arbitrary subject. | ★★ | M | 🏗️ building · [design](../../implementation-artifacts/declared-read-scope-authorization-design.md) §12 · Inc 1 primordialActor global
...
| **[bootstrap] `UpgradePackage` accepts unvalidated mutations against permission/role vertex classes** | No DDL registers canonicalName `permission`/`role`, so step 6 validation is skipped for those classes and `UpgradePackageDDLScript` full-body-replaces any `vtx.*` past a shape check alone. `consoleOperator` ("does not confer root") holds it at the meta lane — a live, non-root escalation. | ★★★ | S–M | 📐 needs designer pass · found by grant-provenance close review 2026-08-14 · [why](../../implementation-artifacts/grant-provenance-runtime-permission-minting-design.md) §5.1 |
