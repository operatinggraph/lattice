...
**Result: Hello Lattice M1–M6 all pass; Gate 5 marker `passed:true`, `milestonesPassed:[1,2,3,4,5,6]`.** Open (non-blocking) follow-ups: tasks/services still assume root `data` (no DDL yet — decide aspect-vs-root when a task DDL is authored); the events-publish redelivery path still uses best-effort `RebuildEventListFromClasses` — Andrew wants a transactional **outbox** (persist the real script-returned EventList on the tracker vertex, publish via a durable consumer) eventually, NOT reconstruction from KV keys.
