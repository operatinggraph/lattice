...
| 1.5.7 | Contract conformance suite + reply-constraint freeze | ✅ SHIPPED (CI green) | `2fddd89` |
| 1.5.6 | Re-enable Hello Lattice M4+M6 (M5 deferred) | 🟡 SUPERSEDED — shipped partial (M1–M4+M6); M5/Gate 5 closed by 1.5.8/1.5.9 | `a6edcb0` |
| 1.5.8 | Capability-lens aspect CDC fan-out | ✅ SHIPPED (CI green) | `65551de` |
| 1.5.9 | Lens property model (data envelope + aspect navigation) → close M5 + Gate 5 | ✅ SHIPPED (CI green) — **Gate 5 full pass M1–M6** | `faac59b` |
| (rename) | Link envelope `youngerVertex`/`olderVertex` → `sourceVertex`/`targetVertex`; drop `CanonicalLinkOrder` | ✅ SHIPPED (CI green) | `0d4d042` |
| 1.5.10 | **Transactional event outbox** (pre-Phase-2 hardening — GATED Phase 2; faithful EventList persisted as `vtx.op.<id>.events` in the step-8 atomic batch with no TTL, durable outbox-only consumer publishes + tombstones; removed `RebuildEventListFromClasses`/`maybeRepublishEvents` + dead step-9 scaffolding) | ✅ SHIPPED (CI green) — CS→DS→CR adjudicated | `d6dba05` |
| 1.5.11 | **Relocate publisher to outbox + renumber commit path to 9 steps** (move `step9_publish.go`→`outbox/publisher.go`; ack 10→9; delete vestigial `EventPublisher` iface; `steps_4_10_stub.go`→`step_interfaces.go`; docs/architecture/prd → 9-step + async-outbox note; no behavior change) | ✅ SHIPPED (CI green) — CS→DS(Sonnet)→CR adjudicated | `9c3bd57` |
...
**1.5.7 RESOLUTION (shipped):** `OperationReply.Detail map[string]any` REMOVED. Commit-trace identifiers → typed `OperationReply.PrimaryKey string`, **enforced PRE-COMMIT** (`primaryKeyInCommit` in `commit_path.go`, after step-6 Validate, before the atomic batch) to lie within the operation's write footprint — a mutation key OR the 3-segment vertex root of one (so aspect-only ops ClaimIdentity/UpdateRole/UpdateMetaVertex return their natural entity key, not an internal aspect). Violations `Term` with `ErrCodeDDLViolation` and never mutate Core KV / publish events (Andrew caught a DS post-commit placement = write-then-fail bug; Winston relocated pre-commit + added a no-Core-KV-mutation conformance assertion). Closed `response` script-return schema = `{primaryKey}` only; any other key fails closed at parse (`ErrCodeScriptFailed`). Read-derived signals (`possibleDuplicateFlag`/`alreadyAssigned`/`alreadyGranted`) DROPPED from the reply (duplicate rides `IdentityCreated.data.duplicate`). **claimKey = Option C (Andrew's call):** the client mints the plaintext, computes sha256, and submits `claimKeyHash`(+`claimKeyAlgo`) in the op payload; `CreateUnclaimedIdentity` stores it verbatim. Plaintext NEVER enters Lattice (not the persisted ops stream, not the reply) — no `OneTimeSecret`, no `secret.mint()` builtin, Processor stays generic. `ScriptError.Detail` (ClaimKeyInvalid Health-KV side-channel, stripped before egress) left intact. New conformance suite (`internal/processor/conformance_test.go`, `make verify-conformance`) freezes the reply field set (asserts no `detail` on the wire), the closed response schema, the pre-commit enforcement (no Core KV mutation on violation), the root-fallback rule, Core KV key shapes, and the DDL aspect set. New **Contract #9** (identity claim flow, docs/contracts). CR: 0 P0 / 0 P1 / 2 P2 (both fixed: stale comment; CLI now surfaces the claim key only on `accepted`, not on `duplicate`).

**1.5.6 note (SHIPPED AS SPLIT — Andrew's call):** Re-enabled Hello Lattice **M4** (Lens→Postgres; fixed by provisioning the `books` table OUT OF BAND in the test harness — NO auto-DDL, per the Materializer design: missing schema → structural-pause; confirmed in the Materializer repo brainstorming) and **M6** (DDL rollback; 1.5.2 tombstone coherence — fixed the stale assertion to `ErrCompensationAspectMissing`). Added a **honesty gate-5 marker** (per-milestone sentinels; certifies `passed` from actual results; a `milestonesDeferred` set keeps it partial without failing the suite). **M5** (AI-agent capability traversal): its blockers were (a) `NoDDLForClass` — FIXED by 1.5.5; (b) missing RBAC `ContextHint.Reads` for GrantPermission `[permKey, roleKey]` / AssignRole `[actorKey, roleKey]` — FIXED; (c) the **capability-lens link fan-out gap** — FIXED (the cap lens now reprojects affected actors on `holdsRole`/`grantedBy` link CDC, create + revoke; `internal/refractor/pipeline/{pipeline,evaluate}.go`, idempotent `adjacency.Build`-before-reproject to win the race vs the adjacency consumer; `refractor_capability_linkfanout_e2e_test.go`). M5 step 5 rewritten to drain-then-time (functional convergence + NFR-P3 best-of-3 ≤500ms probe). BUT M5 remains **DEFERRED**: capability-lens consumer *delivery* is starved (2.75s–15s+, variable) by the **events-publish `atomic publish is disabled` storm** (NATS err 10174) — step 9 `substrate.PublishBatch` (atomic) to `core-events`; stream sets `AllowAtomicPublish:true` but the server rejects it → 3×retry+nak/redeliver on every op saturates NATS. NFR-O3 metrics show per-event *processing* is fine (p95 486ms); it's *delivery* that's starved. **1.5.8 (next):** make step-9 event publish NON-ATOMIC (events are best-effort post-commit, Contract #4 §4.4, consumed idempotently) → kills the storm → cap lens delivers promptly → re-enable M5 (un-skip in `hellolattice_test.go`, drop `milestonesDeferred[5]`) → Gate 5 full pass M1–M6. Subsumes the earlier events-publish spawn_task. Also-open Refractor follow-ups (NOT blocking): the dead `handleAdjUpdate` fast path (bare-NanoID vs `vtx.<type>.<id>`), and capability consumer per-batch delivery latency.

**1.5.8 + 1.5.9 note (M5 / Gate 5 CLOSED — corrects the 1.5.6 deferral diagnosis):** The 1.5.6 note's deferral cause was **WRONG**. There is no "atomic publish storm": atomic batch publish to `core-events` works on NATS 2.14 (`AllowAtomicPublish=true` verified live; a direct `substrate.PublishBatch` succeeds). The dead `handleAdjUpdate`/events-publish theory is retired. M5's real blockers, found by re-measuring against real Processor-written data, were three layered defects:
- **Aspect CDC dropped (`1.5.8`, `65551de`).** The pipeline acked-and-dropped every `KindAspect` event on all lenses. Aspect-only mutations (identity `.state`, role `.description`) never reprojected. Fix: route `KindAspect` on the actor-aware pipeline through a fan-out seeded from the aspect's parent vertex (`processAspectFanOut`/`evaluateAspectFanOut`), symmetric to the link fan-out, no adjacency mutation. E2E `refractor_capability_aspectfanout_e2e_test.go`.
- **Property-model mismatch — gap (a) (`1.5.9`, `faac59b`).** The capability cypher read flat domain fields (`perm.operationType`) but the Processor stores them under a `data` envelope (`perm.data.operationType`), so an operator-role agent projected `platformPermissions=[{null,null}]` → M5 never functionally converged. The fan-out E2Es passed because their fixtures wrote domain fields flat — masking it. Fix: cypher reads `perm.data.*`; all capability fixtures made faithful (domain fields under `data`).
- **NFR-P3 latency — unanchored scan (`1.5.9`).** The `ephemeralGrants` clauses used an unbound `(task:task)` seed → the full engine scanned the entire Core KV bucket on **every** reprojection (`seedNodes` generic path). Anchoring on the bound `identity` dropped per-event projection latency from **~700ms–1.7s to ~85ms** (NFR-P3 ≤500ms now met).
- **Property-model gap (b) — explicit aspect navigation (`1.5.9`, Andrew's call).** Business data lives in **aspects** (`vtx.<type>.<id>.<localName>`, value under `data`), which the engine's `fetchNode` never read. Andrew's model: vertices walk links, aspects hold data, and cypher must be explicit. Engine now resolves `node.<aspect>.data.<field>` — a name absent from the root body is point-read as the aspect key (present→root, absent→aspect; unambiguous, point read not scan; `executor.go` `resolveProperty`). `capabilityRoleIndex` reads `role.canonicalName.data.value` and now resolves real role names in production (`CreateBook→[operator]`). **Permissions keep business data on the vertex root `data` (Andrew's call) — the deliberate exception.** Doc: `docs/components/refractor.md` "Property model".

**Result: Hello Lattice M1–M6 all pass; Gate 5 marker `passed:true`, `milestonesPassed:[1,2,3,4,5,6]`.**

**Carries PROMOTED to planning artifacts (2026-06-01).** These previously lived only here + in agent memory — which the Phase 2 bmad planning sprint does NOT read. Now recorded in `epics.md` (and the outbox in `lattice-architecture.md`) so planning ingests them:
- **Events outbox → Story 1.5.10 (pre-Phase-2 hardening, GATES Phase 2)** per Andrew (fix now before Loom/Weaver, not deferred). Persist the script-returned `EventList` in the step-8 atomic batch (on the `op` tracker vertex); a durable consumer publishes from there, acking on confirmed publish. Removes best-effort `RebuildEventListFromClasses` (reconstruct-from-KV-keys). `core-events` are intentional declared-schema events, not CDC.
- Added to `epics.md` "Phase 2+ Deferred Architectural Capabilities": **read-from-body-not-key** NATS discipline (Andrew binding — adjacency consumer + pipeline fan-out re-parse the key; body is source of truth; also gives `sourceVertex`/`targetVertex` their first reader); task/service root-`data`-vs-aspect placement (decide at first task DDL); Refractor 42703 `undefined_column` structural-pause check; F-004 package version upgrade; UninstallPackage per-key OCC.
