...
**Amended at build (2026-09-04), then reversed the same day by the fix-round review:** the first ruling treated a fault behind a definitive DDL as immaterial (`fault != nil && !ok`, step 6.5's precedent). That is a fail-open here: `classOf` continued the walk past a faulted hop, so a transient read error on the CLOSER authority (whose `permittedCommands` refuses) let a FARTHER one (which admits) decide. The rule of record: **a fault at any hop of the stored-class or declared-class walk stops the walk and is retryable** — the closer authority is unknown, so no later hop's answer can stand in for it; `classOf`'s fault breaks the walk exactly as `instanceOfTargetOf`'s does, which makes `fault != nil` and `!ok` coincide. The absent-at-5.5 root is re-read at commit: `Commit` treats a handed entry as covering a guard's root only when it was FOUND, since an absent key is not revision-conditioned and a root created protected inside the window must still be seen by the backstop. A tombstone over an undecodable body writes `isDeleted` with nothing readable to preserve — the creation triplet is re-stamped from the tombstoning op, the one shape where "preserved whole" has nothing to preserve — and the `buildMutationValue` comment says so.
...
- **(amended at build, 2026-09-04, Inc 3 review)** a `create` at a member key is held to the same seeded-shape rule as an update (a non-seeded-shape create at an absent kernel link key is refused; a seeded-shape one is left to the create-only conflict, or re-establishes the edge — the hard-delete heal). The seeded-shape comparison is a **closed whitelist**: the written document's top-level keys are exactly the six the revive emits (`class`, `isDeleted`, `sourceVertex`, `targetVertex`, `localName`, `data`), anything else refuses; `data` is left uncompared because the revive carries the grant-edge provenance stamp in it and no platform guard governs a link's `data` — every reader of a kernel link today classifies by key and reads only `isDeleted`. `KernelGrantLinkKeys` refuses on any unloaded kernel permission id, and `KernelTopologyLinkKeys` on any unloaded identity id, so no composed key can carry an empty segment.
...
- **Amended at build (2026-09-04, Inc 2 review): the reads are split by consumer.** `ReadPrior` at step 5.5 reads only the **mutation keys** (what the stored-class gate needs); `Commit` reads the **root entries** the step-8 guards need (`protectedRootKey` of every aspect mutation) fresh at commit time, plus the top-up. The prior pass's root snapshot would otherwise be three stages older than today for a key the batch never conditions — a root flipped to `data.protected: true` during step 6.5's Vault round trips would not be seen. Net reads per attempt stay at today's count for every key the pass FOUND; a handed entry covers a guard only when it was found — an absent key is unconditioned, so `Commit` re-reads it (one extra read per absent key, the price of seeing a root created protected inside the window). `classOf`'s committed answer is memoised per node beside the `instanceOf` layer, so a cascade sharing one terminal reads it once. The fallback revision `conditionRevision` takes for a non-hydrated key now comes from the 5.5 read, so such an update conflicts (and retries) across a wider window than before — stricter, never weaker.
...
- **Landed on `main`:** Inc 1 `4f43d19d` · Inc 2 `160d9d6f` · Inc 2 fix round 2 `2307a512` · Inc 3 `e23fa10f` · Inc 3 fix round `591045d2` · close-pass round `d9db9deb`. Kernel reseeded (`verify-kernel: ALL ASSERTIONS PASSED`), the five touched packages diff-applied live (wellness 0.22.21, cafe 0.11.31, lease-signing 0.31.25, privacy-base 0.15.8, edge-manifest 0.17.9 — the last a lint-forced, semantically inert bump: `lint-package-version` triggers on any non-comment edit under `internal/pkgmgr/`, a recorded deliberate coarseness), Processor cycled from `d9db9deb` (boot log: `kernelTopologyLinks=12`). Worktree `../lattice-wt-stored-class` retired.
- **Close-pass classification (seven cold reviews over the item):** *design-gap* ×7 (Processor) — the corrupt-body heal path, the hoisted root snapshot, the fault-before-ok order (ruled wrong once, then reversed), `committedOnly` one hop short, the unstated platform exemptions, the absent→found lost race, the kernel-typed cascade cost; *implementation-bug* ×1 — a create judged on the written rather than stored shape; *census* ×2 — the uncorrected-command re-run (15 sites, not 8) and a runtime premise closed over the authoring corpus; *test* ×8 — five revertible-with-suite-green mechanisms in one increment, a fixture shape the platform never emits, a hand-copied field list, a test-owned floor; *convention* ×9 — history narration, stale comments, a mis-attributed stage in a log line, four privacy-base comments asserting the closed gap, a stale §10 residual, a §2.3 sentence naming constants the code cannot use; *brief-gap* ×1 — a diff range that swept four unrelated commits; *review-over-reach* 0. Dossier routed: Processor entry 10 (corpus-vs-runtime + corrected-command) and entry 3 (seventh sighting, the mandated report shape) — `docs/components/processor.md`.
- **Residuals of record:** §4's close paragraph (the lost-race conflict; the bounded off-budget walk; the inert raw-submit `instanceOf` shape). Package-scoped `permittedCommands` matching and the epoch pre-check stay lines, not rows.
- **Live posture:** the dev stack was idle across the fire (two committed operations in 90 minutes), so the absence of refusals in the Processor log is not evidence; the refusals are proven by the embedded e2e against a real seeded kernel (`packages/rbac-domain/kernel_link_protection_test.go`) and the two package suites (§6.2). A destructive live `RevokeRole` probe against the shared stack was deliberately not run unattended.
