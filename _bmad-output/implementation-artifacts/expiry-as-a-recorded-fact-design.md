...
- **Inc 3 built (`856fbe16`) + cumulative close pass run** (1 blocking / 2 major / 7 minor; fix round in flight). The blocker: the clinic reminder gate's recorded-end term depends on a `pastDueAppointments` marker that lens never writes for `completed`/`noShow` rows — the two lenses' terminal-status sets must agree (fix: shared exclusion set + vectors). Accepted, recorded here: the deploy-time overdue-`@at` burst is one `MarkExpired` per anchor with a past deadline across all ten targets (not only the 12,294 bgcheck instances); `make test-unrouted-convergence` hand-writes its row and never reaches the converted cypher — the conversion's evidence is the per-lens cypher vectors, the sweep-verdict pin and the lease harness; one embedded-NATS handshake flake seen once in a wide run, isolated green.
- **Inc 3 + Inc 4 LANDED on `main`** (`322a3eca`, `a2dfd339`, `d6960bda` — the branch rebased onto main and fast-forwarded; close-pass fix round closed the blocker (shared `nonTerminalAppointment` fragment across the two clinic lenses, pinned on the shipped specs), restated the reminder bound (dispatches bounded by the retry budget, the standing issue by the recorded end via `retireClosedGapIssues`), stated the marker's `optionalReads` obligation in the DDL schema, and spliced every target id from its constant). Live 04:35: clinic-reminders 0.10.9, wellness-reminders 0.3.6, orchestration-base 0.7.18, lease-signing 0.31.21, semantic-contracts 0.4.7 diff-applied; the three vertical apps restarted; the first minute armed ~1.7k overdue `@at`s across seven targets (the sized deploy burst); no sweep content divergence on `unroutedTasks` / `staleAssignedTasks` / `renewalComplete` since. **Every `$now` is gone from the fourteen in-scope declarations**; the two that leave the design keep theirs. The frozen-contract illustration in `10-orchestration-weaver.md` rides this landing (a clause the runtime now has). Follow-on rows: the N-rows-per-anchor plain derivation (📐, lattice), `capabilityEphemeral` (📐, lattice).
