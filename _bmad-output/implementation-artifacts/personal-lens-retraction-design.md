...
**Status: ✅ Andrew-ratified (2026-07-22).** Build runs through the Lattice lane (fires R1 → R2, §7). R1 ✅ SHIPPED (2026-07-22); R2 ✅ SHIPPED (2026-07-22) — both fires closed. · Author: Winston (Designer fire, 2026-07-22) ·
...
- **R1 [lattice · Refractor] — frame production. ✅ SHIPPED.** Envelope `lens` field; `keyset` op + `KeySetPublisher` on `NatsSubjectAdapter`; pipeline emission per enumerated actor after successful write (live + hydrate), empty-frame and identity-tombstone/missing-actor cases (removing the malformed cap-key redelivery loop); `personal.hydrate` response gains `lenses`; Refractor e2e vectors; `docs/components/refractor.md` update. Green alone: old clients ack-and-ignore frames (pinned today in `sync.go:406-408`), so R1 ships with zero client risk. A same-fire adversarial pass found and closed two more must-fixes: `emitPersonalFrames` now takes the SAME adapter instance `writeResults`/`Hydrate` already captured rather than re-resolving `currentAdapter()` (closes a `HotReloadInto` race that could split a write from its frame across two adapter instances); `Reproject` now refuses a `KeySetPublisher`-backed adapter explicitly (closes a fail-loud→fail-silent regression, currently unreachable via DDL routing but no longer relying solely on that for safety). Consumer named and waiting: R2 + the blocked verticals claim beat.
- **R2 [lattice · Edge client] — frame consumption. ✅ SHIPPED.** `Store.ApplyKeySet` + `Sources` attribution + `frameHW` guard + schema-version purge, both engines (bbolt + IndexedDB) + the shared conformance suite; sync-manager `keyset` handling + hydrate lens-set prune; `cmd/facet`'s SSE snapshot burst + `internal/edge/browser.Host.Snapshot` both proven to exclude a retracted row. Proven headless: bolt conformance, the sync package's unit suite, and the real headless-Chrome IndexedDB gate (`make test-edge-idb-conformance`) all green — the mirror-level retraction bar this design owns (§5, minus the live two-device demo, which is the verticals lane's F5 to run once unblocked, per §5's own scoping note). A same-fire adversarial pass (independent reviewer, read-only) found and closed two must-fixes, both folded into the code + conformance suite: **(1)** the IndexedDB engine's `lens=""` upsert path routed through the shared `applyDelta` helper, which overwrites with a wholly fresh `Entry` and silently dropped any `Sources` attribution an earlier attributed upsert had recorded — bbolt's equivalent path already preserved it via `next := cur`; fixed by giving IndexedDB the same cur-preserving path, closing an engine-parity gap no vector had caught. **(2)** `ApplyKeySet`/`PruneDeadLensAttributions` tombstoned an entry (`Deleted = true`) without clearing `Data`, unlike `ApplyDelete`'s fresh-Entry shape — the retracted row's last-known plaintext body stayed resident in the local store under the tombstone, against the design's own hygiene intent; fixed in both engines to clear `Data` on tombstone, matching `ApplyDelete`. No blocker found beyond these two; every other guarantee (frameHW resurrection guard, two-pass bbolt collect/mutate split, IndexedDB transaction chaining, same-key multi-lens refcount, schema-migration purge/no-double-purge) held under attack.

Both fires shipped 2026-07-22 — the blocked ★★★ verticals staff-worlds row (`verticals.md`) is unblocked; its claim beat can now be live-proven.
