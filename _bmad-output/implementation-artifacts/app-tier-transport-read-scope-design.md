...
| G5 | The four vertical apps' entire publish grant is `["$JS.API.>", "$JS.ACK.>"]` (+ `$O.core-objects.>` and the two vault RPCs for loftspace) | `matrix.go:490-520` |
...
| `TestAppTierHoldsNoAckGrant` | I1 | each app is denied on `$JS.ACK.>`, proven by a `+NXT` publish against a live pull consumer failing — the `+NXT` read primitive, not just the absent grant string |
