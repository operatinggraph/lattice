...
### 3.4 The Protected worklist pane (FORK-S1 B — cross-identity, PII-correct; descriptor-driven per facet-discovery-restoration-design.md)

**Amended 2026-07-27 (Andrew-directed covenant restoration):** the pane's read posture below stands, but the app no longer hardcodes its tables, columns, or sections. A pane is a **`pkgmgr.PaneSpec` descriptor** — a `meta.pane` vertex whose `.paneDescriptor` aspect carries the section list (table, PHI-narrowed column list, filter, ordering, dispatch target) as package DATA, delivered to exactly the identities whose `holdsRole → offeredTo` topology earns it via the `edgeStaffPanes` lens (`manifest.pane.*` rows). `cmd/facet` runs ONE generic grammar-validated pane executor (`GET /api/pane`; same DSN, same SELECT-only role, RLS as the signed-in staff actor — the `credentials.go` spine); the FE renders the **Worklist screen archetype** from the descriptor's column roles. A new staff workflow is a descriptor + lens edit, zero app change; `scripts/lint-facet-discovery.go` blocks the hardcoded alternative structurally.

- **Front desk (the shipped `staffWorklist` descriptor, `packages/edge-manifest/panes.go`):** *Applications to review* ← `landlordLeaseApplicationsRead` (pending-decision rows; `applicant_name` decrypts at projection only into rows the reader is granted); *Today's schedule* ← `clinicAppointmentsRead` (per-row `patient_name`, day-filtered; the clinical-content column ban is pinned by `panes_test.go` beside the descriptor it governs); *recurring visit series* ← `visitSeriesRead` (facet-entity-browse-design.md §9); *unified resident context on lookup* ← the front-desk-\* NATS-KV buckets + `cafeTabSettlement` (host-level natsperm reads, same as `cafe-app`; still unbuilt, named consumers).
- **Operations:** occupancy + service-attach-rate (portfolio-pulse), provider utilization (`providerAppointmentsRead` aggregated host-side), vacancy (`availableListings`). Aggregates only — the demo card's own caption is the spec; when built, these are further pane descriptors, not new handlers.

Offline shows the pane unavailable while the manifest half keeps working. `clinicPatientsRead` (wildcard-only roster) is deliberately **not** in v1: the schedule rows already carry the day's patient names; the full-roster switcher stays a clinic-app/operator surface until a scoped anchor model for rosters exists.
