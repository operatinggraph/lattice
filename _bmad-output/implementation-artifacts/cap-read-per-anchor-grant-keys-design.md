...
**Owning components:** `internal/refractor/{projection,pipeline,adapter,capabilityread,keyshredded}` (mechanism), `internal/bootstrap/lenses.go` + `internal/pkgmgr/anchorwalk.go` (producers), `packages/edge-manifest` (+ any package shipping a `cap-read.*` NATS-KV producer). Docs: `docs/contracts/06-capability-kv.md` §6.13/§6.14 (edit staged uncommitted), `docs/components/refractor.md`.
...
**Actor-disappearance, empty-set, and shred — all multi-key now (enumerated, not hand-waved).** `SetActorDeleteKey`'s single-key contract has four consumers that each become a prefix-scoped list-then-tombstone with the same deny-closed partial-failure semantics: (a) the CDC actor-disappearance delete, (b) `emptyBehavior: delete` on a zero-real-entry evaluation, (c) sweep `Reproject` deletes, and (d) the **shred path** (`keyshredded` `NullifyRow(…, MaxInt64)`) — a shredded actor must have **every child key** nullified at `MaxInt64`, not just the parent, or the privacy plane leaks per-anchor grants. Fire 1 lands all four against a prefix-aware delete hook.
...
- **Survey enumeration becomes lens-prefix-scoped.** Today `Sweeper.survey` lists the **whole bucket** per tick (the adapter's `ListKeys` is bucket-scoped and unpaged into memory) — already O(bucket) for every capability-bucket sweeper, and per-anchor keys grow the bucket to O(total grants + revocations). Fire 1 scopes the survey's target listing to the lens's literal `OutputKeyPattern` prefix via the filtered listing, for **all** sweepers (doc-mode ones benefit too). Without this, §5's cardinality growth would tax every sibling sweeper each tick.
...
`Pipeline.Rebuild` forces truncate on guarded adapters, and `NatsKVAdapter.Truncate` purges the whole bucket — on the shared `capability` bucket that is a platform-wide auth wipe (every sibling lens's keys), healed only at sweep pace. This design **prescribes no rebuilds** (§6), and Fire 1 replaces the hazard with a **prefix-scoped truncate**: a perEntry (and doc-mode capability) lens truncates only keys its own `AnchorFromKey` claims. The Postgres side already embodies this rule (`GrantWriterAdapter` implements no Truncater precisely because the table is shared); the KV side gets the equivalent.
...
- **Reader:** `IsReadable` per-anchor membership incl. tombstone-skip, dual-read union, `anchorID` metacharacter rejection (loud error, not broadened filter).
...
1. **Fire 1 (M–L) — mechanism + first consumer:** `entryKeyColumn` descriptor + validation; driver perEntry emission + §4.2 prefix-diff (tombstones-first, tombstone-skip) + all four prefix-delete consumers incl. shred; §4.3 re-evaluation retry (raw-replay refusal); §4.4 sweep deltas (prefix-scoped survey for all lenses, coverage/orphan membership, `AnchorFromKey` variant); §4.5 prefix-scoped truncate; flip the **bootstrap `capabilityRead` base lens** via the lens-vertex migration; `IsReadable` dual-read + anchor-token hardening. Green: units + retry-path vector + base-lens contract test + PL self-anchor e2e. (The mechanism ships with its live consumer — no dead scaffolding.)
