...
| `derivationFellBack`, `derivationOverCapSize` | first fallback after activation | never (a counter and a last-value; a restart zeroes both, as `reconciled` does) | always on a licensed lens; `0` / absent before the first fallback | — |
...
| `TestPlainDerivation_SecureLensNeighbourDropOutRetracts` (e2e, `plain_derivation_act_e2e_test.go` family) | a Secure lens in `act` mode: the neighbour link two hops out is tombstoned → a seq-guarded soft tombstone lands on the protected table; the row reads absent through RLS | 2 |
...
| `TestRenewalsRead_UnmanagedUnitRetractsExistingRow` (`packages/lease-signing`) | the pipeline-level twin of `TestRenewalsRead_UnmanagedUnitProducesNoRow`: the `manages` link is tombstoned **after** the row exists, and the row is retracted — the existing test proves only the fresh projection | 2 |
