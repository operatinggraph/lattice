...
>
> **As-built amendment (Winston, 2026-07-27, covenant restoration — facet-discovery-restoration-design.md).** The vocabulary gained three additive members: a **`manifest.pane.<paneMetaId>` row family** (`edgeStaffPanes` lens over `holdsRole → offeredTo`; `pkgmgr.PaneSpec` descriptors whose `sections` JSON declares a Protected server-pane's tables/columns/filters/dispatch targets as data — the [APP] widenings §3.4 of the staff-worlds design had sanctioned are re-expressed as [DATA]); **`dispatchVisibleWhen {field, equals}`** on `manifest.op` rows (state-conditional offering, fail-closed on a missing field); and a **`typeLabel`** column on every entity-stamped row (the renderer's per-type label ladder reads data, never a client map). The §1 covenant is now structurally enforced: `scripts/lint-facet-discovery.go` (CI, STRICT) default-denies vertical vocabulary, key types, SQL tables, and op literals in `cmd/facet` — comments included — so any future widening must edit the gate's cited allowlists in the same diff.
...
- **On op metas** (`pkgmgr.OpMetaSpec` grows optional fields → aspects):
  ...
  - `.dispatch` `{class, authContext: self|service|task, targetField?, targetType?, contextParams?, reads?: [templates], optionalReads?: [templates]}` — the machine-readable version of the loftspace `COMPLETIONS` registry. Template substitutions: `{actor}`, `{scopedTo}`, `{service}`, `{payload.<field>}`, `{me.<type>}` (a typed self-anchor from `edgeIdentity`'s `selfAnchors`), `{entity.<column>}` (a projected column of the `manifest.ent` row being viewed — the derivation source this design already named as "the entity being viewed", now expressible; it is how wellness `CancelBooking` fills its `session` from the booking's own `sessionKey` instead of asking for a pasted key, and it is available to any entity-anchored op), each optionally suffixed `:id` to substitute the bare NanoID rather than the full key — this turns today's client-side hardcoded `reads` reconstruction into data. `targetType` names the vertex type `targetField` expects, so a client can tell "not offerable from here" from "broken"; `optionalReads` declares absence-tolerant reads (read-before-create / dedup) and is **never** a required key (Contract #2 §2.5). **The live shape is [`internal/pkgmgr/definition.go:492-528`](../../internal/pkgmgr/definition.go)** — this prose is the intent, that struct is the truth.
...
### 7.5 Fire 5 Inc 2 build note (Winston, 2026-07-17) — clinic catalog-path wiring CLOSED

**Shipped (`72dc9ed`, merged to main):** widened `service-domain`'s `SERVICE_FAMILIES` to add `clinic` (mirrors the §7.3 widening precedent exactly — DDL description, `InputSchema` enum, `FieldDescription`, the script's `SERVICE_FAMILIES` list + `required_family` rejection message, `manifest.yaml` prose, version bump 0.4.0→0.5.0), and extended `TestCreateServiceTemplate_ShowcaseFamilies_Accepted` to cover it. `scripts/seed-showcase.go` mints a "Riverside Clinic" service template (`vtx.service.hqYJYTcdwtPPfD2pPG8c`), wires it `availableAt` the showcase building (mixed-use — the same building the two tenants reside in, mirroring `mixed-use-composition-design.md`'s front-desk-clinic-in-a-residential-building precedent; a separate, unrelated clinic building would never be reachable via the actor's own `containedIn` chain), and `permitsOperation`-links it directly to the three clinic self-scope op-metas (not via `RequestService` — these ops dispatch `authContext=self`, so the catalog link goes straight to each op-meta). Found + fixed mid-fire: the loader's original all-or-nothing `if alive(buildingKey)` idempotency gate silently skipped the new clinic wiring entirely on an already-seeded stack, and a first live-run attempt (blocked on the not-yet-live-reinstalled `clinic-domain` op-metas) left the template + `availableAt` link committed but no `permitsOperation` links — so `seedClinicTemplate` was rewritten to per-mutation idempotency-check (template / `availableAt` link / each `permitsOperation` link independently), resuming cleanly from that exact partial state on rerun. Verified: `go build`, `make vet`, `golangci-lint`, `STRICT=1 lint-conventions`, `go test ./packages/service-domain/... ./packages/clinic-domain/... ./packages/edge-manifest/...` all green. Live-verified: diff-applied `service-domain` 0.5.0 + `clinic-domain` 0.18.0 onto the running showcase stack (`lattice-pkg install --force`, no teardown), ran `seed-showcase` twice (second run no-ops, proving idempotency), then a **fresh** Facet engine's `/api/feed` for tenant1 (`znLu22UYHJFCwHVYE2qn`) carried `manifest.svc.hqYJYTcdwtPPfD2pPG8c` (Riverside Clinic) and all three `manifest.op.*` rows (CreateAppointment/RescheduleAppointment/SetAppointmentStatus) with `viaServices` pointing at the clinic template — the acceptance condition this increment named. Note: the pre-existing, already-connected `:7810` Facet process (started before this fire, not one this fire's sandbox could signal/kill) kept showing its stale pre-fire snapshot even after the data landed — a fresh engine picks it up correctly, so this reads as a long-lived-process cache/subscription staleness on that one instance, not a defect in the shipped wiring; unresolved, not blocking, worth a look if it recurs.

Fire 5 residual now: café/wellness self-scope-op adoption (blocked on those verticals having no self-scope ops at all yet — ordinary PO-filed demand, not a Fire 5 blocker) and the second-renderer (iOS/SwiftUI) spike, both unstarted.

### 7.6 Fire 5 Inc 3 build note (Winston, 2026-07-17) — café self-scope adoption Inc 1 (package layer)

Closes half of the residual named in §7.5: café-domain had zero self-scope ops (every op operator-only, POS-submitted). Mirrors wellness-domain's own Fire 5 Inc 1 (`ac21a6c`, shipped the same day) — a resident-invocable slice of the existing op set, package-layer only, FE checkpointed as a later increment (café-app has no per-resident login at all today; it is a single-admin-actor POS, unlike loftspace/clinic/wellness-app).

**Shipped:** `OpenTab` and `Settle` now also grant `consumer`, `scope=self` (`packages/cafe-domain/permissions.go` + `manifest.yaml`). Unlike wellness's `CreateBooking`/`CancelBooking` — where the booker *is* an identity directly — a café tab is anchored to a `leaseapp`, so the Starlark self-scope guard uses clinic-domain's patient/identifiedBy indirection instead: it resolves the lease's own `applicationFor→identity` link (lease-signing's convergence link, already read by wellness's resident-rate check) and requires it name the caller's `authContext.target`, failing closed (`AuthDenied`) otherwise. `OpenTab` reads the link from its own validated `leaseAppKey` payload field; `Settle` recovers the lease from the tab's `.status` aspect (already a declared read for OCC), never from caller input. `Charge` deliberately stays operator-only — it takes a raw `amountCents` with no catalog/menu to bound a self-submitted amount against, unlike a booking/appointment which claims an already-existing, provider-defined slot. Verified: 4 new integration tests (self-scope allow/reject × OpenTab/Settle) + the full existing suite green, `go build`, `make vet`, `golangci-lint`, `STRICT=1 lint-conventions` all clean.

**Residual, next increments:** (1) edge-manifest descriptor metadata (`opmetas.go`) + a café service-template catalog-path wiring, mirroring clinic's Inc 1/Inc 2 (§7.4/§7.5) — makes `OpenTab`/`Settle` Facet-visible; (2) café-app FE gains real per-resident auth (currently single-admin-actor POS only) to actually exercise self-service in-browser; (3) wellness adoption is otherwise complete (Inc 1 `ac21a6c` + descriptor metadata `0d16c34` + catalog wiring `97db230`, all shipped 2026-07-17) — only the second-renderer (iOS/SwiftUI) spike remains unstarted platform-wide.

### 7.7 Fire 5 Inc 4 build note (Winston, 2026-07-17) — café descriptor metadata + catalog-path wiring CLOSED

Closes residual (1) named in §7.6: `cafe-domain.OpMetas()` (`OpenTab`/`Settle`, mirroring clinic/wellness Fire 5 Inc 1) + `service-domain`'s `SERVICE_FAMILIES` widened to admit `cafe` (mirrors §7.3/§7.4's widening precedent) + `seed-showcase.go`'s `seedCafeTemplate` mints a "Riverside Café" service template, `availableAt` the showcase building, `permitsOperation`-linked directly to the two op-metas (per-mutation idempotent, mirroring `seedClinicTemplate`/`seedWellnessTemplate`). `leaseAppKey` (`OpenTab`) has no `dispatch.targetField`/`contextParams` auto-fill — unlike clinic's provider or wellness's session, a tab is a fresh vertex with no pre-existing "entity being viewed" to derive it from, so it is prose-described the same way clinic's own non-auto-fillable `patient` field is. Verified: `go build`, `make vet`, `golangci-lint` (full repo), `STRICT=1 lint-conventions`, `go test ./packages/cafe-domain/... ./packages/service-domain/... ./packages/edge-manifest/...` all green. Live-verified: diff-applied `service-domain` 0.7.0 + `cafe-domain` 0.2.0 onto the running showcase stack (`lattice-pkg install --force`, no teardown), ran `seed-showcase` twice (second run no-ops), then a fresh Facet engine's SSE feed for tenant1 carried `manifest.svc.7HRxY1Ymcjv2kuWoR1uC` (Riverside Café) and both `manifest.op.*` rows (OpenTab/Settle) with `viaServices` pointing at the café template.

Fire 5 residual now: café-app real per-resident auth (to actually exercise self-service in-browser — café-app is still single-admin-actor POS today) and the second-renderer (iOS/SwiftUI) spike, both unstarted.

### 7.8 Fire 5 Inc 5 build note (Winston, 2026-07-17) — café-app real per-resident auth CLOSED

Closes residual (2) named in §7.6: café-app's Resident view now has a real per-resident Me bar (mirroring `cmd/wellness-app`'s), so §7.6's `OpenTab`/`Settle` consumer scope=self grant is actually exercisable in-browser, not just at the package layer. New `/api/residents` (mirrors `cmd/wellness-app/residents.go` byte-for-byte in shape) decodes the shared `leaseApplicationComplete` convergence lens (`weaver-targets`, already read by `tabs.go`'s `handleTabs` under the same bucket) — the only P5 surface that carries lease→applicant identity, since `cafeLeaseAccounts` (the existing lease picker, `leases.go`) is keyed by lease and only gets a row once a tab has ever settled. New `handleDevToken` mints a token for the signed-in resident's own identity (mirrors `cmd/wellness-app`/`cmd/clinic-app`'s own). The Resident view, when signed in, locks the lease picker to the resident's own lease (`leaseAppKeyForBooker`) and renders Open Tab / Settle Tab actions that submit `asSelf`, stamping `authContext.target` and declaring `lnk.leaseapp.<lease_id>.applicationFor.identity.<target_identity_id>` in `optionalReads` — the exact class-(d) declared read `packages/cafe-domain/ddls.go`'s self-scope guard requires (verified the FE's key construction against the Starlark's own `"lnk.leaseapp." + lease_id + ".applicationFor.identity." + target_identity_id"`, character for character). Charge stays out of the Resident view (operator-only, §7.6). Verified: `go build ./...`, `make vet`, `golangci-lint run ./...` (full repo), `STRICT=1 lint-conventions` all green; new `computeResidents`/`handleDevToken` unit tests pass. Live-verified in-browser (rebuilt `bin/cafe-app`, cycled on the running showcase stack): Me bar renders, staff (signed-out) Resident/POS/Front Desk views unchanged, zero console errors. Did NOT live-verify the signed-in write path end-to-end — the showcase stack currently carries zero lease applications (a fresh `vtx.leaseapp.*` scan came back empty) and loftspace-app's own dev sign-in is hitting a pre-existing, unrelated `RotateClaimKey requires state=unclaimed, got claimed` error for its seeded identities, so minting a real resident+lease fixture needs a loftspace-app fix first — flagging as a residual, not swallowing it as covered. Confidence instead rests on: cafe-domain's own 4 shipped integration tests already cover the Starlark self-scope allow/reject (§7.6, `a5828c5`); the FE's link-key construction is character-verified against the script; and the shape mirrors wellness-app's already-verified Me bar precedent almost exactly.
