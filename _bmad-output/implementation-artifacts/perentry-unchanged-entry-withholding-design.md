...
| T9 | **e2e on the real producer:** in the capability e2e harness, one `providedTo` link create against a populated **`edgeManifest`** domain writes exactly the new/dropped entries; the audit subject gains exactly that many; a second identical event writes zero. *(Corrected at build, 2026-09-05: `providedTo` appears in exactly one anchor walk, `edgeInstances`, declared `GrantDomain: domainBase`; every `domainStaff` walk is a held-role / `worksAt` spine, so the create the first draft paired with the staff domain cannot reach it. The producer under test is the generated `edgeManifestReadGrants` — C2's second and third entities were `instanceOf` links, the same domain.)* | `edge_manifest_withholding_exact_writes_e2e_test.go` — 2 instances × 3 identities, a fourth identity's `providedTo` reaching 4 actors / 7 entries: 1 write + 6 withheld, then 0 + 7; every unchanged key's `revision` and `projectionSeq` unmoved. *(The fourth identity's vertex is created in the population phase: written beside the link, its event's evaluation reads live KV and races the link put — 7 withheld on CI run 34003358137, 2026-09-06.)* | 2 |
